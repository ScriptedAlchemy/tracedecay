...
- Hermes profiles bound to a project use the user-level TraceDecay store, scoped to the current project by default; repo-level `.tracedecay` and legacy `.tokensave` databases are fallback/local-only stores, not the default.
...
Never run bare `cargo` against the repo `target/` — it is the user's ~399 GB interactive dir and is contended. Full-workspace integration checks use the serialized `/tmp/tracedecay-target/integration/` lane (one at a time). Reclaim disk with `rm -rf "$CARGO_TARGET_DIR"` before completing. Full policy: `docs/CARGO-CONTENTION-POLICY.md`.
