...
- Finish and merge the worktree lanes already in flight, then delete their merged worktrees; after that, future subagents work directly in the main checkout and must not create new worktrees. Re-read files immediately before editing, never revert or check out a peer's files, and commit only the paths you changed: never `git add -A`, isolate with a temporary index seeded from HEAD (`GIT_INDEX_FILE=$(mktemp -u) git read-tree HEAD`), and verify every claimed file landed with `git cat-file -e HEAD:<path>`. Never `git stash` either: the stash stack is repository-wide rather than per-worktree (`refs/stash` lives in the shared common git dir), so a pop can land a peer's work in your tree.
...
- There is exactly one dashboard. The Hermes plugin serves the existing embedded dashboard rather than shipping its own UI, bundles, or explorers, and the generated contracts module (`dashboard/src/contracts/`, maintained by the `contracts:generate` and `contracts:check` scripts) is the only Rust-to-dashboard wire boundary, with hand-written request/response shapes forbidden. `contracts:check` re-derives from live Rust schemas and catches generated-output drift, but route-to-catalog completeness is a separate invariant because a route omitted from the catalog gives that check no contract to compare.
