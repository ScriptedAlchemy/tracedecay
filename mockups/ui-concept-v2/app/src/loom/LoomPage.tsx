...
export function LoomPage(props: { state?: LoomStateId; onState?: (id: LoomStateId) => void } = {}) {
  ...
  const visibleSubagentCount = new Set(navigationSessions.filter(session => session.isSubagent && session.agentId).map(session => session.agentId)).size;
  ...
  return <section className="aperture is-loom"><div data-workspace={nav.workspace} className={`loom journey ${nav.level === 'workstream' && !compact ? 'is-dense' : ''}`}>
    ...
    <div className="loom-head"><div className="loom-head-l"><b>LOOM / {nav.workspace === 'evidence' ? 'SELECTED EVENT' : nav.workspace === 'feedback' ? 'FEEDBACK CONTINUATION' : nav.workspace === 'gaps' ? 'EVIDENCE GAPS' : proximityActive ? 'CODE PROXIMITY' : nav.replaying ? 'TEMPORAL REPLAY' : broad ? nav.level === 'workstream' ? 'LIVE EXECUTION' : 'OUTCOME OVERVIEW' : nav.level === 'agent' ? 'BRANCHING EXECUTION' : nav.follow ? 'FOLLOW LOADED TAIL' : 'EXECUTION INSPECTION'}</b><em title={compact ? eventContext ?? undefined : undefined}>{compact && selectedEvent ? <>{SOURCE.design ? 'CONCEPT / SYNTHETIC DATA' : `${SOURCE_ID.toUpperCase()} SNAPSHOT`} · {eventContext}</> : <>{!broad ? `${nav.project ?? 'ALL PROJECTS'}${nav.provider ? ` / ${nav.provider}` : ''} · ` : ''}{nav.focus ? `SESSION ${shortId(nav.focus, 14)}` : `${visibleAgentCount} ${SOURCE.design ? 'ILLUSTRATIVE AGENTS' : 'CAPTURED AGENT IDS'} · ${SOURCE.design ? `${visibleSubagentCount} INCLUDED SUBAGENTS` : `${navigationSessions.length} SESSIONS`}`}{!broad ? ` · ${SOURCE.design ? 'CONCEPT / SYNTHETIC DATA' : `${SOURCE_ID.toUpperCase()} SNAPSHOT`}` : ''}</>}</em></div>
