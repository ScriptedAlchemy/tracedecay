...
function FixtureWork() {
  ...
  return <div className="wk-root is-fixture"><main className="wk-main"><ProjectionTabs mode="fixture" selected={projection} onPick={(next) => { setProjection(next); refitAfterRender(); }} />
    <section className="wk-panel wk-fixture-board"><header className="wk-panel-head"><b>CANONICAL TASK GRAPH</b><span>revision {graph.revision} · explicit fixture · {graph.tasks.length} stable TaskIds</span></header>
      ...
      </div><div className="wk-canvas-foot">{sceneScope !== "overview" && <div className="wk-plan-contexts">{FIXTURE_PLANS.filter((plan) => plan.id !== anchor.planId).map((plan) => { const tasks = graph.tasks.filter((task) => task.planId === plan.id); const alerts = tasks.filter((task) => task.status === "blocked" || task.status === "failed" || task.status === "waiting").length; return <button type="button" className="wk-plan-context" key={plan.id} onClick={() => { setSelectedId(tasks[0].id); setAnchorId(tasks[0].id); setScope("outcome"); moveCamera(FOCUS_CAMERA); }}><code>{plan.id}</code><b>{plan.title}</b><span>{tasks.length} tasks · {alerts} attention</span></button>; })}</div>}<span><b>{selected.id}</b> · {sceneScope === "overview" ? "portfolio overview" : `${sceneScope} focus from ${anchor.id}`} · {shownTasks.length} of {graph.tasks.length} tasks</span><span>Only solid cyan arrowheads gate readiness.</span><div className="wk-camera-help">DRAG PAN · WHEEL ZOOM · 0 FIT</div><Minimap graph={graph} projection={projection} scope={sceneScope} tasks={shownTasks} points={points} selectedId={selected.id} camera={camera} viewport={viewport} onFit={fitCurrent} /></div>
    </section>
