...
- **Args:** interpret the text after the command as either a single target branch to compare against the current branch, or "<base> <head>" to diff two branches; if absent, start with `tokensave_branch_list` and ask what to search/compare.
