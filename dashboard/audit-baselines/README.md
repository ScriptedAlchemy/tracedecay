...
`pnpm run visual:audit` renders every registered surface
...
`pnpm run visual:audit -- --diff` re-runs the capture and, for each screenshot,
...
1. Run `pnpm run visual:audit` and review `../audit-gallery/`.
