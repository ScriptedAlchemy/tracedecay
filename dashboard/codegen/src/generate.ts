 ...
 *   - Generated unions are discriminated and exhaustively switchable; the file
 *     emits an `assertNever` helper for `never` checks.
 *   - Unknown enum/union variants decode to a typed `unsupported_schema` branch
 *     instead of throwing. Malformed *known* variants still produce a typed
 *     decode error (they are not silently downgraded).
 ...
 * string enums, arrays, and internally-tagged `oneOf` unions), plus two
 * `x-generic*` extension keys used to carry the `DashboardEnvelope<T>` generic
 * across the schema boundary (schemars monomorphizes generics; the fixture
 * marks the generic explicitly so the emitted TS stays generic).
 */
