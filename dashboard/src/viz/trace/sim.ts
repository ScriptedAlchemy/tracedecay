 ...
 *
 * Lifted essentially verbatim from the round-two live prototype
 * (`mockups/code-topography/prototype/sim.js`, branch
 * `worktree-agent-af882d6565fbab159`) and typed. The physics, the tuning
 * defaults and the numeric contract are unchanged, because they are what the
 * owner reviewed; the diff against the prototype is types, not behaviour.
 *
 * This module is the honesty boundary. Every felt quantity — weight, latency,
 * deformation, settle time — is computed HERE from a stated measurement, and
 * the renderer only draws what comes out. Nothing in this file touches the DOM,
 * a canvas, `Date.now`, `performance.now` or `Math.random`, so the same seed
 * and the same gesture script produce a bit-identical trajectory on every
 * machine and inside vitest.
 *
 ...
 *           layout is an exact equilibrium: the live field at rest is the
 *           static sheet, pixel for pixel.
 ...
 * damping coefficient) the closed form of the anchored oscillator gives a
 * settle time monotone in mass, which is the "hubs are slow and deep, leaves
 * flick" clause of the sensory contract — and `sim.test.ts` asserts the
 * monotonicity rather than trusting the algebra.
 */

/**
 * Tuning defaults. Every one of these is a feel knob; the prototype README
 * carries the table with the reasoning and the measured consequence of each,
 * and the values below are that table's committed column.
 */
export interface SimParams {
  anchorBase: number;
  anchorMassExponent: number;
  edgeStiffnessScale: number;
  dampingRatio: number;
  substep: number;
  restSpeed: number;
  minMass: number;
...
export const DEFAULT_PARAMS: Readonly<SimParams> = Object.freeze({
   ...
   * spread, which reads as weight without stalling.
   */
...
export interface SimNodeSpec {
  ...
  /** Layout anchor, world coordinates. */
  readonly x0: number;
  readonly y0: number;
}

export interface SimSpringSpec {
  readonly a: string;
  readonly b: string;
  /** The edge's call-site count. */
  readonly stiffness: number;
  /** Defaults to the distance between the two anchors. */
  readonly restLength?: number;
}

export interface SimSpec {
  readonly nodes: readonly SimNodeSpec[];
  readonly springs: readonly SimSpringSpec[];
  /** Seed for the startup displacement only. */
...
export interface Simulation {
  ...
  readonly params: Readonly<SimParams>;
  readonly stepCount: number;
  readonly substepCount: number;
  indexOf(nodeId: string): number;
  massOf(nodeId: string): number;
  anchorOf(nodeId: string): { x: number; y: number };
  /** Channel stiffnesses incident on a node, keyed by the other endpoint. */
  springsOf(nodeId: string): Array<{ other: string; stiffness: number; restLength: number }>;
  step(dt: number): Simulation;
  applyDrag(nodeId: string, targetX: number, targetY: number): Simulation;
  release(): Simulation;
  pinnedIds(): string[];
  ...
  isSettled(restSpeed?: number): boolean;
  settle(options?: { dt?: number; maxFrames?: number; restSpeed?: number }): number;
}
...
export function createSimulation(spec: SimSpec): Simulation {
  ...
  const adjacency = new Map<string, Array<{ spring: number; other: string }>>();
  ...
  spec.springs.forEach((spring, i) => {
    ...
    if (a === undefined) {
      throw new TypeError(`springs[${i}].a references unknown node ${String(spring?.a)}`);
    }
    if (b === undefined) {
      throw new TypeError(`springs[${i}].b references unknown node ${String(spring?.b)}`);
    }
  ...
  function resolveIndex(nodeId: string): number {
  ...
  const sim: Simulation = {
     ...
     * `params.substep`. Callers MUST pass a fixed `dt` (the surface drives one
     * fixed step per animation frame) — wall-clock jitter never reaches the
     * integrator, which is the whole reason a run can be replayed.
     */
    step(dt) {
      requireFinite(dt, 'dt');
     ...
     * pinned, this can only fall; `sim.test.ts` asserts exactly that.
     */
    energy() {
      const inv = 1 / params.substep;
      let kinetic = 0;
      ...
      for (let s = 0; s < springCount; s += 1) {
        const stretch =
          Math.hypot(x[springB[s]!]! - x[springA[s]!]!, y[springB[s]!]! - y[springA[s]!]!) -
          springRest[s]!;
     ...
     * it is asserted in `sim.test.ts`, not asserted in prose.
     */
...
/**
 * Hop distance over the channel graph, ignoring direction.
 *
 * @returns id → hops, unreachable ids omitted.
 */
export function hopDistances(sim: Simulation, sourceId: string): Map<string, number> {
  const seen = new Map<string, number>([[sourceId, 0]]);
