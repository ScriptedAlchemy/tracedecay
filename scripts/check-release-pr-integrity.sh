...
while IFS=$'\t' read -r status path _; do
  [[ -n ${status:-} ]] || continue
  case "$path" in
    .release-please-manifest-beta.json | .release-please-manifest.json | CHANGELOG.md | Cargo.lock | Cargo.toml | crates/tracedecay-cli/Cargo.toml | server.json | version.txt)
      if [[ $status != M ]]; then
