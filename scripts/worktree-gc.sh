...
# are GC candidates. DIRTY and ACTIVE trees are never removed, even with
# --force. Orphaned /fast/tmp scratch cargo dirs are reported only.
#
# usage: scripts/worktree-gc.sh [--delete] [--force] [--integration REF]
#                               [--max-behind N] [--repo PATH]
...
Options:
  ...
  --force                  With --delete, pass --force to `git worktree remove`
                           (never `git branch -D`; never overrides DIRTY/ACTIVE)
...
Classification (linked worktrees only; the primary checkout is PRIMARY):
  ...
  STALE-MERGED    clean, on a local branch fully merged into the integration
                  branch with no upstream of its own — GC candidate + `branch -d`
...
classify_worktree() {
    ...
    local branch dirty pids verdict extra=""
    ...
    if ((dirty > 0)); then
        printf '%s\n' "DIRTY|uncommitted changes (${dirty} paths) — never touchable"
        return
    fi

    pids="$(active_pids_for "$path")"
    if [[ -n "$pids" ]]; then
        printf '%s\n' "ACTIVE|process cwd inside worktree (pid ${pids}) — never touchable"
        return
    fi

    if ((locked == 1)); then
        printf '%s\n' "FOREIGN|locked; owner's call"
        return
    fi
    ...
    if [[ -n "$head" ]] && is_ancestor "$head" "$INTEGRATION_TIP"; then
        if [[ -n "$extra" ]]; then
            printf '%s\n' "STALE-MERGED|local branch ${branch} is fully merged into ${INTEGRATION_NAME} and has no upstream; ${extra}"
        else
            printf '%s\n' "STALE-MERGED|local branch ${branch} is fully merged into ${INTEGRATION_NAME} and has no upstream"
        fi
        return
    fi
...
print_report() {
    ...
    local n_primary=0 n_dirty=0 n_active=0 n_current=0
    ...
    printf '\n'
    printf 'summary  %d worktrees  primary=%d  dirty=%d  active=%d  current=%d  foreign=%d  stale-detached=%d  stale-merged=%d\n' \
        "${#WT_PATH[@]}" \
        "$n_primary" \
        "$n_dirty" \
        "$n_active" \
        ...
        "$n_stale_det" \
        "$n_stale_mer"
    printf 'gc candidates are STALE-DETACHED and STALE-MERGED only; DIRTY/ACTIVE are never touchable\n'
}
...
delete_candidates() {
    local i path status branch rc=0 remove_ok=0
    local removed=0 skipped=0 failed=0

    printf '\n'
    printf 'delete mode: removing GC candidates only\n'
    for i in "${!WT_PATH[@]}"; do
        path="${WT_PATH[$i]}"
        status="${WT_STATUS[$i]}"
        case "$status" in
        ...
        if [[ "$status" == STALE-MERGED ]]; then
            ...
            if [[ -n "$branch" ]] && is_ancestor "${WT_HEAD[$i]}" "$INTEGRATION_TIP"; then
                printf '  branch -d %s  (fully merged into %s)\n' "$branch" "$INTEGRATION_NAME"
