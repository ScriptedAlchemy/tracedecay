...
(
  cd "$build_root"
  cargo test --quiet --release -p tracedecay --features test-helpers \
    --test claude_observation_benchmark production_observation_pipeline_baseline -- \
...
(
  ...
  TRACEDECAY_BENCHMARK_EVIDENCE_DIR="$repo_root/benchmark_data/claude-observation" \
    cargo test --quiet --release -p tracedecay --features test-helpers \
      --test claude_observation_benchmark evidence_directory_matches_index_contract -- \
