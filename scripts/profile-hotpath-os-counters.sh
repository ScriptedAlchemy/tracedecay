...
#
# The report also binds the runtime CPU-0% hypothesis to
# src/runtime_telemetry.rs::sample_process_with_window (Agent 1 owns the fix).
#
...
print("cpu0", "%s::%s" % (hyp["file"], hyp["function"]), "still_true=%s" % hyp["still_true_on_linux_head"])
' "$out_dir/report.json"
