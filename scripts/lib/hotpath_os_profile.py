...
# sysinfo 0.32.1 on Linux only computes Process.cpu_usage after
# ProcessesToUpdate::All. TraceDecay samples Some(&[pid]) in
# src/runtime_telemetry.rs::sample_process_with_window.
CPU_TELEMETRY_HYPOTHESIS = {
    "id": "runtime-cpu-percent-stays-zero",
    "still_true_on_linux_head": True,
    ...
    "fix_owner": "agent-1",
    "fix": (
        "Keep the PID-scoped refresh (Windows stack bound). Do not trust "
        "sysinfo Process::cpu_usage after ProcessesToUpdate::Some. Compute "
        "cpu_percent from /proc/self/stat utime+stime deltas over the sample "
        "window (same field split as "
        "src/daemon/semantic_evaluation.rs::read_linux_process_cpu_ticks). "
        "Assert cpu_percent > 0 after a CPU-burning window; "
        "sample_process_fits_in_a_small_stack uses Duration::ZERO and never "
        "checks CPU."
    ),
}
