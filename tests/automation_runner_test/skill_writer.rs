...
async fn skill_writer_runner_stages_consolidations_without_auto_apply() {
    ...
    // auto_enable_skills=true proves consolidations are still staged, never
    // auto-applied.
    let config = AutomationConfig {
        auto_enable_skills: true,
        ..enabled_skill_writer_config()
    ...
    // Overlap evidence surfaced the pair and recommended consolidation.
    assert!(run.report["skill_improvement_recommendations"]
        .as_array()
        .is_some_and(
            |recommendations| recommendations.iter().any(|recommendation| {
    ...
    // Nothing auto-applied: both sides only carry staged pending updates.
    ...
    // Approving the staged consolidation archives the source (content kept)
    // and applies the merged body to the target.
