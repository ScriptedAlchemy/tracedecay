...
//! through owned refs and synthetic worktrees; fork PRs (a
//! `refs/pull/N/head` whose SHA matches no origin head) are skipped. The tests
//! assert that managed state and provenance remain exact, a second poll is a
//! no-op, closing a PR cleans its owned Git artifacts, and the per-cycle
//! new-track cap ramps.
...
async fn tracks_same_repo_pr_indexes_its_content_and_untracks_on_close() {
    ...
    // Managed state reflects the tracked PR.
    ...
    // Facts belong to the project. Branch, ref, worktree, and commit are
    // provenance only, so retiring any of those artifacts must not move,
    // merge, or delete the fact.
    let durable_content = "Project fact survives managed PR worktree retirement";
    let durable_fact = fixture
        .graph()
        .add_fact(AddFactRequest {
            content: durable_content.to_owned(),
            category: MemoryCategory::Project,
            source: Some("pr-autotrack".to_owned()),
            tags: vec!["managed-pr".to_owned()],
            entities: Vec::new(),
            trust: Some(0.9),
            metadata: serde_json::json!({
                "branch": tracking_label,
                "ref": "refs/tracedecay/pr/1",
                "worktree": "pr-worktrees/pr-1",
                "commit": discovery.open[0].head_sha.clone(),
            }),
        })
        .await
        .unwrap()
        .fact
        .expect("project fact must be stored");
    ...
    // Advance the remote PR head. Reconciliation must refresh managed
    // provenance instead of serving the previous commit indefinitely.
    fixture.git(&[
        "checkout",
        "-b",
        &head_branch,
    ...
    assert_eq!(
        pr_autotrack::managed_summary(&data_root)[0].head_sha,
        refreshed_discovery.open[0].head_sha,
        "changed PR head must replace stale managed provenance",
    );
    let after_head_refresh = fixture
        .graph()
        .get_fact(durable_fact.fact_id)
        .await
        .unwrap()
        .expect("project fact must survive managed provenance refresh");
    assert_eq!(after_head_refresh.content, durable_content);
    ...
    let after_worktree_retirement = fixture
        .graph()
        .get_fact(durable_fact.fact_id)
        .await
        .unwrap()
        .expect("project fact must survive branch and worktree retirement");
    assert_eq!(after_worktree_retirement.content, durable_content);
}
...
/// with no state. Once the PR head advances, the old `-b` path wedged
...
async fn interrupted_track_with_advanced_head_recovers_instead_of_wedging() {
    ...
    assert!(
        ...
        "recovered worktree generation serves the advanced head's content"
    );
}
...
/// than stranding worktrees, refs, and synthetic branches forever.
#[tokio::test]
