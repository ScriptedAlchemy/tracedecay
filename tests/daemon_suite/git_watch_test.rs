...
/// A deferred initial worktree sync must leave no completed receipt behind:
/// retrying after the shared store lock clears creates and synchronizes the
/// worktree's own graph.
#[tokio::test]
async fn deferred_worktree_tracking_retries_to_a_synchronized_store() {
    let (_root, project, harness) = init_production_indexed_repo().await;
    let worktree = project.parent().unwrap().join("wt-deferred");
    git(
        &project,
        &[
            "worktree",
    ...
    let data_root = harness.project_data_root(&project).await.unwrap();
    let sync_lock = tracedecay::locking::try_acquire_sync_lock_at(&data_root.join("sync.lock"))
        .expect("test must hold the parent store sync lock");

    let deferred = harness
        .track_worktree_branch(&project, &worktree, "feat/deferred")
        .await
        .unwrap();
    assert_eq!(deferred, BranchAddOutcome::Deferred);
    assert!(
        load_branch_meta(&data_root).is_none_or(|meta| !meta.is_tracked("feat/deferred")),
        "a deferred first sync must roll back its branch receipt"
    );

    drop(sync_lock);
    fs::write(
        worktree.join("src/deferred_retry.rs"),
        "pub fn deferred_retry_symbol() {}\n",
    )
    .unwrap();
    commit_all(&worktree, "deferred worktree retry");
    ...
    let meta = load_branch_meta(&data_root).expect("retry must publish branch metadata");
    let database = data_root.join(&meta.branches["feat/deferred"].db_file);
    assert_eq!(
        ...
        .unwrap(),
        1,
        "retry must synchronize the deferred worktree rather than accept a stale receipt"
    );
}

/// A mounted owner graph must refresh an already-tracked linked worktree after
/// its HEAD advances; it cannot reject the linked root for lacking its own
/// retained project server.
#[tokio::test]
async fn linked_worktree_head_advance_catches_up_through_its_owner_graph() {
    let (_root, project, harness) = init_production_indexed_repo().await;
    let worktree = project.parent().unwrap().join("wt-owner-refresh");
    git(
        &project,
        &[
            "worktree",
    ...
    assert_eq!(
        harness
            .track_worktree_branch(&project, &worktree, "feat/owner-refresh")
            .await
            .unwrap(),
        BranchAddOutcome::Added
    );
    ...
    fs::write(
        worktree.join("src/owner_refresh.rs"),
        "pub fn owner_refresh_symbol() {}\n",
    )
    .unwrap();
    commit_all(&worktree, "owner graph refresh");
    ...
    let data_root = harness.project_data_root(&project).await.unwrap();
    let meta = load_branch_meta(&data_root).expect("linked branch metadata");
    let database = data_root.join(&meta.branches["feat/owner-refresh"].db_file);
    assert_eq!(
        rusqlite::Connection::open_with_flags(
            database,
            rusqlite::OpenFlags::SQLITE_OPEN_READ_ONLY,
        ...
        .unwrap(),
        1,
        "linked HEAD advance must synchronize through the retained owner graph"
    );
}
