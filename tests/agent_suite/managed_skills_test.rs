use tracedecay::automation::managed_skills::{
    ...
    set_managed_skill_state, stage_managed_skill_archive, stage_managed_skill_update,
...
async fn staged_managed_skill_archive_keeps_content_until_approval() {
    ...
    // Staging must not touch the active revision.
    ...
    // Approval archives without deleting any content.
    let approved = approve_managed_skill(&profile_root, "repo-hygiene")
        .await
        .unwrap();
...
async fn staged_managed_skill_archive_rejects_pinned_stale_and_duplicates() {
    ...
    // Already-archived skills cannot be staged for archive again.
