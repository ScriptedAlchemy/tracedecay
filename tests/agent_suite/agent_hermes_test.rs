...
fn manifest_list(manifest: &str, key: &str) -> Vec<&str> {
    let (_, tail) = manifest
        .split_once(&format!("{key}:\n"))
        .unwrap_or_else(|| panic!("generated manifest must contain {key}"));
    tail.lines()
...
fn test_hermes_user_install_writes_single_plugin() {
    ...
    let provided_tools = manifest_list(&manifest, "provides_tools");
    assert!(provided_tools.contains(&"tracedecay_context"));
    assert!(provided_tools.contains(&"tracedecay_lcm_status"));
    assert!(provided_tools.contains(&"tracedecay_lcm_doctor"));
    assert!(
        !provided_tools.contains(&"tracedecay_lcm_compress")
            && !provided_tools.contains(&"tracedecay_lcm_session_boundary"),
        "LCM compaction and session-boundary reporting are hook-runtime actions, not public tools"
    );
    assert_eq!(
        manifest_list(&manifest, "provides_hooks"),
        ["pre_llm_call", "post_tool_call"],
    );
    assert!(manifest_list(&manifest, "provides_commands").contains(&"/tracedecay_status"));
...
#[test]
fn test_hermes_install_emits_public_lcm_tool_contract() {
    let home = TempDir::new().unwrap();
    ...
    let manifest =
        std::fs::read_to_string(home.path().join(".hermes/plugins/tracedecay/plugin.yaml"))
            .unwrap();
    let schemas = read_json(&home.path().join(".hermes/plugins/tracedecay/schemas.json"));
    let schema_names: Vec<&str> = schemas
        .as_array()
        .expect("generated schemas.json must be an array")
        .iter()
        .filter_map(|schema| schema["name"].as_str())
        .collect();
    let public_lcm_tools = [
        "tracedecay_lcm_grep",
    ...
    let manifest_tools = manifest_list(&manifest, "provides_tools");
    for name in public_lcm_tools {
        assert!(
            manifest_tools.contains(&name),
            "plugin manifest must expose the public {name} tool"
        );
        assert!(
            schema_names.contains(&name),
            "plugin schema output must expose the public {name} tool"
        );
    }
    for private_action in [
        "tracedecay_lcm_compress",
        "tracedecay_lcm_compact",
        "tracedecay_lcm_session_boundary",
    ] {
        assert!(
            !manifest_tools.contains(&private_action) && !schema_names.contains(&private_action),
            "{private_action} is a hook-runtime action, not a public Hermes tool"
        );
    }

    let doctor_schema = schemas
        .as_array()
        .unwrap()
        .iter()
        .find(|schema| schema["name"] == "tracedecay_lcm_doctor")
        .expect("doctor must have a generated public schema");
    assert_eq!(
        doctor_schema["parameters"]["properties"],
        serde_json::json!({}),
        "doctor is read-only and accepts no repair or mutation controls"
    );
}
...
assert "tracedecay_lcm_compact" not in registered
assert "tracedecay_lcm_session_boundary" not in registered
...
assert calls and calls[0][0] == "tracedecay_hook_runtime"
