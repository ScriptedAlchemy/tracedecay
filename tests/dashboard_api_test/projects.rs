...
fn project_scoped_plugin_routes_read_selected_project_store() {
    ...
    runtime.block_on(async {
        ...
        // projection and canonical fact identity stay coherent. The dashboard
        // reads facts through that projection, so a manufactured
        // `memory_facts` row without a canonical identity is invisible.
        target_cg
