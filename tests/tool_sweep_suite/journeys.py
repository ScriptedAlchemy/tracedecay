...
def _rename_plan(
    ...
    *,
    ...
    response = _checked_call(
        ...
        calls,
        role="producer-plan",
        tool="tracedecay_api_migration_plan",
        arguments={
            "family_id": family_id,
            "operations": [
                {
                    "kind": "rename_bound_symbol",
                    "operation_id": f"{family_id}.rename",
                    "depends_on": [],
                    "symbol": {
                        "node_id": node_id,
        ...
        deadline_ms=deadline_for("tracedecay_api_migration_plan"),
    )
    for value in _objects(response):
        if (
            ...
            return value
    raise JourneyError("rename planner omitted its exact immutable plan", calls)


def _renamed_symbol_identity(
    ...
    for value in _objects(response):
        ...
        if (
            ...
            and (
                name == expected_name
                or qualified_name.endswith(f"::{expected_name}")
            )
        ):
...
def _rename_symbol_journey(
    ...
    calls: list[JourneyCall] = []
    source = fixture.root / fixture.file
    baseline = source.read_bytes()
    old_name = "sweep_anchor"
    new_name = "sweep_anchor_renamed"
    plan = _rename_plan(
        client,
        runtime,
        calls,
        deadline_for,
        family_id="tool-sweep.rename.forward",
    ...
    preview = _checked_call(
        ...
        deadline_ms=deadline_for("tracedecay_rename_symbol"),
    )
    expected_state = _required_expected_state(preview, calls, "rename preview")
    arguments = {
        "plan": plan,
        "plan_digest": plan_digest,
        "dry_run": False,
        "verify": True,
        "idempotency_key": "tool-sweep.tracedecay_rename_symbol.forward",
        "expected_state": expected_state,
        "format": "json",
    }
    ...
    def verify(response: dict[str, Any]) -> str | None:
        if not _has_true(response, "success"):
            return "rename apply did not report success"
        effect_id = _first_value(response, {"effect_id"})
        replay = _checked_call(
            client,
            runtime,
        ...
        if not _has_true(replay, "replayed"):
            return "rename retry did not replay its durable receipt"
        if not isinstance(effect_id, str) or _first_value(replay, {"effect_id"}) != effect_id:
            return "rename retry changed its durable effect identity"
        candidate = source.read_text()
        if f"fn {new_name}" not in candidate or f"fn {old_name}" in candidate:
            return "rename receipt did not match exact declaration bytes"
        return None

    def cleanup(_response: dict[str, Any] | None) -> str:
        search = _checked_call(
            client,
            runtime,
            calls,
            role="rollback-identity-producer",
            ...
            deadline_ms=deadline_for("tracedecay_search"),
        )
        identity = _renamed_symbol_identity(search, new_name)
        if identity is None:
            raise JourneyError(
                "rename rollback could not resolve the renamed graph identity", calls
            )
        inverse_plan = _rename_plan(
            client,
            runtime,
            calls,
            deadline_for,
            family_id="tool-sweep.rename.rollback",
            node_id=identity[0],
        ...
        inverse_digest = inverse_plan["plan_digest"]
        inverse_preview = _checked_call(
            client,
            runtime,
            calls,
        ...
        inverse_state = _required_expected_state(
            inverse_preview, calls, "rename rollback preview"
        )
        inverse = _checked_call(
            client,
            runtime,
            ...
            arguments={
                ...
                "dry_run": False,
                "verify": True,
                "idempotency_key": "tool-sweep.tracedecay_rename_symbol.rollback",
                "expected_state": inverse_state,
                "format": "json",
            },
            deadline_ms=deadline_for("tracedecay_rename_symbol"),
        ...
        if not _has_true(inverse, "success"):
            raise JourneyError("rename inverse did not report success", calls)
        if source.read_bytes() != baseline:
            raise JourneyError("rename inverse did not restore exact bytes", calls)
        return "rename plan/preview/apply/replay/inverse exact bytes verified"
...
_JOURNEYS: dict[str, Callable[..., PreparedEffectJourney]] = {
    ...
    "tracedecay_rename_symbol": _rename_symbol_journey,
}
