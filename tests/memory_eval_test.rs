...
//! replays a scripted tool-call sequence through the real `tokensave` binary
//! (the same write/curation paths an agent hits over MCP), then asserts on
//! end-state with plain SQL against the fixture's `.tokensave/tokensave.db`.
...
impl Fixture {
    fn db_path(&self) -> PathBuf {
        self.project.path().join(".tokensave/tokensave.db")
    }
    ...
    fn command(&self) -> Command {
        ...
        command
            ...
            .env(
                "TOKENSAVE_GLOBAL_DB",
                self.home.path().join(".tokensave/global.db"),
            )
            .stdin(Stdio::null())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped());
...
fn run_with_timeout(mut command: Command, timeout: Duration) -> Output {
    let mut child = command
        .spawn()
        .unwrap_or_else(|e| panic!("failed to spawn tokensave: {e}"));
