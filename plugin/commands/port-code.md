...
3. After each batch: re-run `tracedecay_port_status`; run the applicable native typecheck; `tracedecay_diagnostics` only reads retained evidence. Cross-branch parity → `tracedecay_branch_diff` / `tracedecay_changelog`.
