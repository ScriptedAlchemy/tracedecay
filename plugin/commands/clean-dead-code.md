...
1. Discover with `tracedecay_dead_code` / `tracedecay_unused_imports` / `tracedecay_redundancy`. Use `tracedecay_complexity` and `tracedecay_coupling` for focused quality checks. Use `tracedecay_unmounted_files` for build-mount evidence, but verify runtime loaders and external consumers before treating a file as dead.
