...
3. Unreachable code → `tracedecay_dead_code` (`include_public: true` for workspace-internal audits), `tracedecay_unused_imports`, and `tracedecay_unmounted_files` (files no indexed build root reaches; verify runtime loaders and external builds before calling them unused).
