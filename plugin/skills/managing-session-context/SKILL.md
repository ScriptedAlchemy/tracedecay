...
2. **Compress → `tracedecay_lcm_compress`** (same identity/pressure args plus
   `focus-topic` and `expected-current-frontier-store-id` as an optimistic
   guard): ask the daemon to compress content already admitted through
   canonical host ingestion. Do not send transcript messages or a
   host-provided summary. **Mutates** the store.
...
4. **Doctor → `tracedecay_lcm_doctor`** (`provider`, `mode`:
   ...
   diagnostics. Doctor reports evidence and recommendations; it never repairs,
   cleans, or garbage-collects storage.
...
`mode: "diagnose"` and review its evidence. Any separate operator remediation
must use its own explicit authority; Doctor never applies it.

## Guardrails

- Retrieval rungs above, `preflight`, `status`, and every Doctor mode are
  read-only (grep/status may touch access counters). `compress` and
  `session_boundary` **mutate** durable session state through the daemon — run
  them only with clear lifecycle or user intent, never speculatively during
  recall.
