...
pub fn measure_impl(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    // Keep the async body in its caller frame. Forwarding it by value through
    // nested measurement futures otherwise multiplies retained state per level.
    let wrapped_body = if !is_async_fn {
        ...
        if enable_result_logging {
            quote! {
                hotpath::functions::measure_async_future_log(#measurement_loc, ::core::pin::pin!(async #block)).await
            }
        } else {
            quote! {
                hotpath::functions::measure_async_future(#measurement_loc, ::core::pin::pin!(async #block)).await
            }
        }
    } else if enable_result_logging {
        quote! {
            hotpath::functions::measure_async_log(#measurement_loc, ::core::pin::pin!(async #block)).await
        }
    } else {
        quote! {
            hotpath::functions::measure_async(#measurement_loc, ::core::pin::pin!(async #block)).await
        }
    };
