...
`benchmark_data/runtime/run.py` composes them (`restore_driver.py` in this
...
3. Each sample: `benchmark_data/runtime/lifecycle.py::OwnedDaemon` cold-started a
...
2. **Indexing did not run under `scripts/with-isolated-tracedecay-daemon.sh`.**
   ...
   `benchmark_data/runtime/run.py`'s own primitives instead (the other sanctioned
   isolation vehicle).
