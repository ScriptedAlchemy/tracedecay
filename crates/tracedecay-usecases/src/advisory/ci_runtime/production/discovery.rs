...
pub async fn discover_production_ci_failure_request_v1(
    ...
    let Some((context, http)) = bounded_ci_discovery_inputs(context, &config.http, deadline) else {
    ...
    let Some(client) =
        GitHubReadOnlyClientV1::new_for_ci(config.target.clone(), config.credential.clone(), http)
    else {
        return ProductionCiFailureDiscoveryOutcomeV1::Unavailable;
    };
...
fn bounded_ci_discovery_inputs(
    context: &RequestContext,
    http: &GitHubHttpReadConfigV1,
    deadline: MonotonicDeadline,
) -> Option<(RequestContext, GitHubHttpReadConfigV1)> {
    let remaining = deadline.instant().checked_duration_since(Instant::now())?;
    let wall_remaining = i64::try_from(remaining.as_micros()).unwrap_or(i64::MAX);
    let cycle_expires_at = now_micros().0.saturating_add(wall_remaining);
    let wall_deadline = Deadline::new(UtcMicros(
        context.deadline().expires_at.0.min(cycle_expires_at),
    ))
    .ok()?;
    Some((
        context.clone().with_deadline(wall_deadline),
        http.bounded_by(remaining)?,
    ))
}
...
pub(super) async fn discover_production_ci_failure_request_scan_v1(
    ...
    let workflow_runs =
        match collect_workflow_runs(context, config, scope, client, cancellation).await {
            Ok(records) => records,
    ...
    let workflow_jobs = match collect_workflow_jobs(
        context,
        config,
        scope,
        client,
    ...
    let check_runs = match collect_check_runs(
        context,
        config,
        scope,
        client,
...
pub(super) async fn collect_workflow_runs(
    ...
    for page_number in 1..=MAX_CI_DISCOVERY_PAGES_V1 {
        ...
        let body = discovery_response_body(
            bounded_ci_discovery_read(
                context,
                client.read_workflow_runs_for_head(
                    context,
...
pub(super) async fn collect_workflow_jobs(
    ...
    for page_number in 1..=MAX_CI_DISCOVERY_PAGES_V1 {
        ...
        let body = discovery_response_body(
            bounded_ci_discovery_read(
                context,
                client.read_workflow_jobs(context, run_id, page_number),
                cancellation,
            )
            .await?,
        )?;
...
pub(super) async fn collect_check_runs(
    ...
    for page_number in 1..=MAX_CI_DISCOVERY_PAGES_V1 {
        ...
        let body = discovery_response_body(
            bounded_ci_discovery_read(
                context,
                client.read_check_runs(context, check_suite_id, page_number),
                cancellation,
            )
            .await?,
        )?;
...
async fn bounded_ci_discovery_read<T>(
    ...
    cancellation: &CancellationToken,
) -> Result<T, ProductionCiFailureDiscoveryOutcomeV1> {
    let remaining_micros = context
        .deadline()
        .expires_at
        .0
    ...
    tokio::select! {
        result = tokio::time::timeout(Duration::from_micros(remaining_micros as u64), read) => {
            result.map_err(|_| ProductionCiFailureDiscoveryOutcomeV1::Unavailable)
        }
        () = cancellation.cancelled() => Err(ProductionCiFailureDiscoveryOutcomeV1::Unavailable),
    }
}
...
async fn bounded_ci_source_authorization(
    ...
    bounded_ci_discovery_read(
        context,
        Box::pin(authorize_ci_source(context, config, scope)),
        cancellation,
    )
    .await?
}
