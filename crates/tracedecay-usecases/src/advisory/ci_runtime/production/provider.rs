    ...
    fn read_record<'a>(
        ...
        Box::pin(async move {
            ...
            let live = match self.live_consensus_record(context, request).await {
                ...
                Err(LiveCiReadFailureV1::Stale) => {
                    return source_failure_result(
                        &self.provider,
                        request,
                        LiveCiReadFailureV1::Stale,
                    );
                }
...
pub(super) async fn authorize_live_ci_source(
    ...
    match source_access.authorize_ci(context, &request.scope).await {
        ...
        CiSourceAccessOutcomeV1::Stale => Err(LiveCiReadFailureV1::Stale),
...
pub(super) fn source_failure_result(
    ...
    match failure {
        ...
        LiveCiReadFailureV1::Stale => CiProviderReadResultV1 {
            provider: provider.clone(),
            run: request.run.clone(),
            state: CiFailureLocalizationStateV1::Stale,
            coverage: CiFailureCoverageV1::Stale,
            source_degradation: Some(CiFailureSourceDegradationV1::Stale),
            failures: 0,
            checks: 0,
            annotations: 0,
            record: None,
