    ...
    async fn read_annotations(
        ...
        if admitted_pages == 0 {
            return Err(admission
                .checkpoint()
                .map_or(LiveCiReadFailureV1::Unavailable, |checkpoint| {
                    LiveCiReadFailureV1::RateLimited(ci_rate_limit(checkpoint.clone()))
                }));
        }
...
mod discovery_tests {
    ...
    fn target(
        _fixture: &crate::advisory::fixtures::AdvisorySourceBackedCompositeFixtureV1,
