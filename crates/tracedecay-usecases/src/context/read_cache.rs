...
/// Error details preserved by the root compatibility facade.
#[derive(Debug)]
pub struct ReadCacheError {
    pub message: String,
...
type Result<T> = std::result::Result<T, ReadCacheError>;
...
pub async fn get(
    ...
    let mut rows = conn
        ...
        .await
        .map_err(|e| ReadCacheError {
            message: format!("read_cache lookup failed: {e}"),
            operation: "read_cache::get",
        })?;

    let row = rows.next().await.map_err(|e| ReadCacheError {
    ...
    let cached_mtime: i64 = row.get(0).map_err(|e| ReadCacheError {
    ...
    let digest: String = row.get(1).map_err(|e| ReadCacheError {
        message: format!("read_cache column 1: {e}"),
        operation: "read_cache::get",
    })?;
    let body: Vec<u8> = row.get(2).map_err(|e| ReadCacheError {
        message: format!("read_cache column 2: {e}"),
        operation: "read_cache::get",
    })?;
    let token_count: i64 = row.get(3).map_err(|e| ReadCacheError {
...
pub async fn put(
    ...
    .map_err(|e| ReadCacheError {
        ...
        operation: "read_cache::put",
    })?;
    Ok(())
}

/// Deletes rows older than the cache age limit. Returns the number of rows
...
mod tests {
    ...
    use super::{args_hash, digest_bytes};

    #[test]
    fn argument_hash_ignores_object_key_order() {
        assert_eq!(
            args_hash(&json!({"mode": "lines", "range": "10-12"})),
            args_hash(&json!({"range": "10-12", "mode": "lines"}))
        );
    }

    #[test]
    fn byte_digest_changes_with_content() {
        assert_ne!(digest_bytes(b"before"), digest_bytes(b"after"));
    }
}
