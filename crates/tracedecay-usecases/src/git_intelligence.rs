...
const HISTORY_CACHE_CAPACITY: usize = 32;
...
fn history_cache() -> &'static Mutex<VecDeque<HistoryCacheEntry>> {
    static CACHE: OnceLock<Mutex<VecDeque<HistoryCacheEntry>>> = OnceLock::new();
    CACHE.get_or_init(|| Mutex::new(VecDeque::with_capacity(HISTORY_CACHE_CAPACITY)))
}
...
impl NativeGitIntelligence {
    ...
    /// managed-test consumers that do not need worktree status entries.
    ...
    pub fn history(
        ...
        let open_repository = gix::open(&self.repo_root).ok();
        let snapshot = match open_repository.as_ref() {
            Some(repository) if repository.object_hash() == gix::hash::Kind::Sha1 => {
                self.repository_snapshot_from_open(repository)?
            }
            _ => self.cli_repository_snapshot()?,
        };
        let mut degradations = snapshot.degradations.clone();
        degradations.extend(snapshot.head_degradations.iter().copied());
        ...
        let (commits, truncated) = if request.path.is_none()
            && !request.follow
            && snapshot.object_format == GitObjectFormatV1::Sha1
            && open_repository.is_some()
        {
            self.pathless_history(
                &snapshot,
                open_repository.as_ref().ok_or_else(|| {
                    GitIntelligenceError::NotARepository(self.repo_root.display().to_string())
                })?,
                max_count,
                request.first_parent,
            )?
        } else {
            self.cli_history(request, max_count)?
        };
        if truncated {
    ...
    fn cli_history(
        ...
        request: &GitHistoryRequest,
        max_count: u32,
    ) -> Result<(Vec<GitCommitMetadataV1>, bool), GitIntelligenceError> {
    ...
    fn pathless_history(
        ...
        let head = snapshot.head.commit().cloned().ok_or_else(|| {
            GitIntelligenceError::MalformedOutput {
                operation: "history",
                detail: "born HEAD did not carry a commit".to_owned(),
            }
        })?;
        let key = HistoryCacheKey {
            repository: self.repository.clone(),
            canonical_common_dir: snapshot.canonical_common_dir.clone(),
            head: head.clone(),
            max_count,
            first_parent,
        };
        if let Ok(mut cache) = history_cache().lock()
            && let Some(position) = cache.iter().position(|entry| entry.key == key)
            && let Some(entry) = cache.remove(position)
        {
            let result = (entry.commits.clone(), entry.truncated);
        ...
        if head_id.to_hex().to_string() != head.as_str() {
            return Err(GitIntelligenceError::MalformedOutput {
                operation: "history",
                detail: "HEAD changed while preparing history".to_owned(),
            });
        }
        let mut walk =
        ...
        let mut commits = Vec::with_capacity(max_count as usize + 1);
        for info in walk.take(max_count as usize + 1) {
            let info = info.map_err(|error| GitIntelligenceError::MalformedOutput {
                operation: "history",
                detail: error.to_string(),
            })?;
            let commit = repo.find_commit(info.id).map_err(|error| {
                GitIntelligenceError::MalformedOutput {
                    operation: "history",
                    detail: error.to_string(),
                }
        ...
        if let Ok(mut cache) = history_cache().lock() {
            if cache.len() == HISTORY_CACHE_CAPACITY {
                cache.pop_front();
            }
            cache.push_back(HistoryCacheEntry {
                key,
...
fn commit_metadata(commit: &gix::Commit<'_>) -> Result<GitCommitMetadataV1, GitIntelligenceError> {
    let malformed = |detail: String| GitIntelligenceError::MalformedOutput {
        operation: "history",
        detail,
    };
    let identity =
        |signature: gix::actor::SignatureRef<'_>| -> Result<GitCommitIdentityV1, GitIntelligenceError> {
            let name = signature
                .name
                .to_str()
            ...
            let email = signature
                ...
                .map_err(|error| malformed(error.to_string()))?
                .to_owned();
            Ok(GitCommitIdentityV1 {
                name,
                email,
                at: UtcMicros(signature.seconds().saturating_mul(1_000_000)),
            })
        };
    let message = commit
        .message_raw()
    ...
    Ok(GitCommitMetadataV1 {
        commit: GitOidV1::new(commit.id().to_hex().to_string())?,
        tree: GitOidV1::new(
            commit
                .tree_id()
                .map_err(|error| malformed(error.to_string()))?
        ...
        parents: commit
            .parent_ids()
            .map(|id| GitOidV1::new(id.to_hex().to_string()))
