...
mod checked_read_models;
pub use checked_read_models::{costs_read_model_checked, observatory_read_model_checked};
...
pub async fn observatory_read_model(
    ...
    match observatory_read_model_checked(db, scope_ref, since_seconds).await {
        Ok(model) => model,
        Err(_) => observatory_unavailable_read_model(
            scope_ref,
            since_seconds,
...
pub async fn costs_read_model(
    ...
    match costs_read_model_checked(db, scope_ref, since_seconds).await {
        Ok(model) => model,
        Err(reason) => costs_unavailable_read_model(scope_ref, since_seconds, &reason),
    }
}
