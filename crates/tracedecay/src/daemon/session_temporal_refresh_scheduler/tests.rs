...
async fn retained_begin_retry_prevents_discovery_queue_growth_and_cursor_advance() {
    ...
    let db = authority.database();
    let store = tracedecay_global_db::session_temporal::GlobalDbSessionTemporalStore::new(db);
...
async fn restart_resumes_each_committed_boundary_without_writer_fallback() {
    ...
    let db = authority.database();
    tracedecay_global_db::session_temporal::GlobalDbSessionTemporalStore::new(db)
...
async fn stale_owner_cannot_persist_after_cancellation() {
    ...
    let store = tracedecay_global_db::session_temporal::GlobalDbSessionTemporalStore::new(authority.database());
    store
...
async fn project_retirement_cancels_and_awaits_an_inflight_projector() {
    ...
    tracedecay_global_db::session_temporal::GlobalDbSessionTemporalStore::new(authority.database())
