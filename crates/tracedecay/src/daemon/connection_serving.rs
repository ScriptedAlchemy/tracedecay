...
#[cfg(unix)]
struct AdmittedBrokerClient {
    transport: BrokerStreamTransport,
    engine: DaemonEngine,
    handshake: DaemonHandshake,
    first_request: AuthenticatedFirstRequest,
    setup_activity: DaemonActivity,
    _per_client_permit: Option<DaemonPerClientPermit>,
}
...
fn serve_broker_socket_client_inner(
    ...
    auth_token: Option<String>,
    admission_class: DaemonClientAdmissionClass,
) -> BrokerConnectionPhaseFutureV1<'static, ()> {
    // Keep lifecycle stages erased so profiling cannot inline their combined
    // state onto a Tokio worker stack before the first request is served.
    boxed_broker_connection_phase(async move {
        let Some(client) = boxed_broker_connection_phase(authenticate_broker_client(
            stream,
            engine,
            auth_token,
            admission_class,
        ...
        let Some((client, initialize_route)) =
            boxed_broker_connection_phase(prepare_broker_request(client)).await?
        else {
            return Ok(());
        };
        boxed_broker_connection_phase(dispatch_broker_request(client, initialize_route)).await
    })
}

#[cfg(unix)]
async fn authenticate_broker_client(
    stream: BrokerStream,
    engine: DaemonEngine,
    auth_token: Option<String>,
    admission_class: DaemonClientAdmissionClass,
) -> Result<Option<AdmittedBrokerClient>> {
...
#[cfg(unix)]
async fn prepare_broker_request(
    client: AdmittedBrokerClient,
) -> Result<Option<(AdmittedBrokerClient, Option<InitializeRouteMetadata>)>> {
    let AdmittedBrokerClient {
        mut transport,
        engine,
        mut handshake,
        first_request,
    ...
    let git_watcher_health = if doctor_runtime_request(first_request.parsed()).is_some() {
        Some(Box::pin(engine.git_watcher_health(handshake.project_path.as_deref())).await)
    } else {
        None
    };
...
async fn dispatch_broker_request(
    ...
    if let Some(invocation) = parse_daemon_invocation_request(first_request.raw()) {
        return boxed_broker_connection_phase(async move {
            serve_retained_broker_invocations(&mut transport, &engine, &handshake, invocation).await
        })
        .await;
    }

    if boxed_broker_connection_phase(serve_broker_bootstrap(
        &mut transport,
        &engine,
        &handshake,
        &first_request,
    ...
    boxed_broker_connection_phase(serve_broker_project_request(
        AdmittedBrokerClient {
            transport,
            engine,
            handshake,
...
#[cfg(unix)]
async fn serve_retained_broker_invocations(
    ...
    let result = boxed_broker_connection_phase(async {
        loop {
            ...
            let write_result =
                write_daemon_invocation_response(transport, &response).await;
                ...
                match ack_line {
                    Some(line) => {
                        ...
                        if let Some(ack) = ack.filter(|ack| {
                            ...
                            write_daemon_delivery_ack_response(transport, &ack_response)
                                .await?;
...
#[cfg(unix)]
async fn serve_broker_bootstrap(
    transport: &mut BrokerStreamTransport,
    engine: &DaemonEngine,
    handshake: &DaemonHandshake,
    first_request: &AuthenticatedFirstRequest,
    ...
    if let Some(request) = first_request.parsed() {
        ...
        if !initialized_project_server_ready
            && let Some(mut response) =
                daemon_bootstrap_response(request, initialize_route, project_node_count)
        {
            ...
            if let Some(key) = engine
                .claim_catalog_refresh(handshake, first_request.parsed(), catalog_is_provisional)
                .await
                && let Err(error) = write_tool_list_changed_notification(transport).await
            {
...
#[cfg(unix)]
async fn serve_broker_project_request(
    client: AdmittedBrokerClient,
    initialize_route: Option<InitializeRouteMetadata>,
) -> Result<()> {
    let AdmittedBrokerClient {
