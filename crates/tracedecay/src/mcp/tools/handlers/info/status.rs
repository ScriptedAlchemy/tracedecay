...
pub(crate) async fn handle_status(
    ...
    let retrieval_serving =
        attach_code_index_freshness(cg, &mut output, code_index_freshness_reader).await;
    ...
    if include_session_ingest {
        attach_status_session_ingest(cg, &mut output, project_session_db).await;
    }
