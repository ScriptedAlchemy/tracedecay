...
async fn trace_decay_init_defaults_to_profile_shard_without_repo_marker() {
    ...
    assert!(!TraceDecay::is_initialized_in_profile(&profile, &project));

    let cg = init_with_maintenance(&profile, &project).await.unwrap();
    ...
    assert_eq!(profile.discover_project_root(&child), Some(project.clone()));
    assert!(
...
async fn trace_decay_init_registers_default_profile_shard_globally() {
    ...
    drop(db);
    open_with_maintenance(&profile, &project).await.unwrap();
    assert!(
