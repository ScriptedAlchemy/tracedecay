...
const MINIMUM_RECEIPTS_BEFORE_CANCELLATION: usize = 641;
...
fn successor_receipt_count(home: &Path, project: &Path, predecessor_generation_id: &str) -> usize {
    let predecessor_directory = hex::encode(Sha256::digest(predecessor_generation_id.as_bytes()));
    let receipts = code_index_store_scope(home, project)
        .join("code-generation-build-receipts-v1")
...
async fn wait_for_successor_receipts(
    home: &Path,
    project: &Path,
    ...
    tokio::time::timeout(RECEIPT_TIMEOUT, async {
        loop {
            let receipt_count = successor_receipt_count(home, project, predecessor_generation_id);
            if receipt_count >= MINIMUM_RECEIPTS_BEFORE_CANCELLATION {
    ...
    .await
    .expect("successor produced no durable file receipt before cancellation")
}
...
async fn mounted_incremental_lifecycle_preserves_only_complete_compatible_generations() {
    ...
    let observed_receipts =
        wait_for_successor_receipts(environment.home(), &project, &overflowed.generation_id).await;
    assert!(
        observed_receipts >= MINIMUM_RECEIPTS_BEFORE_CANCELLATION,
        "receipt wait returned before the mid-build interruption boundary"
    );
    ...
    let retained_receipts =
        successor_receipt_count(environment.home(), &project, &overflowed.generation_id);
    assert!(
        retained_receipts >= observed_receipts && retained_receipts <= CANCELLATION_BATCH_FILES,
        "graceful cancellation must retain the observed successor receipts: \
         observed={observed_receipts}, retained={retained_receipts}"
    );
    ...
    println!(
        "successor receipt recovery: retained_receipts={retained_receipts} \
         reextracted_files={reextracted_files}"
    );
    assert_eq!(
        reextracted_files,
        CANCELLATION_BATCH_FILES - retained_receipts,
        "restart must re-extract only successor files without durable receipts"
    );
    assert_eq!(
