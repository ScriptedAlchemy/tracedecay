...
pub async fn grep(
    ...
    let git_scope_session_ids = if request.git_filter.is_empty() {
        ...
        if session_ids.len() > crate::MAX_SESSIONS_FOR_LIMIT {
...
fn push_grep_relationship_scope_filter(
    ...
    let is_subagent = match scope {
        crate::SessionSearchScope::All => return,
        crate::SessionSearchScope::ParentsOnly => 0,
        crate::SessionSearchScope::SubagentsOnly => 1,
    };
    filters.push(format!(
