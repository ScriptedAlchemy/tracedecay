...
mod tests {
    ...
    #[test]
    fn memory_policy_uses_accepted_or_rejected_next_actions() {
        let policy = artifact_policy(AgentTaskKind::MemoryCurator);

        assert_eq!(
            policy.next_actions(1),
            vec![
                "review accepted memory curation ops",
                "apply through dashboard or CLI if approved",
            ]
    ...
    #[test]
    fn every_task_has_one_handoff_test_and_eval_replay_command() {
        for task in [
