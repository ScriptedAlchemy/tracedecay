...
async fn schema_v2_commit_rows_migrate_to_observed_without_becoming_producers() {
    ...
    ensure_git_correlation_schema(&conn)
        .await
        .expect("exact-final validation must be repeat-safe");
}

#[tokio::test]
async fn partial_git_correlation_schema_is_rejected_without_rewrite() {
    let conn = GitCorrelationTestDb::uninitialized();
    conn.execute_batch(
        "CREATE TABLE git_correlation_meta (
            key TEXT PRIMARY KEY,
         ...
         INSERT INTO git_correlation_meta(key, value)
            VALUES ('schema_identity', 99);",
    )
    .await
    .unwrap();
