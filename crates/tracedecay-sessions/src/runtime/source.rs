...
/// Concrete transcript-persistence operations needed by the provider-neutral
/// ingest loop. Root code implements this over `GlobalDb`; parsing and ingest
/// control flow stay independent of the root crate.
pub trait TranscriptIngestStore {
    ...
    fn upsert_transcript(
        ...
        commit_records: &[crate::runtime::git_correlation::CommitSessionRecord],
        span_observations: &[crate::runtime::git_correlation::SpanObservation],
        path: &str,
        cursor: StoredCursor,
    ...
    S: TranscriptIngestStore,
{
    ...
    let prev = db.load_cursor(&path_str).await;
    ...
    if parsed.messages.is_empty() {
        ...
        db.advance_cursor(&path_str, parsed.new_cursor).await;
        return TranscriptIngestStats::default();
    }
    ...
    let existing = db.existing_session(provider, &draft.session_id).await;
