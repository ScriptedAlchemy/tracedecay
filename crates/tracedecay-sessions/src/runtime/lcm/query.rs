...
fn hit_is_inventory(hit: &LcmGrepHit) -> bool {
    ...
    crate::compatibility::is_inventory_text(&hit.snippet)
}
...
fn push_raw_grep_filters(
    ...
    if let Some(predicate) =
        crate::compatibility::message_type_predicate_sql("r", false, retrieval_filters.message_type)
    {
...
fn dedupe_related_raw_hits(candidates: Vec<RawGrepCandidate>) -> Vec<LcmGrepHit> {
    crate::compatibility::dedupe_related_message_copies(candidates, |candidate| {
        crate::compatibility::RelatedMessageCopyIdentity {
