...
/// #1355: import alias (`WalkBuilder as Builder`) must still bind through the
/// imported type name when a typed local calls a blocklisted method.
#[test]
fn cross_file_aliased_builder_method_calls_bind_through_typed_locals() {
    let sources = [
        (
            "file.builder.alias.caller",
            "crates/app/src/lib.rs",
            "rust",
            "use builder::WalkBuilder as Builder;\npub fn make_walk() -> u32 {\n    let mut builder = Builder::new();\n    builder.build()\n}\n",
        ),
        (
            "file.builder.alias.def",
            "crates/builder/src/lib.rs",
            "rust",
            "pub struct WalkBuilder;\nimpl WalkBuilder {\n    pub fn new() -> Self { WalkBuilder }\n    pub fn build(&self) -> u32 { 1 }\n}\n",
        ),
    ];
    let mut request = request("file.builder.alias.seed", 1_100_000);
    request.snapshot.files.clear();
    request.snapshot.sanitization_receipts.clear();
    request.captured_files.clear();
    let mut identity = Sha256::new();
    ...
    assert!(
        incoming_callers.contains(&make_walk),
        "aliased Builder local must call WalkBuilder::build: edges={:?} symbols={:?}",
        generation.edges(),
        generation
            .symbols()
            .symbols
...
const PARTITIONED_FORMAT_STATE_DIGEST: &str =
    "sha256:0723449848a473743b447019455f9dc56561d2432b16bd4e4a12c8b4035270ee";
const PARTITIONED_FORMAT_SEGMENTS: &[(&str, u64)] = &[
    ...
    (
        "sha256:2aae415a560831a1614356fcdc9ee339862a9531b0d40f451c8838f5bdd2697f",
        10_133,
    ),
];
