...
fn sealed_bytes_at_width(width: usize, file_count: usize) -> Vec<u8> {
    tracedecay_code_index::parallelism::force_indexing_workers_for_test(width);
    let store = SharedPublicationStore::default();
    let mut owner = CodeIndexProductionOwnerV1::new(config(), store, ApplyingProjectionSink)
        .expect("production owner");
    let generation = owner
...
fn parallel_and_sequential_generations_are_byte_identical() {
    ...
    let parallel = sealed_bytes_at_width(
        tracedecay_code_index::parallelism::indexing_worker_target(64),
        FILES,
    );
