use tracedecay_code_index::extraction::ScalaExtractor;
use tracedecay_code_index::types::{EdgeKind, NodeKind};

fn extract(source: &str) -> tracedecay_code_index::types::ExtractionResult {
...
fn test_scala_visibility_private() {
    ...
    assert_eq!(
        ...
        tracedecay_code_index::types::Visibility::Private
    );
}
...
fn test_scala_visibility_default_is_public() {
    ...
    let methods: Vec<_> = result
        ...
        .collect();
    assert_eq!(methods.len(), 1);
    assert_eq!(methods[0].visibility, tracedecay_code_index::types::Visibility::Pub);
}
