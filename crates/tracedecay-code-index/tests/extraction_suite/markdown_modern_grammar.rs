...
fn timed_extract(source: String, timeout: Duration) -> Option<(f64, usize, usize)> {
    ...
    std::thread::spawn(move || {
        let t0 = Instant::now();
        let res = tracedecay_code_index::extraction::MarkdownExtractor::extract_markdown("t.md", &source);
...
fn extracts_headings_and_links() {
    ...
    let res = tracedecay_code_index::extraction::MarkdownExtractor::extract_markdown("doc.md", src);
    let modules: Vec<&str> = res
        .nodes
        .iter()
        .filter(|n| matches!(n.kind, tracedecay_code_index::types::NodeKind::Module))
        .map(|n| n.name.as_str())
        .collect();
    ...
    let uses_count = res
        .edges
        .iter()
        .filter(|e| matches!(e.kind, tracedecay_code_index::types::EdgeKind::Uses))
        .count();
