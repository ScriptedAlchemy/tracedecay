...
pub enum CodeIndexPublicationStoreErrorV1 {
    ...
    CompareAndSwap,
    #[error(
        "the canonical generation exceeds resident-memory admission: used={used_bytes} requested={requested_bytes} limit={limit_bytes}"
    )]
    ResidentMemoryUnavailable {
        used_bytes: u64,
        requested_bytes: u64,
        limit_bytes: u64,
    },
...
impl CodeIndexPublishedGenerationV1 {
    ...
    pub fn sealed_resident_memory_estimate(
        ...
        for _ in 0..2 {
            resident_add(
                &mut reservation_bytes,
                resident_allocation_upper_bound::<tracedecay_domain::CodeSearchChunkV1>(
                    chunk_count,
                )?,
            )?;
            resident_add(
                &mut reservation_bytes,
                resident_allocation_upper_bound::<crate::lineage::LineageSymbolRecordV1>(
                    symbol_count,
                )?,
            )?;
            resident_add(
