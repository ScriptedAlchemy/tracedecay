...
/// Encode one completed physical artifact so an interrupted successor can
/// resume without exposing that artifact as a published generation.
...
/// Restore one completed physical artifact after its exact reuse identity was
/// selected from the current generation inputs.
