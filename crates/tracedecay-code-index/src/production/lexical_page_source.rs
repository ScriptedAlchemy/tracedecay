...
/// Default source-side reservation for layout, one decoded file, and one page.
pub const VERIFIED_SEALED_LEXICAL_SOURCE_MEMORY_BUDGET_BYTES_V1: usize = 128 * 1024 * 1024;
const VERIFIED_SEALED_LEXICAL_LAYOUT_RESERVATION_CEILING_BYTES_V1: usize = 16 * 1024 * 1024;
const VERIFIED_SEALED_LEXICAL_DECODE_AMPLIFICATION_V1: usize = 32;
...
impl<R: Read + Seek> VerifiedSealedLexicalPageSourceV1<R> {
    pub fn open(
        ...
        Self::open_with_memory_budget(
            reader,
            admitted_len,
            expected_state_digest,
            maximum_page_chunks,
            maximum_page_bytes,
            VERIFIED_SEALED_LEXICAL_SOURCE_MEMORY_BUDGET_BYTES_V1,
            control,
        )
    }

    /// Open with an explicit source-memory reservation. The reservation is
    /// validated before layout allocation, and the source refuses a file
    /// before reading or decoding it when the conservative decoded-file plus
    /// retained-page peak would exceed this bound.
    pub fn open_with_memory_budget(
        ...
        if memory_budget_bytes == 0 {
            return Err(CodeIndexProductionErrorV1::Contract(
                "sealed lexical source memory budget must be non-zero".to_owned(),
            ));
        }
        let layout_reservation_bytes = memory_budget_bytes
            .saturating_div(8)
            .min(VERIFIED_SEALED_LEXICAL_LAYOUT_RESERVATION_CEILING_BYTES_V1);
        let maximum_file_ranges = layout_reservation_bytes
            .checked_div(std::mem::size_of::<Range<u64>>())
            .unwrap_or(0);
        if maximum_file_ranges == 0 {
            return Err(CodeIndexProductionErrorV1::Contract(
                "sealed lexical source budget cannot reserve one file range".to_owned(),
            ));
        }
        let layout = scan_layout(&mut reader, admitted_len, maximum_file_ranges, control)?;
        ...
        let layout_reserved_bytes = layout
            .file_ranges
            .capacity()
            .saturating_mul(std::mem::size_of::<Range<u64>>());
        let staging_window_bytes = layout_reserved_bytes
            .saturating_add(decode_reservation_bytes(layout.maximum_file_bytes))
            .saturating_add(page_staging_upper_bound(
                maximum_page_chunks,
                maximum_page_bytes,
            ));
        if staging_window_bytes > memory_budget_bytes {
            return Err(CodeIndexProductionErrorV1::Contract(format!(
                "sealed lexical source needs a {staging_window_bytes}-byte layout/decode/page reservation, exceeding its {memory_budget_bytes}-byte memory budget"
            )));
        }
        Ok(Self {
            reader,
    ...
    pub fn staging_window_bytes(&self) -> usize {
        self.layout_reserved_bytes
            .saturating_add(decode_reservation_bytes(self.maximum_file_bytes))
            .saturating_add(page_staging_upper_bound(
    ...
    fn stage_next_page(
            ...
            .is_some_and(|ordinal| ordinal < self.file_ranges.len())
        {
            ...
            let admitted = self.read_admitted_file(file_ordinal, control)?;
            let AdmittedSealedLexicalFileV1 {
                chunks: admitted_chunks,
                imports: admitted_imports,
            } = admitted;
            ...
            if chunk_ordinal > admitted_chunks.len() {
                return Err(CodeIndexProductionErrorV1::Contract(
                    "sealed lexical cursor exceeds its file chunk count".to_owned(),
                ));
            }
            for (ordinal, admitted_chunk) in
                admitted_chunks.into_iter().enumerate().skip(chunk_ordinal)
            {
                checkpoint(control)?;
                let serialized = serde_json::to_vec(admitted_chunk.chunk()).map_err(|error| {
                    CodeIndexProductionErrorV1::Contract(format!(
                        "sealed lexical chunk serialization failed: {error}"
                    ))
                })?;
                ...
                chunks.push(admitted_chunk);
                chunk_ordinal = ordinal.saturating_add(1);
            ...
            if import_ordinal > admitted_imports.len() {
                return Err(CodeIndexProductionErrorV1::Contract(
                    "sealed lexical cursor exceeds its file import count".to_owned(),
                ));
            }
            for (ordinal, evidence) in admitted_imports
                .into_iter()
                .enumerate()
                .skip(import_ordinal)
            {
                ...
                import_ordinal = ordinal.saturating_add(1);
    ...
    fn read_admitted_file(
        ...
        let required = self
            .layout_reserved_bytes
            .saturating_add(decode_reservation_bytes(byte_len))
            .saturating_add(page_staging_upper_bound(
                self.maximum_page_chunks,
                self.maximum_page_bytes,
            ));
        if required > self.memory_budget_bytes {
            return Err(CodeIndexProductionErrorV1::Contract(format!(
                "sealed lexical file needs a {required}-byte decode/page reservation, exceeding its {}-byte source budget",
...
fn decode_reservation_bytes(serialized_file_bytes: usize) -> usize {
    serialized_file_bytes.saturating_add(
        serialized_file_bytes.saturating_mul(VERIFIED_SEALED_LEXICAL_DECODE_AMPLIFICATION_V1),
    )
}

fn page_staging_upper_bound(maximum_page_chunks: usize, maximum_page_bytes: usize) -> usize {
    // Every canonical admitted chunk serializes substantially more than this
    // fixed structural floor (typed identity, anchor, revisions and text).
    let byte_bounded_chunks = maximum_page_bytes.saturating_div(64).saturating_add(1);
    let maximum_page_chunks = maximum_page_chunks.min(byte_bounded_chunks);
    maximum_page_bytes.saturating_mul(4).saturating_add(
        maximum_page_chunks
            .saturating_mul(std::mem::size_of::<ExtractionAdmittedCodeSearchChunkV1>()),
    )
}
...
fn scan_layout<R: Read + Seek>(
    ...
    let mut scanner = LayoutScanner::with_reserved_file_ranges(maximum_file_ranges)?;
...
impl LayoutScanner {
    fn with_reserved_file_ranges(
        maximum_file_ranges: usize,
    ) -> Result<Self, CodeIndexProductionErrorV1> {
        let mut file_ranges = Vec::new();
        file_ranges
            .try_reserve_exact(maximum_file_ranges)
            .map_err(|error| {
                CodeIndexProductionErrorV1::Contract(format!(
                    "sealed lexical layout reservation failed: {error}"
                ))
            })?;
        Ok(Self {
            brace_depth: 0,
    ...
    fn observe(&mut self, byte: u8, offset: u64) -> Result<(), CodeIndexProductionErrorV1> {
        ...
        match byte {
            ...
            b'}' => {
                ...
                {
                    if self.file_ranges.len() == self.maximum_file_ranges {
                        return Err(CodeIndexProductionErrorV1::Contract(format!(
                            "sealed lexical layout exceeds its {}-file reserved bound",
                            self.maximum_file_ranges
                        )));
                    }
                    let byte_len = usize::try_from(offset.saturating_add(1).saturating_sub(start))
                        .map_err(|_| {
                            CodeIndexProductionErrorV1::Contract(
                                "sealed lexical file range exceeds the platform limit".to_owned(),
                            )
