...
mod lexical_page_source_tests {
    ...
    fn partitioned_reopen_reports_encoded_byte_progress_and_bounds_prefetch() {
        ...
        source.rewind().expect("rewind before batch replay");
        loop {
            let bounds = VerifiedSealedLexicalPageBatchBoundsV1::new(2, 4 << 20)
                .expect("batch replay bounds");
            let read = source
                .next_page_batch_if(&ActiveControl, bounds, |pages| {
                    Ok::<_, ()>(NonZeroUsize::new(pages.len()).expect("non-empty page batch"))
                })
                .expect("batch replay reads")
                .expect("batch replay accepts");
            if let VerifiedSealedLexicalPageBatchReadV1::Complete(receipt) = read {
                receipt
                    .verify_completion(Some(source.cursor()))
                    .expect("batch completion cursor verifies");
                assert_eq!(source.completed_files(), source.total_files());
                assert_eq!(
                    source
                        .completed_lexical_bytes()
                        .expect("batch completed bytes"),
                    total_segment_bytes
                );
                break;
            }
