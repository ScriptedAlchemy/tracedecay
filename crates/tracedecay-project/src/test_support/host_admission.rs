...
#[doc(hidden)]
pub use verified_graph_test_support::await_bound_graph_runtime;
...
impl HostAdmissionTestRuntimeV1 {
    ...
    async fn open(profile_root: PathBuf, project: Option<(PathBuf, ProjectId)>) -> Result<Self> {
        ...
        let session_registry = {
            ...
            match registries.get_live(&profile_key) {
                ...
                None => {
                    // The fixture stands in for the composition root: it
                    // registers the runtime ports with the fixture daemon
                    // client, exactly as it registers the fixture product
                    // runtime above.
    ...
    /// The registered session database lease for `scope`, borrowed, or `None`
    /// for a profile-only runtime asked for its project mount.
    pub fn registered_database_lease(
        &self,
        scope: HostAdmissionScope,
    ) -> Option<&RegisteredGlobalDbLeaseV1> {
        match scope {
            HostAdmissionScope::Project => self.project_registered.as_ref(),
            HostAdmissionScope::Profile => Some(&self.profile_registered),
        }
    }

    /// The registered profile database lease this runtime mounted.
    pub fn profile_database_lease(&self) -> &RegisteredGlobalDbLeaseV1 {
    ...
    pub fn host_admission_broker_for_test(
        &self,
        scope: HostAdmissionScope,
