...
impl Client {
        ...
        Operation::Result: DeserializeOwned,
    {
        ...
        self.execute_http::<Operation>(route, request)
    }
    ...
    where
        ...
        Operation::Result: DeserializeOwned,
    {
        let response = self.request_route(route, &request)?;
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedTransport {
                ...
                formatter,
                "{operation_id} cannot be invoked through the HTTP client; use the {transport} transport"
            ),
