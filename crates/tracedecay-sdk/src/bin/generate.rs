...
function validateCanonicalSchema(value: unknown, schemaValue: unknown, root: CanonicalJsonSchema, path: string, resolving: Set<string>): void {
  ...
  if (typeof value === "number") { if (typeof schema.minimum === "number" && value < schema.minimum) throw new TypeError(`${path} is below minimum`); if (typeof schema.maximum === "number" && value > schema.maximum) throw new TypeError(`${path} exceeds maximum`); }
  if (Array.isArray(value)) { if (typeof schema.minItems === "number" && value.length < schema.minItems) throw new TypeError(`${path} has too few items`); if (typeof schema.maxItems === "number" && value.length > schema.maxItems) throw new TypeError(`${path} has too many items`); if (schema.uniqueItems === true && value.some((item, index) => value.slice(0, index).some((previous) => sameJson(previous, item)))) throw new TypeError(`${path} contains duplicate items`); if (Array.isArray(schema.prefixItems)) schema.prefixItems.forEach((item, index) => { if (index < value.length) validateCanonicalSchema(value[index], item, root, `${path}[${index}]`, new Set(resolving)); }); if (schema.items !== undefined && !Array.isArray(schema.prefixItems)) value.forEach((item, index) => validateCanonicalSchema(item, schema.items, root, `${path}[${index}]`, new Set(resolving))); }
...
mod tests {
    ...
    #[test]
    fn work_registry_exports_available_and_typed_unavailable_operations_separately() {
        ...
        assert!(!generated.contains(
            "operation: \"work_attempt_acquire_lease\", operationId: \"operation.work.attempt_acquire_lease\", route:"
        ));
    }
}
