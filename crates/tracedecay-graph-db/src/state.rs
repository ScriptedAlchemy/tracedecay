...
pub(crate) fn load_relation_by_locator(
    ...
    let (source_namespace, source_identity) = entity_endpoint_identity(database, edge.src)?;
    let (target_namespace, target_identity) = entity_endpoint_identity(database, edge.dst)?;
