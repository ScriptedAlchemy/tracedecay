...
impl ReaderQueryExecutor for NoReads {
    ...
    {
        unreachable!("benchmark publication authority uses exact SQL only")
    }
}
...
pub(super) fn operation_control(sequence: usize) -> (RuntimeRequestControlV1, BenchmarkProbe) {
    ...
    let deadline = RuntimeDeadlineV1 {
        deadline_id: RuntimeDeadlineIdV1::new(format!("benchmark-deadline:{sequence}"))
...
impl PersistentBenchmarkGraph {
    pub fn new() -> Self {
        ...
        Self {
            ...
            binding,
            graph_path: root.path().join("publication.grafeo"),
            _writer: writer,
            _readers: readers,
            latest_head: None,
    ...
    pub fn publish(
        ...
        lifecycle: Option<(&AtomicBool, &AtomicBool)>,
    ) -> VerifiedGraphSnapshot {
        let completion = lifecycle.map(|(_, completed)| CompletionSignal(completed));
        if let Some((started, _)) = lifecycle {
        ...
        let replay = manifest
            .relational_metadata_replay(
                ...
                GraphIdempotencyKey::new(format!("benchmark-publication:{}", self.sequence))
