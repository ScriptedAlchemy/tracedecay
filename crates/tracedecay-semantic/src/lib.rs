...
mod document_composition_tests {
    ...
    fn encode_with(
        ...
        let mut encoder = RuntimeChunkVectorEncoderV1::new(
            runtime,
            Arc::new(SemanticRuntimeScheduleCancellationV1::new(chunks.len() as u64)),
            authority.embedding_execution_plan(),
            documents(),
        );
        encoder
    ...
    fn header_composition_changes_the_tensor_input_and_only_that() {
        let body = "pub fn get(&self, key: u32) -> Option<u32> {\n    self.map.get(&key).copied()\n}";
        ...
        assert_ne!(header[0], sanitized[0], "the header must change the symbol chunk's input");
        assert_eq!(
