...
mod tests {
    ...
    fn repeated_identical_store_reuses_the_record_without_writing() {
        let root = tempfile::tempdir().unwrap();
        let first = store_response_handle_in_root(root.path(), "same search", 10).unwrap();
        ...
        let second = store_response_handle_in_root(root.path(), "same search", 20);
        ...
        let renewed = store_response_handle_in_root(
            root.path(),
            "same search",
