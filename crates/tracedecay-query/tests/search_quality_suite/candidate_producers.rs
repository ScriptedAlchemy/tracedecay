...
use tracedecay_code_index::production::{
    ...
    CodeIndexProductionConfigV1, CodeIndexProductionOwnerV1, CodeIndexPublicationStoreErrorV1,
...
use tracedecay_query::retrieval::lexical::{
    ...
    CodeLexicalArtifactErrorV1, CodeLexicalArtifactMountMetadataV1, CodeLexicalArtifactReaderV1,
...
#[test]
fn disk_artifact_phrase_and_fuzzy_results_match_projection_after_reopen() {
    ...
    let one_shot = CodeLexicalProjectionAdapterV1::new_admitted(metadata.clone(), chunks)
        .expect("one-shot lexical projection");
    let directory = tempfile::tempdir().expect("artifact tempdir");
    let artifact_path = directory.path().join("phrase-fuzzy-parity.sqlite");
    let control = ArtifactControl { cancelled: false };
    let verified = {
        let mut builder = CodeLexicalArtifactBuilderV1::create(&artifact_path, metadata.clone())
            .expect("create artifact");
        ...
        builder
            .rebuild_and_finalize(&mut source, &control)
            .expect("finalize artifact")
    };

    let missing_path = directory.path().join("must-not-be-opened.sqlite");
    assert!(matches!(
        CodeLexicalArtifactReaderV1::open(&missing_path, &verified, 1),
        Err(CodeLexicalArtifactErrorV1::Contract(_))
    ));
    assert!(
        !missing_path.exists(),
        "reader reservation denial must precede every artifact path touch"
    );

    let artifact_reader = CodeLexicalArtifactReaderV1::open(
        &artifact_path,
        &verified,
        CODE_LEXICAL_ARTIFACT_QUERY_CACHE_BUDGET_BYTES_V1,
    )
    .expect("open artifact");
    let mut phrase = lexical_request(r#""return value""#, &[], &[], &["return value"], 0, 8);
    phrase.generation = metadata.generation.clone();
    let expected_phrase = LexicalLane::new(one_shot.clone())
        .retrieve_lexical(&phrase)
        .expect("one-shot phrase query");
    let artifact_phrase = LexicalLane::new(artifact_reader.clone())
        .retrieve_lexical(&phrase)
        .expect("artifact phrase query");
    assert_eq!(artifact_phrase, expected_phrase);

    let mut fuzzy = lexical_request("rendr", &["rendr"], &[], &[], 8, 8);
    fuzzy.generation = metadata.generation.clone();
    let expected_fuzzy = LexicalLane::new(one_shot.clone())
        .retrieve_lexical(&fuzzy)
        .expect("one-shot fuzzy query");
    let artifact_fuzzy = LexicalLane::new(artifact_reader.clone())
        .retrieve_lexical(&fuzzy)
        .expect("artifact fuzzy query");
    assert_eq!(artifact_fuzzy, expected_fuzzy);

    let mut disabled_fuzzy = lexical_request("rendr", &["rendr"], &[], &[], 0, 8);
    disabled_fuzzy.generation = metadata.generation;
    let expected_disabled = LexicalLane::new(one_shot)
        .retrieve_lexical(&disabled_fuzzy)
        .expect("one-shot disabled fuzzy query");
    let artifact_disabled = LexicalLane::new(artifact_reader)
        .retrieve_lexical(&disabled_fuzzy)
        .expect("artifact disabled fuzzy query");
    assert_eq!(artifact_disabled, expected_disabled);

    let reopened = CodeLexicalArtifactReaderV1::open(
        &artifact_path,
        &verified,
        CODE_LEXICAL_ARTIFACT_QUERY_CACHE_BUDGET_BYTES_V1,
    )
    ...
    assert_eq!(
        LexicalLane::new(reopened)
            .retrieve_lexical(&phrase)
            .expect("reopened phrase query"),
        expected_phrase,
    );

    let connection = rusqlite::Connection::open(&artifact_path).expect("open metadata mutation");
    let metadata_bytes: Vec<u8> = connection
        .query_row(
            "SELECT metadata FROM artifact_state WHERE singleton = 1",
            [],
            |row| row.get(0),
        )
        .expect("mount metadata");
    let mut mount_metadata: serde_json::Value =
        serde_json::from_slice(&metadata_bytes).expect("decode mount metadata");
    mount_metadata["exact_score_domain"] =
        serde_json::Value::String("score.tampered.v1".to_owned());
    connection
        .execute(
            "UPDATE artifact_state SET metadata = ?1 WHERE singleton = 1",
            [serde_json::to_vec(&mount_metadata).expect("encode tampered metadata")],
        )
        .expect("tamper mount metadata");
    drop(connection);
    assert!(matches!(
...
fn disk_artifact_rebuild_restores_canonical_artifact_state_before_return() {
    ...
    assert_eq!(
        reader.metadata(),
        &CodeLexicalArtifactMountMetadataV1::from(&fixture.metadata)
    );
}
...
#[test]
fn disk_artifact_budget_refusal_precedes_progress_and_resumes_the_same_source() {
    let (fixture, pages, source_receipt) = real_verified_pages_with_maximum_page_chunks(1);
    let metadata = fixture.metadata.clone();
    let control = ArtifactControl { cancelled: false };
    let directory = tempfile::tempdir().expect("artifact tempdir");

    // Read the deterministic allocation-free reservation charges with a
    // default builder. Preflight must not clone/project/tokenize/serialize in
    // order to decide whether this budget fits.
    ...
    let mut refused = CodeLexicalArtifactBuilderV1::create_with_memory_budget(
        directory.path().join("refused.sqlite"),
        metadata.clone(),
    ...
    // On the rebuild path the source's one-decoded-file staging window is
    // charged before any page is staged: a budget the window itself
    // exhausts refuses up front and never touches the source.
    let mut source = fixture.open_source(1);
    let window = source.staging_window_bytes();
    assert!(window > 0, "a real sealed source stages a nonzero window");
    let mut window_refused = CodeLexicalArtifactBuilderV1::create_with_memory_budget(
        directory.path().join("window-refused.sqlite"),
        metadata.clone(),
        fixed + window,
    )
    .expect("create window-exhausted builder");
    assert!(matches!(
        window_refused.rebuild_and_finalize(&mut source, &control),
        Err(CodeLexicalArtifactErrorV1::Contract(_))
    ));
    assert_eq!(
        source.cursor().next_page_ordinal(),
        0,
        "an exhausted staging window must refuse before the source stages anything"
    );

    // A per-page ledger refusal runs inside `next_page_if` and must leave
    // the sealed source's cursor and hash authorities unchanged.
    let mut page_refused = CodeLexicalArtifactBuilderV1::create_with_memory_budget(
        directory.path().join("page-refused.sqlite"),
        metadata.clone(),
        fixed + window + first_page_charge - 1,
    )
    .expect("create one-byte-under rebuild builder");
    assert!(matches!(
        page_refused.rebuild_and_finalize(&mut source, &control),
        Err(CodeLexicalArtifactErrorV1::Contract(_))
    ));
    assert_eq!(
        source.cursor().next_page_ordinal(),
        0,
        "a refused page must not advance the sealed lexical source"
    );
    assert_eq!(
        page_refused
            .progress()
            .expect("post-refusal progress")
            .next_page_ordinal,
        0
    );
    ...
    // The refused source instance RESUMES: a builder whose budget admits
    // the staging window and every page seals the very same source, and
    // every row advances once.
    let sealed_path = directory.path().join("sealed.sqlite");
    let mut sealed = CodeLexicalArtifactBuilderV1::create_with_memory_budget(
        &sealed_path,
        metadata,
        fixed + window + max_page_charge,
    )
    .expect("create admitting builder");
    let verified = sealed
        .rebuild_and_finalize(&mut source, &control)
        .expect("the refused source must resume instead of blocking");
