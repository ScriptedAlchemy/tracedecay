...
use tracedecay_code_index::production::{
    CodeIndexExecutionControlV1, VerifiedSealedLexicalPageV1, VerifiedSealedLexicalSourceReceiptV1,
};
...
use super::{
    ...
    open_builder_connection, sqlite_error,
};
...
impl CodeLexicalArtifactBuilderV1 {
    ...
    pub fn append_page(
        ...
        if page.chunk_count != page.chunks.len() as u64
            || page.import_count != page.imports.len() as u64
            || page.next_cursor.next_page_ordinal != page.page_ordinal.saturating_add(1)
            || page.next_cursor.cumulative_digest != page.cumulative_digest
        {
            return Err(CodeLexicalArtifactErrorV1::Contract(
                "sealed lexical page receipt is internally inconsistent".to_owned(),
            ));
        }
        ...
        append_imports(&transaction, page)?;
    ...
    pub fn finalize(
        ...
        if progress.next_page_ordinal != source.page_count
            || progress.completed_chunks != source.total_chunks
            || progress.completed_payload_bytes != source.total_payload_bytes
            || progress.completed_imports != source.total_imports
            || progress.completed_import_payload_bytes != source.import_payload_bytes
            || progress.import_dictionary_digest.as_ref() != Some(&source.import_dictionary_digest)
            || progress.cumulative_source_digest.as_ref() != Some(&source.cumulative_digest)
        {
            return Err(CodeLexicalArtifactErrorV1::Contract(
                "final lexical source receipt does not match staged pages".to_owned(),
            ));
        }
        let sections = compute_section_digests(&self.connection, control)?;
        ...
        let file_size_bytes = page_count.checked_mul(page_size).ok_or_else(|| {
...
fn progress(
    ...
    let progress: (i64, i64, i64, i64, i64, Option<String>, Option<String>) = connection
        .query_row(
            "SELECT COUNT(*), COALESCE(SUM(chunk_count), 0), COALESCE(SUM(payload_bytes), 0), COALESCE(SUM(import_count), 0), COALESCE(SUM(import_payload_bytes), 0), (SELECT import_dictionary_digest FROM source_pages ORDER BY page_ordinal DESC LIMIT 1), (SELECT cumulative_digest FROM source_pages ORDER BY page_ordinal DESC LIMIT 1) FROM source_pages",
            [],
...
fn encode_cursor(
    ...
    serde_json::to_vec(&(
        ...
        cursor.next_chunk_ordinal,
        cursor.next_page_ordinal,
        cursor.emitted_chunks,
        cursor.emitted_payload_bytes,
        cursor.next_import_ordinal,
        cursor.emitted_imports,
        cursor.emitted_import_payload_bytes,
        cursor.import_dictionary_digest.as_str(),
        cursor.cumulative_digest.as_str(),
    ))
    .map_err(|error| CodeLexicalArtifactErrorV1::Contract(error.to_string()))
}
