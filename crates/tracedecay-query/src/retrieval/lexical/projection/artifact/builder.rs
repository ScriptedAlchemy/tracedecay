...
use super::format::{
    ...
    metadata_digest, mount_metadata_digest, new_verified_receipt, padded_receipt,
};
...
impl CodeLexicalArtifactBuilderV1 {
    ...
    /// allocation-free conservative per-chunk/per-import transient bound
    /// (chunk clone, projected row and
    ...
    pub fn rebuild_and_finalize<R: Read + Seek>(
        ...
        let serving_metadata = CodeLexicalArtifactMountMetadataV1::from(&metadata);
        let serving_metadata_digest = mount_metadata_digest(&serving_metadata)?;
        let metadata_bytes = serde_json::to_vec(&serving_metadata)
            .map_err(|error| CodeLexicalArtifactErrorV1::Contract(error.to_string()))?;
...
fn validated_fixed_ledger_charge(
    ...
    // Canonical JSON can at most add escaping and structural delimiters to
    // the already-owned metadata. Charge a conservative expansion without
    // serializing: budget denial must precede JSON scratch allocation.
    let retained_metadata_bytes = metadata_retained_bytes(metadata);
    let serialized_bytes = retained_metadata_bytes
        .saturating_mul(2)
        .saturating_add(4 * 1024);
    let fixed = ARTIFACT_SQLITE_CACHE_BYTES
        .checked_add(retained_metadata_bytes.checked_mul(2).ok_or_else(|| {
            CodeLexicalArtifactErrorV1::Contract(
...
/// Conservative allocation-free upper bound for the widest transient used
/// while staging this page.
///
/// This preflight deliberately performs no clone, projection, tokenization,
/// n-gram reservation, or JSON serialization. The page's already-retained
/// heap is measured from its real capacities, while expansion-heavy derived
/// structures are charged from source lengths with a conservative multiplier.
/// A denial therefore precedes every staging allocation and mutation.
fn page_transient_peak_bytes(
    ...
    for admitted in page.chunks() {
        peak = peak.max(projected_chunk_transient_upper_bound(metadata, admitted)?);
        if peak > abort_above {
            return Ok(peak);
        }
    }
    for evidence in page.imports() {
        peak = peak.max(import_transient_upper_bound(evidence));
        if peak > abort_above {
            return Ok(peak);
        }
    }
...
fn projected_chunk_transient_upper_bound(
    metadata: &CodeLexicalProjectionMetadataV1,
    ...
    let exact_bytes = exact_terms_owned_bytes(chunk.exact_terms.capacity(), &chunk.exact_terms);
    let token_slots = text_bytes
        .saturating_add(logical_path.len())
        .saturating_add(exact_bytes);
    let field_bytes = token_slots.saturating_mul(
        std::mem::size_of::<String>()
            .saturating_add(BTREE_MAP_ENTRY_OVERHEAD_BYTES)
            .saturating_add(8),
    );
    let frequency_bytes = token_slots.saturating_mul(
        std::mem::size_of::<(&str, u32)>().saturating_add(BTREE_MAP_ENTRY_OVERHEAD_BYTES),
    );
    let normalized_scratch = ngram_scratch_upper_bound(text_bytes);
    let raw_scratch = ngram_scratch_upper_bound(text_bytes);
    let row_bytes = clone_bytes
        .saturating_add(logical_path.capacity())
        .saturating_add(text_bytes);
    let serialized_bytes = row_bytes.saturating_mul(2).saturating_add(4 * 1024);
    Ok(clone_bytes
...
fn ngram_scratch_upper_bound(text_len: usize) -> usize {
    text_len
        .saturating_mul(3)
...
fn import_transient_upper_bound(evidence: &CodeIndexImportEvidenceV1) -> usize {
    let owned = evidence
        .logical_path
        .capacity()
    ...
    owned.saturating_mul(2).saturating_add(1024)
}
...
fn append_page_rows(
    ...
    for admitted in page.chunks() {
        ...
        transaction
            .execute(
                "INSERT INTO rows(document_id, chunk_id, normalized_text, row) VALUES (?1, ?2, ?3, ?4)",
                params![
                    document,
                    artifact_row.id.as_str(),
                    artifact_row.normalized_text,
                    bytes
                ],
            )
            .map_err(|error| CodeLexicalArtifactErrorV1::Contract(error.to_string()))?;
...
pub(super) fn compute_section_digests(
    ...
    let queries = [
        ...
        "SELECT document_id, chunk_id, normalized_text, row FROM rows ORDER BY document_id",
...
fn verify_committed_artifact_state(
    ...
    let stored_metadata: CodeLexicalArtifactMountMetadataV1 =
        serde_json::from_slice(&metadata_bytes)
            .map_err(|error| CodeLexicalArtifactErrorV1::Corrupt(error.to_string()))?;
    let expected_serving_metadata = CodeLexicalArtifactMountMetadataV1::from(expected_metadata);
    ...
    if format_revision != CODE_LEXICAL_ARTIFACT_FORMAT_REVISION_V1
        || stored_metadata != expected_serving_metadata
