...
impl CodeLexicalArtifactReaderV1 {
    ...
    pub fn clone_body(
        ...
        let body = self.clone_body_from_row(row)?;
        if body.occurrence.symbol_occurrence_id != *symbol {
            return Err(CodeLexicalArtifactErrorV1::Corrupt(
                "clone body lookup returned the wrong occurrence".to_owned(),
            ));
        }
        Ok(Some(body))
    }
    ...
    fn clone_body_from_row(
        &self,
        row: (Vec<u8>, Option<Vec<u8>>),
    ) -> Result<CodeIndexCloneBodyV1, CodeLexicalArtifactErrorV1> {
        let (occurrence, payload) = row;
