...
fn measure_paired_effect(
    ...
    let status = match &interval {
        Some(interval)
            if sufficient
                && interval.mean_ppm >= i64::from(methodology.practical_effect_threshold_ppm)
...
fn paired_interval_95(differences: &[i64]) -> Option<PairedIntervalV1> {
    ...
    let variance = differences
        ...
        .sum::<f64>()
        / (population - 1.0);
    let half_width = t_critical_95(count as u64 - 1) * (variance / population).sqrt();
...
fn t_critical_95(degrees_of_freedom: u64) -> f64 {
    T_CRITICAL_95_BY_DEGREES_OF_FREEDOM
        ...
        .map_or(T_CRITICAL_95_BY_DEGREES_OF_FREEDOM[0].1, |(_, value)| {
            *value
        })
}
...
mod tests {
    ...
    fn held_out_effect_below_the_practical_threshold_fails() {
        ...
        for profile_id in [SEMANTIC_PROFILE, RERANK_PROFILE] {
            ...
            let baseline = profile_with_effect_needs(
            ...
            let effects =
                evaluate_paired_effects(&workload, &[baseline.clone(), candidate.clone()]);
