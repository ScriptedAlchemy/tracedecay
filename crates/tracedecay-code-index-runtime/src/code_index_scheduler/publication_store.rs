...
const FILE_ARTIFACT_CHECKPOINT_DIRECTORY_V1: &str = "code-generation-build-receipts-v1";
#[cfg(feature = "hotpath")]
...
impl DaemonCodeIndexPublicationStoreV1 {
    pub(super) fn new(
        ...
        // A retry after a failed parent sync observes `AlreadyExists`, so every
        // successful open must prove the ancestor entry durable again.
    ...
    fn file_artifact_checkpoint_directory_name(
    ...
    fn open_file_artifact_checkpoint_directory(
        ...
        match self.file_artifact_checkpoints_root.open_dir_nofollow(&name) {
            Ok(directory) => {
                if create {
                    // Re-prove the directory entry after both a fresh create
                    // and a retry that observed an already-created directory.
    ...
    fn clear_file_artifact_checkpoints(&self, predecessor_generation: Option<&CodeGenerationId>) {
        let directory =
    ...
    fn publish_atomically(
        ...
        *self
        ...
        if self.disposition == CodeIndexPublicationDispositionV1::Active {
            self.clear_file_artifact_checkpoints(predecessor_generation.as_ref());
        }
        *self
            .unpublished_candidate
            .lock()
