//! `SQLite` primitives for the final configuration control plane.
...
use canonical::validate_final_snapshot;
use codec::{
    StoredConfigurationProtectedOperationV1, insert_configuration_projections, invalid_store_data,
    unavailable_store,
};
#[cfg(test)]
...
use mutation::{current_state_from_transaction, map_store_error};
...
use write::insert_snapshot_entries;
...
impl<'db> GlobalDbConfigurationControlStore<'db> {
    ...
    /// Reports whether the installed final-schema store has no revision yet.
    pub fn is_empty(&self) -> ConfigurationOperationFuture<'_, bool> {
        Box::pin(async move {
            let read = self
                .db
                .read_snapshot()
            ...
            let revision_count = rows
                ...
                .ok_or_else(|| {
                    ConfigurationError::validation_message(
                        "configuration revision count returned no row",
                    )
                })?
                .get::<i64>(0)
                .map_err(|_| {
                ...
                .is_some()
            {
                return Err(ConfigurationError::validation_message(
                    "configuration revision count returned multiple rows",
                ));
            }
            Ok(revision_count == 0)
        })
    ...
    /// Atomically persists the one complete snapshot allowed in an empty final
    /// configuration store.
    pub async fn initialize_genesis(
        ...
        resolution
            ...
            .validate()
            .map_err(ConfigurationError::validation)?;
        validate_final_snapshot(&resolution.snapshot)?;
        let transaction = self
            .db
        ...
        let outcome = async {
            ...
            let existing = rows
                ...
                .ok_or_else(|| {
                    ConfigurationError::validation_message(
                        "configuration genesis count returned no row",
                    )
                })?
                .get::<i64>(0)
                .map_err(|_| {
                    ConfigurationError::validation_message(
                        "configuration genesis count is not an integer",
                    )
                })?;
            if existing != 0 {
