//! Publishing a profile-sharded store's project, store, and branch scope rows.
...
use crate::{GraphScopeUpsert, RegisteredGlobalDb, StoreArtifactUpsert, StoreInstanceUpsert};

/// Cheap fingerprint of everything that would change what
/// [`register_project_store`] writes.
///
...
/// project id, so a redundant [`register_project_store`] call (every writable
/// open re-runs this) can skip straight to `Ok(())` instead of redoing
/// branch-meta/git lookups and every upsert.
...
pub async fn register_project_store(
    ...
    let profile_root = profile_root_for_layout(store_layout)
    ...
    let (meta, git_common_dir, primary_root, git_remote_url, digest) =
        hotpath::measure_block!("lifecycle.register_project_store.digest", {
            ...
            // straight from `project_root` would let whichever worktree
            ...
            let artifact_mtimes = vec![
                artifact_mtime(&store_layout.graph_db_path),
                artifact_mtime(&store_layout.sessions_db_path),
                artifact_mtime(&store_layout.branch_meta_path),
                store_layout
                    .manifest_path
                    .as_deref()
                    .and_then(artifact_mtime),
            ];
            let git_remote_url = tracedecay_runtime_core::git::git_remote_url(project_root);
    ...
    storage::write_repository_identity_marker(project_root, &project.project_id)?;
    ...
    if let Some(primary_root) = primary_root.as_deref() {
        ...
        global_db
            .upsert_project_alias(project_root, &project.project_id)
            .await?;
    ...
    let manifest_relpath = store_layout
        .manifest_path
        .as_ref()
    ...
    push_existing_store_artifact(
        ...
        "branch_meta",
        &profile_root,
        &store_layout.branch_meta_path,
        None,
        now,
    );
    if let Some(manifest_path) = &store_layout.manifest_path {
        push_existing_store_artifact(
            &mut artifacts,
            &store.store_id,
            "store_manifest",
