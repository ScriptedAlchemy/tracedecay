...
const TEMPORAL_SCHEMA_OBJECTS: &[(&str, &str)] = &[
    ...
    ("index", "idx_session_relation_publications_pending"),
    ...
    ("index", "idx_session_temporal_ingest_dispositions_kind"),
    ("index", "idx_session_temporal_ingest_dispositions_row"),
    ("index", "idx_session_temporal_ingest_receipts_source"),
    ...
    ("table", "session_relation_publications"),
    ...
    (
        "trigger",
        "session_temporal_ingest_dispositions_immutable_delete_v1",
    ),
    (
        "trigger",
        "session_temporal_ingest_dispositions_immutable_update_v1",
    ),
    (
        "trigger",
        "session_temporal_ingest_receipts_immutable_delete_v1",
    ),
    (
        "trigger",
        "session_temporal_ingest_receipts_immutable_update_v1",
    ),
    (
        "trigger",
...
async fn temporal_schema_object_catalog(db_path: &Path) -> Vec<(String, String)> {
    ...
    while let Some(row) = rows.next().await.unwrap() {
        ...
        let temporal_namespace = table_name.starts_with("session_agent")
            ...
            || table_name.starts_with("session_relation_")
