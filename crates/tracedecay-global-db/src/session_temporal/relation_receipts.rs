        ...
        .execute(
            ...
            params![
                ...
                match &projection.scope {
                    super::relations::SessionRelationScope::Project { .. } => "project",
                    super::relations::SessionRelationScope::Profile { .. } => "profile",
                },
...
async fn expected_watermark(
    ...
    let mut rows = conn
        .query(
            "SELECT expected_graph_watermark
             FROM session_relation_receipts
             WHERE session_id = ?1 AND generation = ?2",
            params![
                session_id.as_str(),
    ...
    let watermark = rows
        ...
        .get::<String>(0)
        .map_err(|error| storage(RECEIPT_OPERATION, error))?;
    GraphWatermark::new(watermark).map_err(|error| storage(RECEIPT_OPERATION, error))
}
