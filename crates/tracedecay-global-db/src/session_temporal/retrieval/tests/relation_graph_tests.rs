...
use crate::session_temporal::relations::{
    LogicalCopyRelation, SessionRelationGraphStore, SessionRelationProjection,
    SessionRelationScope, SummaryRelationNode, SummarySourceRef,
};

fn profile_scope() -> SessionRelationScope {
    SessionRelationScope::profile(UserProfileId::new("profile.fixture").expect("profile"))
}
...
async fn records_from_projection(
    ...
    let store = SessionRelationGraphStore::memory().expect("relation graph");
