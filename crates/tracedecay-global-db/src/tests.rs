...
#[tokio::test]
async fn registered_mount_publishes_complete_final_schema() {
    let harness = RegisteredGlobalDbHarness::open("complete-final-schema").await;
    ...
    for (table, column) in [
        ...
        assert!(
            rows.next().await.unwrap().is_some(),
            "registered schema installation omitted {table}.{column}"
        );
    }
}
