...
use tracedecay_contracts::retrieval::{
    ...
    SimilarAlignedDifferenceV1, SimilarCoverageV1, SimilarFamilyV1, SimilarMatchClassV1,
    SimilarNearPairV1, SimilarOccurrenceV1, SimilarResultV1, SimilarSurfaceRequestV1,
    SimilarTargetV1,
};
use tracedecay_domain::errors::{Result, TraceDecayError};
use tracedecay_domain::{ExactClass, ProjectId, RepositoryId};
#[cfg(test)]
...
pub async fn handle_similar(ctx: &McpToolContext<'_>, args: Value) -> Result<ToolResult> {
    ...
    let (result, touched_files) =
        similar_surface_from_clone_index(similar, &project_id, &repository_id)?;
    let value =
...
fn similar_surface_from_clone_index(
    ...
    if let Some(near) = similar.near {
        for pair in near.page.members {
            for occurrence in pair.occurrences {
                ...
                let candidate = similar_occurrence(&occurrence);
                touched_files.push(candidate.path.clone());
