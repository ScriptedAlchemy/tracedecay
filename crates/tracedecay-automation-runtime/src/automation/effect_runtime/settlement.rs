...
//!
//! Admission of the retained request context stays in the composition root.
//! This kernel takes that already-admitted request plus the owner-resolved
//! memory authority and settles the two-phase journal.
...
use tracedecay_contracts::prepare_retained_effect;
use tracedecay_contracts::retained_receipts::PreparedRetainedEffect;
...
pub struct AutomationEffectAuthority {
...
pub struct RetainedSettlementWaiter<T> {
    task: tokio::task::JoinHandle<T>,
}

pub struct ReusedSchedulerSkipStartError {
...
impl ReusedSchedulerSkipStartError {
    pub fn into_error(self) -> tracedecay_domain::errors::TraceDecayError {
        self.error
    }
}
...
pub type AutomationLedgerObserver = Box<dyn FnOnce(&AutomationRunLedgerRecord) + Send + 'static>;
...
pub struct DeferredRunSettlementRequest {
    pub ledger: AutomationRunLedgerRecord,
    pub committed: Option<AutomationCommittedReceipt>,
    pub observer: Option<AutomationLedgerObserver>,
}

pub struct DeferredProblemSettlementRequest {
    pub error: AutomationRunError,
    pub observer: Option<AutomationLedgerObserver>,
}

pub enum DeferredSettlementRequest {
    Run(Box<DeferredRunSettlementRequest>),
    Problem(Box<DeferredProblemSettlementRequest>),
    Abandon,
}

pub struct DeferredSettledOutcome {
    pub terminal: AutomationSettledTerminal,
}

pub enum DeferredSettlementOutcome {
    Settled(Box<DeferredSettledOutcome>),
    Abandoned,
}

pub enum RetainedAutomationSettlementOutcome {
    Run {
...
pub enum RetainedAutomationSettlementProjection {
    Run {
        record: AutomationRunLedgerRecord,
...
pub struct RetainedSettlementPairWaiter {
...
pub struct DeferredSettlementPairSubmission<T> {
    payload: Option<T>,
...
impl<T> DeferredSettlementPairSubmission<T> {
    pub fn take_payload(&mut self) -> Result<T> {
        self.payload
            .take()
...
///
/// The root owns daemon-service request registration. This kernel never calls
/// that service; it only settles the journal from the admitted values.
pub struct AdmittedAutomationEffectRequest {
...
pub enum AutomationEffectAdmission {
...
impl AutomationEffectAuthority {
    ...
    pub fn start_retained_combined_settlement_pair(
    ...
    pub fn start_deferred_run_settlement_observed(
        self,
        ledger: AutomationRunLedgerRecord,
    ...
    pub fn start_deferred_problem_settlement_observed(
        self,
    ...
    pub fn start_reused_scheduler_skip_abandonment_observed(
        self,
    ...
    where
        M: FnOnce() -> Result<FactOwnerV1> + Send,
        F: FnOnce(RunId, FactReadControl) -> Fut + Send,
        Fut: Future<Output = Result<ProjectMemoryAutomationRunReceiptsV1>> + Send,
    {
        let AdmittedAutomationEffectRequest {
            context,
            cancellation,
            observed_at,
        ...
        let admission = match reservation {
            ...
            ReservationResult::Recover { retirement } => {
                ...
                let recovered =
                    read_receipts(authority.admission.request.run_id.clone(), read_control)
                        .await
                        .map_err(|error| {
                            contract_error(format!(
...
fn settle_direct(
    ...
    admission: &DurableAutomationAdmission,
    cancellation: &CancellationSignal,
    mut terminal: AutomationSettledTerminal,
    budget: Duration,
) -> Result<AutomationSettledTerminal> {
    let started = std::time::Instant::now();
    let mut delay = Duration::from_millis(25);
    loop {
        let error = match settle_direct_once(dashboard_root, journal_path, admission, &mut terminal)
        {
            Ok(()) => return Ok(terminal),
            Err(error) => {
                match classify_durable_settlement_blocking(journal_path, admission, &terminal, None)
                {
                    Ok(classification) if classification.is_terminal() => {
                        ...
                        cleanup_direct_terminal(dashboard_root, journal_path, admission);
                        return Ok(terminal);
                    }
                    Ok(_) => {
...
fn settle_direct_once(
    dashboard_root: &Path,
    journal_path: &Path,
    admission: &DurableAutomationAdmission,
    terminal: &mut AutomationSettledTerminal,
) -> Result<()> {
    *terminal = persist_terminal_blocking(journal_path, admission, terminal.clone())?;
    cleanup_direct_terminal(dashboard_root, journal_path, admission);
    Ok(())
}

fn cleanup_direct_terminal(
    dashboard_root: &Path,
    journal_path: &Path,
    admission: &DurableAutomationAdmission,
) {
    if let Err(error) = remove_pending_blocking(dashboard_root, journal_path) {
        tracing::warn!(
...
/// Durably abandons a reservation before the caller releases its scheduler guards.
fn abandon(
    dashboard_root: &Path,
    journal_path: &Path,
    admission: &DurableAutomationAdmission,
    ...
    let mut reservation_abandoned = false;
    let started = std::time::Instant::now();
    let mut delay = Duration::from_millis(25);
    loop {
        match abandon_once(
...
fn abandon_once(
    ...
    journal_path: &Path,
    admission: &DurableAutomationAdmission,
    reservation_abandoned: &mut bool,
) -> Result<()> {
    if !*reservation_abandoned {
        if let Err(error) = abandon_reservation_blocking(journal_path, admission) {
    ...
    remove_pending_blocking(dashboard_root, journal_path)
}
