...
pub(super) fn build_skill_writer_prompt(evidence: &Value) -> String {
    const POLICY: &str = concat!(
        ...
        "\n",
        "Keep each skill scoped to a recognizable recurring workflow. Prefer a concise body with the decisions and steps the agent needs; link optional detail in support_files and state when to read it. Do not force unrelated workflows into a broad umbrella or split one workflow into session-specific skills.\n",
        "\n",
        "Evaluate corrections, techniques, and skill-use failures against repeated workflow evidence before proposing a change. A single correction, explicit 'remember this', successful workaround, or skill load is not by itself evidence that a skill is needed. General preferences belong in memory or host instructions; include them in a skill only when they change that skill's recurring workflow.\n",
        "\n",
        "For an evidenced skill gap, choose the smallest change that fits:\n",
        "1. PATCH an existing skill only when its actual selection boundary and workflow cover the learning. Recent use or loading alone does not make it the right target. Correct obsolete guidance instead of accumulating exceptions.\n",
        "2. Put conditional reference material in that skill's support_files with a concise pointer and read condition in the body. Preserve its scope; do not broaden triggers merely to capture a new lesson.\n",
        "3. CREATE a skill only for a repeated workflow with a distinct selection boundary that existing skills do not cover. Use a durable task name, without PR numbers, transient error strings, or session narratives.\n",
        "Keep routing descriptions short and specific to when the skill is useful. Preserve the user's task, authorization, and completion requirements; do not add mandatory planning, approval, tool calls, or reports without a concrete workflow need. Retain applicable safety, validation, and error handling.\n",
        "\n",
        ...
        "- Negative claims about tools or features ('X is broken', 'browser tools do not work'). These harden into refusals the agent cites against itself long after the actual problem was fixed. If repeated workflow evidence supports a reusable setup fix, capture it under a relevant existing setup or troubleshooting skill — never 'this tool does not work' as a standalone constraint.\n",
        "- Session-specific transient errors that resolved before the session ended. A successful retry alone does not justify a persistent retry rule.\n",
