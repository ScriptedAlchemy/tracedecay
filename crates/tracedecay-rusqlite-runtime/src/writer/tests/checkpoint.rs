...
#[test]
fn scheduled_checkpoint_does_not_resample_wal_while_pressure_stays_below_soft() {
    let database = TestDatabase::new();
    let first = request(metadata(
        "operation.checkpoint.below-soft.first",
        "key.checkpoint.below-soft.first",
        's',
    ));
    let second = request(metadata(
        "operation.checkpoint.below-soft.second",
        "key.checkpoint.below-soft.second",
        't',
    ));
    let writer = start(&database, &first, Arc::new(AtomicU64::new(0)));
    let mut checkpoint_status = writer.checkpoint_handle().status_subscription();
    ...
    runtime
        ...
        .expect("first product write samples WAL once");
    let after_first = writer.telemetry_snapshot();
    assert!(
        matches!(
            checkpoint_status.borrow().latest.as_ref(),
            Some(CheckpointOutcome::BelowSoft { .. })
        ),
        "first write must still publish the existing below-soft sample"
    );
    assert_eq!(after_first.wal.checkpointed_frames, 0);
    assert_eq!(after_first.wal.hard_pressure_events, 0);
    assert_eq!(after_first.batches.committed_batches, 1);
    ...
    let after_second = writer.telemetry_snapshot();
    assert_eq!(after_second.batches.committed_batches, 2);
    assert_eq!(
        after_second.wal.wal_frames, after_first.wal.wal_frames,
        "second write below the soft threshold must not run another scheduled WAL sample"
    );
    assert_eq!(after_second.wal.wal_bytes, after_first.wal.wal_bytes);
    assert_eq!(after_second.wal.checkpointed_frames, 0);
    assert_eq!(after_second.wal.hard_pressure_events, 0);
    assert!(
        !checkpoint_status
            .has_changed()
            .expect("checkpoint watch stays open"),
        "below-soft product writes must not republish a scheduled checkpoint sample"
    );
