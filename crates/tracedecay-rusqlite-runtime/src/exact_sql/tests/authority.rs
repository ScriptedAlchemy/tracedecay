...
fn attach_rejects_different_verified_locators() {
    ...
    let result = ExactSqlHandle::attach(&fixture.writer, &fixture.readers);

    assert!(matches!(result, Err(ExactSqlError::AuthorityMismatch)));
}
...
fn attach_rejects_same_locator_bound_to_different_files() {
    ...
    let result = ExactSqlHandle::attach(&first.writer, &second.readers);
...
fn read_only_clone_cannot_recover_writer_authority() {
    ...
    assert!(matches!(error, ExactSqlError::WriterUnavailable));
}
...
fn writer_actor_allows_only_product_schema_pragmas() {
    ...
    for pragma in [
        ...
        assert!(
            matches!(error, ExactSqlError::Sqlite { .. }),
            "{pragma}: {error}"
        );
    }
}
...
/// denying it made every projection rebuild fail.
#[test]
...
#[test]
fn ordinary_transaction_cannot_request_an_authority_revalidated_batch() {
    let fixture = fixture('a', 'a');
    ...
    let error = transaction
        .execute_authority_revalidated_batch(
            "CREATE TABLE forbidden_schema_mode (id INTEGER)".to_owned(),
        )
        .unwrap_err();
...
fn long_lease_transaction_renews_its_lease_after_successful_bounded_steps() {
    ...
    let rows = base
        ...
        .unwrap();
    assert_eq!(rows.rows[0].values, vec![ExactSqlValue::Integer(5)]);
}

#[test]
fn authority_revalidated_batch_has_no_guessed_deadline_and_rechecks_authority() {
    let fixture = fixture('a', 'a');
    ...
    transaction
        .execute_authority_revalidated_batch(
            "CREATE TABLE long_revalidated_batch (value INTEGER);
                 WITH RECURSIVE n(value) AS (
                     VALUES(1)
                     UNION ALL
                     SELECT value + 1 FROM n WHERE value < 10000
                 )
                 INSERT INTO long_revalidated_batch SELECT value FROM n;"
                .to_owned(),
        )
        .unwrap();
...
#[test]
fn authority_loss_during_revalidated_batch_rolls_back_transaction() {
    let fixture = fixture('a', 'a');
    let base = ExactSqlHandle::attach(&fixture.writer, &fixture.readers).unwrap();
    let channel = ExactSqlHandle::attach(&fixture.writer, &fixture.readers)
        .unwrap()
    ...
    let error = transaction
        .execute_authority_revalidated_batch(
            "CREATE TABLE revoked_revalidated_batch (value INTEGER);
                 WITH RECURSIVE n(value) AS (
                     VALUES(1)
                     UNION ALL
                     SELECT value + 1 FROM n WHERE value < 10000
                 )
                 INSERT INTO revoked_revalidated_batch SELECT value FROM n;"
                .to_owned(),
        )
        .unwrap_err();
    ...
    let rows = base
        .query(
            statement(
                "SELECT count(*) FROM sqlite_schema WHERE name = 'revoked_revalidated_batch'",
                vec![],
            ),
            Duration::from_secs(1),
        )
