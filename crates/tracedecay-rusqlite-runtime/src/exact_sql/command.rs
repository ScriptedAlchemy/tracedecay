...
fn verify_family(family_guard: Option<&SqliteFamilyGuard>) -> Result<(), ExactSqlError> {
    family_guard
        .map_or(Ok(()), SqliteFamilyGuard::probe)
        .map_err(ExactSqlError::SqliteFamily)
}
...
fn verify_family_after_write(
    ...
    family_guard
        ...
        .map_err(ExactSqlError::CommitRecoveryRequired)
}

const fn is_write_intent(intent: ExactSqlWriteIntent) -> bool {
    matches!(
        intent,
        ExactSqlWriteIntent::Execute | ExactSqlWriteIntent::ExecuteBatch
    )
}

fn reject_transaction_command(command: TransactionCommand, error: ExactSqlError) {
    match command {
