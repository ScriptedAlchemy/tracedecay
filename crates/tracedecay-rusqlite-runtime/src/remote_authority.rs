...
impl RegisteredRemoteEnrollmentAuthorityV1 {
    ...
    pub fn provision_grant(
        ...
        let existing = enrollment_query_tx(
            ...
            vec![ExactSqlValue::Text(grant.grant_id.as_str().to_owned())],
        )?;
        ...
        enrollment_execute_tx(
            ...
            vec![
                ExactSqlValue::Text(grant.grant_id.as_str().to_owned()),
                ExactSqlValue::Integer(revision),
                ExactSqlValue::Text(encoded),
                ExactSqlValue::Text(admission_json),
            ],
        )?;
        transaction
            .commit()
        ...
        let rows = self
            ...
            .query(
                enrollment_statement(
                    ...
                    vec![ExactSqlValue::Text(enrollment_id.as_str().to_owned())],
                )?,
            ...
            .query(
                enrollment_statement(
                    "SELECT grant_json, consumed_at FROM remote_enrollment_grants_v1
                     WHERE grant_id = ?1",
                    vec![ExactSqlValue::Text(grant_id.as_str().to_owned())],
                )?,
                Duration::from_secs(5),
            )
            .map_err(enrollment_unavailable)?;
    ...
    fn commit_enrollment(
        ...
        let persisted = enrollment_query_tx(
            ...
            vec![
                ...
                ExactSqlValue::Text(grant_json.clone()),
            ],
        )?;
        ...
        let updated = transaction
            .execute(enrollment_statement(
                ...
                vec![
                    ...
                    ExactSqlValue::Text(enrollment.enrollment_id.as_str().to_owned()),
                    ExactSqlValue::Text(grant.grant_id.as_str().to_owned()),
                    ExactSqlValue::Integer(
                        i64::try_from(grant.revision)
                            .map_err(|_| RemoteEnrollmentAuthorityErrorV1::IdentityConflict)?,
        ...
        let inserted = transaction
            .execute(enrollment_statement(
                ...
                vec![
                    ExactSqlValue::Text(enrollment.enrollment_id.as_str().to_owned()),
                    ExactSqlValue::Integer(revision),
                    ExactSqlValue::Text(enrollment_json),
                ],
            )?)
            .map_err(enrollment_unavailable)?;
        ...
        let receipt_written = transaction
            .execute(enrollment_statement(
                ...
                vec![
                    ExactSqlValue::Text(enrollment.enrollment_id.as_str().to_owned()),
                    ExactSqlValue::Text(receipt_json),
                ],
            )?)
            .map_err(enrollment_unavailable)?;
        ...
        self.handle
            .execute(
                enrollment_statement(
                    ...
                    vec![
                        ExactSqlValue::Text(scope_digest.as_str().to_owned()),
                        ExactSqlValue::Integer(revision),
                        ExactSqlValue::Text(encoded.clone()),
                    ],
                )
                .map_err(|_| RemoteReplayApplicationErrorV1::PolicyUnavailable)?,
            )
        ...
        let rows = self
            ...
            .query(
                enrollment_statement(
                    ...
                    vec![ExactSqlValue::Text(scope_digest.as_str().to_owned())],
                )
                .map_err(|_| RemoteReplayApplicationErrorV1::PolicyUnavailable)?,
                Duration::from_secs(5),
            )
...
fn enrollment_statement(
    sql: &str,
    params: Vec<ExactSqlValue>,
) -> Result<ExactSqlStatement, RemoteEnrollmentAuthorityErrorV1> {
    ExactSqlStatement::new(sql.to_owned(), params).map_err(enrollment_unavailable)
}
...
fn enrollment_query_tx(
    ...
    sql: &str,
    params: Vec<ExactSqlValue>,
) -> Result<ExactSqlRows, RemoteEnrollmentAuthorityErrorV1> {
    transaction
...
fn enrollment_text(values: &[ExactSqlValue], index: usize) -> Option<&str> {
...
fn enrollment_unavailable(_: ExactSqlError) -> RemoteEnrollmentAuthorityErrorV1 {
