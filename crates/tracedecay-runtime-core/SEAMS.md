...
`leases`, `open`, `ports`, `tests`), `resolver`, `shard`, and `telemetry`.
See seam 9 for the part that stayed.
...
| `daemon::store_runtime::{graph_metadata, registry, resolver, shard, telemetry}` | Moved into `runtime_core::store_runtime`; root `src/daemon/store_runtime.rs` glob-re-exports. Closes seams 1 and 2. |
...
| `application::context::{CancellationToken, MonotonicDeadline}` | Moved into the new `runtime_core::cancellation`; `src/application/context.rs` re-exports. `is_same_token` widened from `pub(crate)` to `pub` for `src/application/session/types.rs`. |
