...
//! `FactStore` implementation. It owns every runtime projection
...
use super::super::primitives::{
    FACT_READ_OPERATION, FACT_WRITE_OPERATION, OwnerKey, QUERY_OPERATION, category_label,
    event_time, from_json, legacy_timestamp, nonnegative_u64, row_i64, row_string, source_store_id,
    storage_error, storage_message, to_json,
};
use super::super::projection::{
    fact_for_legacy_id_tx, load_projection_tx, load_projections_tx, resolve_target_tx,
};
use super::{DEFAULT_TRUST, FACT_RETENTION_CLASS, commit_fact_tx, query_fact_lineage_tx};
...
use tracedecay_store::{
    FactCommitOutcome, FactCommitReceipt, FactContentDigestQuery, FactHistory, FactHistoryQuery,
    FactLineageError, FactLineageQuery, FactLineageResult, FactListQuery, FactPage, FactProjection,
    FactStoreResult, FactTarget, FactWriteBatch,
};
pub(in crate::store::memory) async fn list_facts_tx(
    transaction: &Transaction<'_>,
    query: &FactListQuery,
) -> FactStoreResult<FactPage> {
    let key = OwnerKey::new(query.owner())?;
    let category = query.category().map(category_label);
    ...
    {
        fact_ids.push(
            FactId::new(row_string(&row, 0, QUERY_OPERATION)?).map_err(FactLineageError::from)?,
        );
    }
    drop(rows);
    ...
    let next = has_more
        ...
        .flatten();
    FactPage::new(query.owner().clone(), facts, next).map_err(Into::into)
}

pub(in crate::store::memory) async fn get_fact_tx(
    transaction: &Transaction<'_>,
    target: &FactTarget,
    ...
    load_projection_tx(transaction, target.owner(), &fact_id)
        .await
        .map_err(Into::into)
}

fn content_digest(content: &str) -> FactLineageResult<LocatorDigest> {
...
pub(in crate::store::memory) async fn find_fact_by_content_digest_tx(
    transaction: &Transaction<'_>,
    ...
    match matching_fact_id {
        Some(fact_id) => load_projection_tx(transaction, query.owner(), &fact_id)
            .await
            .map_err(Into::into),
        None => Ok(None),
    }
...
pub(in crate::store::memory) async fn fact_history_tx(
    transaction: &Transaction<'_>,
    ...
    FactHistory::new(query.target().owner().clone(), fact_id, events, None).map_err(Into::into)
}
...
pub(in crate::store::memory) fn value_strings(
    value: &Value,
    field: &'static str,
...
pub(in crate::store::memory) fn sanitize_payload(
    ...
    entities: &[String],
    metadata: &Value,
) -> FactLineageResult<Option<SanitizedPayload>> {
    ...
    let retention =
        RetentionClass::new(FACT_RETENTION_CLASS.to_owned()).map_err(FactLineageError::from)?;
    ...
    Ok(Some(SanitizedPayload {
        payload: fact_payload,
        access,
    }))
}

pub(in crate::store::memory) fn mirror_vector(
    payload: &FactPayloadV1,
) -> FactLineageResult<Vec<u8>> {
    let encoder = HolographicEncoder::new();
    HolographicEncoder::serialize(&encoder.encode_fact(payload.content(), payload.entities()))
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))
}

pub(super) async fn last_insert_rowid_tx(transaction: &Transaction<'_>) -> FactLineageResult<i64> {
    let mut rows = transaction
...
pub(in crate::store::memory) async fn mark_owner_banks_dirty_tx(
    db: &Database,
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    category: FactCategoryV1,
...
async fn mirror_replace_entities_tx(
    transaction: &Transaction<'_>,
    legacy_fact_id: i64,
    entities: &[String],
    timestamp: i64,
    ...
    while let Some(row) = rows
        ...
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
    {
        old_entity_ids.push(row_i64(&row, 0, FACT_WRITE_OPERATION)?);
    }
    drop(rows);
    transaction
        .execute(
...
pub(super) enum MirrorInsert {
    Inserted(i64),
    Existing { fact_id: FactId },
}

pub(super) async fn mirror_insert_tx(
    db: &Database,
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    payload: &FactPayloadV1,
    ...
    trust: Confidence,
    now: UtcMicros,
) -> FactLineageResult<MirrorInsert> {
    let timestamp = legacy_timestamp(now);
    let mut existing = transaction
        .query(
        ...
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
    {
        ...
        let Some(fact_id) = fact_for_legacy_id_tx(transaction, owner, legacy_fact_id).await? else {
            return Err(storage_message(
                FACT_WRITE_OPERATION,
                "compatibility mirror content is already bound to another owner or an unmigrated row",
            ));
        };
        return Ok(MirrorInsert::Existing { fact_id });
    }
    drop(existing);
    ...
    transaction
        ...
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
    let legacy_fact_id = last_insert_rowid_tx(transaction).await?;
    mirror_replace_entities_tx(transaction, legacy_fact_id, payload.entities(), timestamp).await?;
    mark_owner_banks_dirty_tx(db, transaction, owner, payload.category(), now).await?;
    Ok(MirrorInsert::Inserted(legacy_fact_id))
}

#[allow(clippy::too_many_arguments)]
pub(in crate::store::memory) async fn mirror_update_tx(
    db: &Database,
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    legacy_fact_id: i64,
...
pub(super) fn legacy_mapping_for_new_fact(
    owner: &FactOwnerV1,
    legacy_fact_id: i64,
    now: UtcMicros,
) -> FactLineageResult<(FactIdentityMaterialV1, LegacyFactMappingV1)> {
    let source_store_id = source_store_id()?;
...
pub(super) fn initial_batch(
    ...
    let imported_at = event_time(now, 0)?;
    let asserted_at = event_time(now, 1)?;
...
) -> FactLineageResult<(FactCommitReceipt, bool)> {
...
pub(super) async fn active_fact_count_tx(
    ...
    let row = rows
        ...
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
        .ok_or_else(|| storage_message(FACT_WRITE_OPERATION, "compatibility count is missing"))?;
    nonnegative_u64(row_i64(&row, 0, FACT_WRITE_OPERATION)?, "active fact count")
}

pub(in crate::store::memory) async fn mirror_delete_tx(
    db: &Database,
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    legacy_fact_id: i64,
    ...
    while let Some(row) = rows
        ...
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
    {
        entity_ids.push(row_i64(&row, 0, FACT_WRITE_OPERATION)?);
    }
    drop(rows);
    transaction
        .execute(
