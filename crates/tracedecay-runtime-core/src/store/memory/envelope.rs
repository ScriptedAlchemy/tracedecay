...
use tracedecay_store::{
    FactLineageError, FactLineageResult, FactStoreError, FactStoreResult, FactTarget,
};

use super::DatabaseFactStore;
use super::primitives::{
    FACT_READ_OPERATION, FACT_WRITE_OPERATION, OwnerKey, QUERY_OPERATION, from_json,
    row_optional_string, row_string, storage_error, storage_message, to_json,
};

pub(super) async fn finish_read_snapshot<T>(
    snapshot: Transaction<'_>,
    result: FactLineageResult<T>,
) -> FactLineageResult<T> {
    match result {
        Ok(value) => {
            snapshot
...
pub(super) fn digest(material: Value) -> FactLineageResult<String> {
...
pub(super) async fn lookup_operation_receipt_tx(
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    operation_id: &ProvenanceId,
    expected_kind: &'static str,
    request_digest: &str,
) -> FactLineageResult<Option<OperationReceipt>> {
    ...
    let operation_kind = row_string(&row, 0, FACT_WRITE_OPERATION)?;
    let stored_digest = row_string(&row, 1, FACT_WRITE_OPERATION)?;
    ...
    let fact_id = row_optional_string(&row, 2, FACT_WRITE_OPERATION)?
        .map(FactId::new)
        .transpose()
        .map_err(FactLineageError::from)?;
    let event_id = row_optional_string(&row, 3, FACT_WRITE_OPERATION)?
        .map(FactEventId::new)
        .transpose()
        .map_err(FactLineageError::from)?;
...
pub(super) async fn record_operation_receipt_tx(
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    operation_id: &ProvenanceId,
...
pub(super) fn target_digest(target: &FactTarget) -> FactLineageResult<Value> {
    match target {
        FactTarget::Canonical(target) => Ok(json!({
            "canonical_fact_id": target.fact_id().as_str(),
        })),
        FactTarget::Legacy(query) => Ok(json!({
...
pub(super) fn receipt_u64(receipt: &Value, field: &'static str) -> FactLineageResult<u64> {
...
impl DatabaseFactStore<'_> {
    pub(super) async fn read<T>(
        ...
        let snapshot = self
            .db
            .begin_memory_read_transaction(FACT_READ_OPERATION)
            .await
            .map_err(|error| FactStoreError::Store(storage_error(FACT_READ_OPERATION, error)))?;
        let result = work(&snapshot).await;
        match result {
            Ok(value) => {
                snapshot.commit().await.map_err(|error| {
                    FactStoreError::Store(storage_error(FACT_READ_OPERATION, error))
                })?;
                Ok(value)
            }
            Err(error) => match snapshot.rollback().await {
                Ok(()) => Err(error),
                Err(rollback) => Err(FactStoreError::Store(storage_error(
                    FACT_READ_OPERATION,
                    std::io::Error::other(format!(
                        "{error}; read snapshot rollback also failed: {rollback}"
                    )),
    ...
    pub(super) async fn write<T>(
        ...
        let transaction = self
            .db
            .begin_memory_write_transaction(FACT_WRITE_OPERATION)
            .await
            .map_err(|error| FactStoreError::Store(storage_error(FACT_WRITE_OPERATION, error)))?;
        let result = work(&transaction).await;
        match result {
            Ok(value) => {
                transaction.commit().await.map_err(|error| {
                    FactStoreError::Store(storage_error(FACT_WRITE_OPERATION, error))
                })?;
                Ok(value)
            }
