//! Owner-entity resolution, entity merges/aliases, and curation oplog replay.

use super::super::crud::{
    commit_batch_tx, load_current_fact_tx, mark_owner_banks_dirty_tx, mirror_update_tx,
    sanitize_payload,
};
use super::super::envelope::{OperationReceipt, receipt_u64, target_digest};
use super::super::primitives::{
    FACT_WRITE_OPERATION, OwnerKey, from_json, legacy_timestamp, row_i64, row_string,
    source_store_id, storage_error, storage_message, to_json,
};
use super::super::projection::{required_mapping_tx, source_for_fact_tx};
use super::super::proposals::proposal_category;
use super::{
    curated_correction_batch, curation_evidence_ids_tx, curation_mappings_from_ids_tx,
    record_curated_correction_provenance_tx, relation_label,
};
use crate::db::Database;
use crate::db::DatabaseMemoryTransaction as Transaction;
use crate::db::engine::params;
use crate::memory::entities::normalize_entity;
use serde_json::{Value, json};
use tracedecay_domain::{ActorId, FactId, FactOwnerV1, UtcMicros};
use tracedecay_store::{
    FactAddAlias, FactCurationOperation, FactCurationReceipt, FactEntityTarget, FactLineageError,
    FactLineageResult, FactMapping, FactMergeEntities, FactStoreResult, FactTarget,
    MemoryRepairStats,
};
async fn owner_entity_tx(
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    entity_id: i64,
) -> FactLineageResult<(String, Vec<String>)> {
    let key = OwnerKey::new(owner)?;
    let source_store_id = source_store_id()?;
    let foreign_links = transaction
        .query(
            "SELECT COUNT(*)
             FROM memory_fact_entities AS links
             LEFT JOIN memory_v2_legacy_map AS mappings
               ON mappings.legacy_fact_id = links.fact_id
             WHERE links.entity_id = ?1
               AND (
                    mappings.legacy_fact_id IS NULL
                    OR mappings.owner_kind <> ?2
                    OR mappings.project_id <> ?3
                    OR mappings.owner_json <> ?4
                    OR mappings.source_store_id <> ?5
               )",
            params![
                entity_id,
                key.kind,
                key.project_id.as_str(),
                key.json.as_str(),
                source_store_id.as_str(),
            ],
        )
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
    let mut foreign_links = foreign_links;
    let row = foreign_links
        .next()
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
        .ok_or_else(|| {
            storage_message(
                FACT_WRITE_OPERATION,
                "compatibility entity ownership count is missing",
            )
        })?;
    if row_i64(&row, 0, FACT_WRITE_OPERATION)? != 0 {
        return Err(storage_message(
            FACT_WRITE_OPERATION,
            "compatibility curation entity is shared outside this owner",
        ));
    }
    drop(foreign_links);
    let mut rows = transaction
        .query(
            "SELECT name, aliases FROM memory_entities WHERE entity_id = ?1",
            params![entity_id],
        )
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
    let row = rows
        .next()
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
        .ok_or_else(|| {
            storage_message(
                FACT_WRITE_OPERATION,
                "compatibility curation entity is missing",
            )
        })?;
    Ok((
        row_string(&row, 0, FACT_WRITE_OPERATION)?,
        from_json::<Vec<String>>(
            &row_string(&row, 1, FACT_WRITE_OPERATION)?,
            FACT_WRITE_OPERATION,
        )?,
    ))
}

async fn entity_linked_to_evidence_tx(
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    entity_id: i64,
    evidence_ids: &[FactId],
) -> FactLineageResult<()> {
    let key = OwnerKey::new(owner)?;
    let source_store_id = source_store_id()?;
    let placeholders = std::iter::repeat_n("?", evidence_ids.len())
        .collect::<Vec<_>>()
        .join(",");
    let sql = format!(
        "SELECT 1
         FROM memory_fact_entities AS links
         JOIN memory_v2_legacy_map AS mappings ON mappings.legacy_fact_id = links.fact_id
         WHERE links.entity_id = ?
           AND mappings.owner_kind = ? AND mappings.project_id = ?
           AND mappings.owner_json = ? AND mappings.source_store_id = ?
           AND mappings.fact_id IN ({placeholders})
         LIMIT 1"
    );
    let mut values = Vec::with_capacity(evidence_ids.len() + 5);
    values.push(crate::db::engine::Value::Integer(entity_id));
    values.push(crate::db::engine::Value::Text(key.kind.to_string()));
    values.push(crate::db::engine::Value::Text(key.project_id.clone()));
    values.push(crate::db::engine::Value::Text(key.json.clone()));
    values.push(crate::db::engine::Value::Text(
        source_store_id.as_str().to_owned(),
    ));
    values.extend(
        evidence_ids
            .iter()
            .map(|fact_id| crate::db::engine::Value::Text(fact_id.as_str().to_owned())),
    );
    let mut rows = transaction
        .query(&sql, values)
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
    if rows
        .next()
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
        .is_none()
    {
        return Err(storage_message(
            FACT_WRITE_OPERATION,
            "compatibility curation entity is not linked to supplied evidence",
        ));
    }
    Ok(())
}

async fn owner_entity_fact_ids_tx(
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    entity_ids: &[i64],
) -> FactLineageResult<Vec<FactId>> {
    let key = OwnerKey::new(owner)?;
    let source_store_id = source_store_id()?;
    let placeholders = std::iter::repeat_n("?", entity_ids.len())
        .collect::<Vec<_>>()
        .join(",");
    let sql = format!(
        "SELECT DISTINCT mappings.fact_id
         FROM memory_fact_entities AS links
         JOIN memory_v2_legacy_map AS mappings ON mappings.legacy_fact_id = links.fact_id
         WHERE mappings.owner_kind = ? AND mappings.project_id = ?
           AND mappings.owner_json = ? AND mappings.source_store_id = ?
           AND links.entity_id IN ({placeholders})
         ORDER BY mappings.fact_id ASC LIMIT 257"
    );
    let mut values = Vec::with_capacity(entity_ids.len() + 4);
    values.push(crate::db::engine::Value::Text(key.kind.to_string()));
    values.push(crate::db::engine::Value::Text(key.project_id.clone()));
    values.push(crate::db::engine::Value::Text(key.json.clone()));
    values.push(crate::db::engine::Value::Text(
        source_store_id.as_str().to_owned(),
    ));
    values.extend(
        entity_ids
            .iter()
            .copied()
            .map(crate::db::engine::Value::Integer),
    );
    let mut rows = transaction
        .query(&sql, values)
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
    let mut fact_ids = Vec::new();
    while let Some(row) = rows
        .next()
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
    {
        fact_ids.push(
            FactId::new(row_string(&row, 0, FACT_WRITE_OPERATION)?)
                .map_err(FactLineageError::from)?,
        );
    }
    if fact_ids.len() > 256 {
        return Err(storage_message(
            FACT_WRITE_OPERATION,
            "compatibility entity curation exceeds the fixed 256-fact bound",
        ));
    }
    Ok(fact_ids)
}

async fn fact_entities_tx(
    transaction: &Transaction<'_>,
    legacy_fact_id: i64,
) -> FactLineageResult<Vec<String>> {
    let mut rows = transaction
        .query(
            "SELECT entities.name
             FROM memory_fact_entities AS links
             JOIN memory_entities AS entities ON entities.entity_id = links.entity_id
             WHERE links.fact_id = ?1
             ORDER BY entities.normalized_name ASC, entities.entity_id ASC",
            params![legacy_fact_id],
        )
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
    let mut entities = Vec::new();
    while let Some(row) = rows
        .next()
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
    {
        entities.push(row_string(&row, 0, FACT_WRITE_OPERATION)?);
    }
    Ok(entities)
}

pub(super) async fn merge_entities_tx(
    db: &Database,
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    actor: Option<&ActorId>,
    operation: &FactMergeEntities,
    now: UtcMicros,
) -> FactLineageResult<Vec<FactId>> {
    let evidence = curation_evidence_ids_tx(transaction, owner, operation.evidence_facts()).await?;
    let winner_id = operation.winner().legacy_entity_id();
    let (winner_name, winner_aliases) = owner_entity_tx(transaction, owner, winner_id).await?;
    entity_linked_to_evidence_tx(transaction, owner, winner_id, &evidence).await?;
    let mut entity_ids = vec![winner_id];
    let mut aliases = winner_aliases;
    for loser in operation.losers() {
        let loser_id = loser.legacy_entity_id();
        let (name, loser_aliases) = owner_entity_tx(transaction, owner, loser_id).await?;
        entity_linked_to_evidence_tx(transaction, owner, loser_id, &evidence).await?;
        entity_ids.push(loser_id);
        aliases.push(name);
        aliases.extend(loser_aliases);
    }
    let fact_ids = owner_entity_fact_ids_tx(transaction, owner, &entity_ids).await?;
    let mut normalized_aliases = std::collections::BTreeMap::new();
    for alias in aliases {
        let alias = normalize_entity(&alias);
        if !alias.is_empty() && !alias.eq_ignore_ascii_case(&winner_name) {
            normalized_aliases
                .entry(alias.to_ascii_lowercase())
                .or_insert(alias);
        }
    }
    transaction
        .execute(
            "UPDATE memory_entities SET aliases = ?1, updated_at = ?2 WHERE entity_id = ?3",
            params![
                to_json(
                    &normalized_aliases.into_values().collect::<Vec<_>>(),
                    "serialize compatibility entity aliases",
                )?,
                legacy_timestamp(now),
                winner_id,
            ],
        )
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
    for loser in operation.losers() {
        let loser_id = loser.legacy_entity_id();
        transaction
            .execute(
                "INSERT OR IGNORE INTO memory_fact_entities(fact_id, entity_id)
                 SELECT fact_id, ?1 FROM memory_fact_entities WHERE entity_id = ?2",
                params![winner_id, loser_id],
            )
            .await
            .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
        transaction
            .execute(
                "DELETE FROM memory_fact_entities WHERE entity_id = ?1",
                params![loser_id],
            )
            .await
            .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
        transaction
            .execute(
                "DELETE FROM memory_entities WHERE entity_id = ?1
                 AND NOT EXISTS(SELECT 1 FROM memory_fact_entities WHERE entity_id = ?1)",
                params![loser_id],
            )
            .await
            .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
    }
    let owner_key = OwnerKey::new(owner)?;
    for fact_id in &fact_ids {
        let Some(fact) = load_current_fact_tx(transaction, &owner_key, owner, fact_id).await?
        else {
            continue;
        };
        let Some(payload) = fact.payload() else {
            continue;
        };
        let mapping = required_mapping_tx(transaction, owner, fact_id).await?;
        let entities = fact_entities_tx(transaction, mapping.legacy_fact_id()).await?;
        let Some(sanitized) = sanitize_payload(
            payload.content(),
            payload.category(),
            payload.tags(),
            &entities,
            payload.metadata(),
        )?
        else {
            return Err(storage_message(
                FACT_WRITE_OPERATION,
                "compatibility merged entities were rejected by the privacy sanitizer",
            ));
        };
        let source = source_for_fact_tx(transaction, &mapping).await?;
        let batch =
            curated_correction_batch(&fact, sanitized.payload.clone(), actor.cloned(), now)?;
        commit_batch_tx(transaction, &batch).await?;
        // The evidence facts are themselves linked to the merged entities, so
        // they are always members of `fact_ids`. Recording one as derived from
        // itself trips the self-reference guard and would roll back the whole
        // merge, so correct each fact against the remaining evidence only and
        // skip the relation entirely when it was the sole evidence fact.
        let fact_evidence = evidence
            .iter()
            .filter(|evidence_fact_id| *evidence_fact_id != fact_id)
            .cloned()
            .collect::<Vec<_>>();
        if !fact_evidence.is_empty() {
            record_curated_correction_provenance_tx(
                transaction,
                owner,
                fact_id,
                &fact_evidence,
                operation.confidence(),
                "merge_entities",
                actor,
                now,
            )
            .await?;
        }
        mirror_update_tx(
            db,
            transaction,
            owner,
            mapping.legacy_fact_id(),
            &sanitized.payload,
            &source,
            fact.trust(),
            now,
        )
        .await?;
    }
    Ok(fact_ids)
}

pub(super) async fn add_entity_alias_tx(
    db: &Database,
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    operation: &FactAddAlias,
    now: UtcMicros,
) -> FactLineageResult<Vec<FactId>> {
    let evidence = curation_evidence_ids_tx(transaction, owner, operation.evidence_facts()).await?;
    let entity_id = operation.entity().legacy_entity_id();
    let (name, mut aliases) = owner_entity_tx(transaction, owner, entity_id).await?;
    entity_linked_to_evidence_tx(transaction, owner, entity_id, &evidence).await?;
    let alias = normalize_entity(operation.alias());
    if alias.is_empty() || alias.eq_ignore_ascii_case(&name) {
        return Err(storage_message(
            FACT_WRITE_OPERATION,
            "compatibility alias is not distinct from its entity",
        ));
    }
    aliases.push(alias);
    let mut canonical_aliases = std::collections::BTreeMap::new();
    for value in aliases {
        let value = normalize_entity(&value);
        if !value.is_empty() && !value.eq_ignore_ascii_case(&name) {
            canonical_aliases
                .entry(value.to_ascii_lowercase())
                .or_insert(value);
        }
    }
    transaction
        .execute(
            "UPDATE memory_entities SET aliases = ?1, updated_at = ?2 WHERE entity_id = ?3",
            params![
                to_json(
                    &canonical_aliases.into_values().collect::<Vec<_>>(),
                    "serialize compatibility entity aliases",
                )?,
                legacy_timestamp(now),
                entity_id,
            ],
        )
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
    let fact_ids = owner_entity_fact_ids_tx(transaction, owner, &[entity_id]).await?;
    for fact_id in &fact_ids {
        let mapping = required_mapping_tx(transaction, owner, fact_id).await?;
        let mut rows = transaction
            .query(
                "SELECT category FROM memory_facts WHERE fact_id = ?1",
                params![mapping.legacy_fact_id()],
            )
            .await
            .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
        let row = rows
            .next()
            .await
            .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
            .ok_or_else(|| {
                storage_message(
                    FACT_WRITE_OPERATION,
                    "compatibility alias fact is missing from the legacy mirror",
                )
            })?;
        let category = proposal_category(&row_string(&row, 0, FACT_WRITE_OPERATION)?)?;
        mark_owner_banks_dirty_tx(db, transaction, owner, category, now).await?;
    }
    Ok(fact_ids)
}

pub(super) fn curation_operation_digest(
    operation: &FactCurationOperation,
) -> FactLineageResult<Value> {
    let evidence = |targets: &[FactTarget]| {
        targets
            .iter()
            .map(target_digest)
            .collect::<FactLineageResult<Vec<_>>>()
    };
    match operation {
        FactCurationOperation::NormalizeTags(operation) => Ok(json!({
            "kind": "normalize_tags",
            "fact": target_digest(operation.fact())?,
            "tags": operation.tags(),
            "evidence": evidence(operation.evidence_facts())?,
            "confidence": operation.confidence().as_f64(),
        })),
        FactCurationOperation::MergeEntities(operation) => Ok(json!({
            "kind": "merge_entities",
            "winner": operation.winner().legacy_entity_id(),
            "losers": operation.losers().iter().map(FactEntityTarget::legacy_entity_id).collect::<Vec<_>>(),
            "evidence": evidence(operation.evidence_facts())?,
            "confidence": operation.confidence().as_f64(),
        })),
        FactCurationOperation::AddAlias(operation) => Ok(json!({
            "kind": "add_alias",
            "entity": operation.entity().legacy_entity_id(),
            "alias": operation.alias(),
            "evidence": evidence(operation.evidence_facts())?,
            "confidence": operation.confidence().as_f64(),
        })),
        FactCurationOperation::LinkFacts(operation) => Ok(json!({
            "kind": "link_facts",
            "source": target_digest(operation.source())?,
            "target": target_digest(operation.target())?,
            "relation": relation_label(operation.relation()),
            "evidence": evidence(operation.evidence_facts())?,
            "confidence": operation.confidence().as_f64(),
            "source_label": operation.source_label(),
            "metadata": operation.metadata(),
        })),
        FactCurationOperation::RepairVector(operation) => Ok(json!({
            "kind": "repair_vector",
            "fact": target_digest(operation.fact())?,
            "evidence": evidence(operation.evidence_facts())?,
            "confidence": operation.confidence().as_f64(),
        })),
    }
}

pub(super) async fn record_oplog_tx(
    transaction: &Transaction<'_>,
    operation: &str,
    mapping: Option<&FactMapping>,
    detail: &Value,
    now: UtcMicros,
) -> FactLineageResult<()> {
    transaction
        .execute(
            "INSERT INTO memory_oplog(ts, op, fact_id, detail_json) VALUES(?1, ?2, ?3, ?4)",
            params![
                legacy_timestamp(now),
                operation,
                mapping.and_then(FactMapping::legacy_fact_id),
                to_json(detail, "serialize compatibility oplog detail")?,
            ],
        )
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
    Ok(())
}

pub(super) async fn replay_curation_tx(
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    receipt: &OperationReceipt,
) -> FactStoreResult<FactCurationReceipt> {
    let ids = receipt
        .receipt
        .get("changed_fact_ids")
        .and_then(Value::as_array)
        .ok_or_else(|| {
            storage_message(
                FACT_WRITE_OPERATION,
                "compatibility curation receipt changed facts are malformed",
            )
        })?;
    let mut fact_ids = Vec::with_capacity(ids.len());
    for id in ids {
        fact_ids.push(
            FactId::new(id.as_str().ok_or_else(|| {
                storage_message(
                    FACT_WRITE_OPERATION,
                    "compatibility curation receipt fact id is malformed",
                )
            })?)
            .map_err(FactLineageError::from)?,
        );
    }
    let mappings = curation_mappings_from_ids_tx(transaction, owner, &fact_ids).await?;
    let derived_repair = MemoryRepairStats::new(
        receipt_u64(&receipt.receipt, "missing_vectors_repaired")?,
        receipt_u64(&receipt.receipt, "banks_rebuilt")?,
    );
    FactCurationReceipt::new(
        owner.clone(),
        mappings,
        receipt_u64(&receipt.receipt, "normalized_tags")?,
        receipt_u64(&receipt.receipt, "merged_entities")?,
        receipt_u64(&receipt.receipt, "aliases_added")?,
        receipt_u64(&receipt.receipt, "facts_linked")?,
        receipt_u64(&receipt.receipt, "vectors_repaired")?,
        derived_repair,
    )
    .map_err(Into::into)
}
