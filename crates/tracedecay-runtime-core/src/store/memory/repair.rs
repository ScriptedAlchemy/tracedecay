...
use tracedecay_store::{
    FactLineageError, FactLineageResult, FactRepairVector, FactStoreResult, FeedbackRepairProgress,
    MemoryRepairCommand, MemoryRepairStats,
};

use super::crud::{load_current_fact_tx, mark_owner_banks_dirty_tx, mirror_vector};
use super::curation::{available_curation_fact_tx, curation_evidence_ids_tx};
use super::envelope::{
    digest, lookup_operation_receipt_tx, receipt_u64, record_operation_receipt_tx,
};
use super::primitives::{
    FACT_READ_OPERATION, FACT_WRITE_OPERATION, OwnerKey, legacy_timestamp, nonnegative_u64, now,
    row_i64, row_string, source_store_id, storage_error, storage_message,
};
use super::projection::required_mapping_tx;
...
/// converging backlog.
pub(crate) const FACT_REPAIR_VECTOR_BATCH: i64 = 512;

pub(crate) const FACT_REPAIR_BANK_BATCH: i64 = 32;
...
/// the daemon scheduler can consume [`MemoryRepairStats::saturated`]
/// without depending on these store-internal constants.
fn repair_batches_saturated(missing_vectors_repaired: u64, banks_rebuilt: u64) -> bool {
    missing_vectors_repaired >= FACT_REPAIR_VECTOR_BATCH as u64
        || banks_rebuilt >= FACT_REPAIR_BANK_BATCH as u64
}

pub(super) async fn repair_vector_for_fact_tx(
    db: &Database,
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    operation: &FactRepairVector,
    now: UtcMicros,
    ...
    mark_owner_banks_dirty_tx(db, transaction, owner, payload.category(), now).await?;
    Ok(fact_id)
}

pub(super) fn repair_request_digest(request: &MemoryRepairCommand) -> FactLineageResult<String> {
    digest(json!({
        "owner": request.owner(),
...
pub(super) async fn repair_memory_tx(
    db: &Database,
    transaction: &Transaction<'_>,
    ...
    .await?
    {
        let missing_vectors_repaired = receipt_u64(&receipt.receipt, "missing_vectors_repaired")?;
        let banks_rebuilt = receipt_u64(&receipt.receipt, "banks_rebuilt")?;
        return Ok(
            MemoryRepairStats::new(missing_vectors_repaired, banks_rebuilt).with_saturated(
                repair_batches_saturated(missing_vectors_repaired, banks_rebuilt),
            ),
        );
    }
    let now = now()?;
    let missing_vectors_repaired =
        repair_missing_vectors_tx(db, transaction, request.owner(), FACT_REPAIR_VECTOR_BATCH)
            .await?;
    mark_absent_banks_dirty_tx(db, transaction, request.owner(), now).await?;
...
pub(super) async fn repair_missing_vectors_tx(
    db: &Database,
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    limit: i64,
) -> FactLineageResult<u64> {
    ...
    for fact_id in fact_ids {
        ...
        let vector = mirror_vector(payload)?;
        let changed = transaction
            .execute(
                "UPDATE memory_facts
                 SET hrr_vector = ?1,
...
async fn mark_absent_banks_dirty_tx(
    db: &Database,
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
    now: UtcMicros,
    ...
    let bank_count = rows
        ...
        .await
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
        .map(|row| row_i64(&row, 0, FACT_WRITE_OPERATION))
        .transpose()?
        .unwrap_or(0);
    drop(rows);
    if bank_count > 0 {
    ...
    while let Some(row) = rows
        ...
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
    {
        bank_names.push(row_string(&row, 0, FACT_WRITE_OPERATION)?);
    }
    drop(rows);
    ...
    for bank_name in bank_names {
        db.mark_memory_v2_bank_dirty_in_transaction(
            transaction,
            owner,
            &source_store_id,
            &bank_name,
...
pub(super) async fn rebuild_dirty_banks_tx(
    db: &Database,
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
) -> FactLineageResult<u64> {
        ...
        .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
    {
        dirty.push((
            ...
            UtcMicros(row_i64(&row, 1, FACT_WRITE_OPERATION)?),
        ));
    }
    drop(rows);
    let now = now()?;
    ...
    for (bank_name, dirty_updated_at) in dirty {
        ...
        while let Some(row) = vectors
            ...
            .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?
        {
            let legacy_fact_id = row_i64(&row, 0, FACT_WRITE_OPERATION)?;
            let fact_id = FactId::new(row_string(&row, 1, FACT_WRITE_OPERATION)?)
                .map_err(FactLineageError::from)?;
        ...
        for (legacy_fact_id, fact_id) in malformed_legacy_fact_ids {
            let replacement = match load_current_fact_tx(transaction, &key, owner, &fact_id).await?
            {
                Some(fact) => fact.payload().and_then(|payload| {
                    mirror_vector(payload).ok().and_then(|bytes| {
                        HolographicEncoder::deserialize(&bytes)
                            .ok()
                            .filter(|vector| {
                                vector.len() == HolographicEncoder::DIMENSIONS
        ...
        if decoded.is_empty() {
            db.delete_memory_v2_bank_in_transaction(
                transaction,
                owner,
                &source_store_id,
                bank_name.as_str(),
            ...
            let vector = HolographicEncoder::serialize(&average_vectors(&decoded))
                .map_err(|error| storage_error(FACT_WRITE_OPERATION, error))?;
...
pub(super) async fn feedback_history_repair_progress_tx(
    transaction: &Transaction<'_>,
    owner: &FactOwnerV1,
) -> FactStoreResult<FeedbackRepairProgress> {
    let key = OwnerKey::new(owner)?;
    let source_store_id = source_store_id()?;
    let mut rows = transaction
        .query(
    ...
    let Some(row) = rows
        ...
        return Ok(FeedbackRepairProgress::NotRequired);
    };
    if row_string(&row, 0, FACT_READ_OPERATION)? != key.json {
    ...
    match row_string(&row, 3, FACT_READ_OPERATION)?.as_str() {
        "pending" => Ok(FeedbackRepairProgress::Incomplete {
            processed: 0,
            remaining: Some(frontier.saturating_sub(cursor)),
        }),
        "complete" => Ok(FeedbackRepairProgress::Complete { processed: 0 }),
