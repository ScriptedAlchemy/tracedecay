...
/// `tracedecay-global-db`, which already depends on this crate, so the kernel
/// cannot name it without a Cargo cycle.
///
/// This port **fails closed**: an uninitialised profile or session store is not
/// safe to publish, so an
