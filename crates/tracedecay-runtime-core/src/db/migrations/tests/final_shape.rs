...
fn seed_released_project_rows(path: &Path) {
    tamper(
        ...
        "INSERT INTO metadata(key, value) VALUES('mac.profile', 'beta.32');
         INSERT INTO semantic_vector_stage_census_authority VALUES('released-shard', 7);
...
fn seeded_project_rows(path: &Path) -> Vec<String> {
    ...
    for sql in [
        "SELECT json_array(assertion_id, fact_id, payload_json, content) FROM memory_v2_assertion_payloads ORDER BY assertion_id".to_owned(),
        "SELECT json_array(shard_id, revision) FROM semantic_vector_stage_census_authority ORDER BY shard_id".to_owned(),
...
async fn released_project_store_migrates_and_retains_every_row() {
    ...
    let read_only = verify_final_schema_connection(&TestConnection::open(&path))
        .await
        .expect_err("a released store must await writer convergence");
    assert!(read_only.reset_required_context().is_none(), "{read_only}");
    let seeded = seeded_project_rows(&path);
    assert_eq!(seeded.len(), 6, "the fixture must seed rows to retain");
    ...
    verify_final_schema_connection(&TestConnection::open(&path))
        .await
        .expect("read-only admission accepts the converged released store");
    assert_eq!(
...
async fn released_project_store_without_diagnostics_converges() {
    ...
    tamper(
        &path,
        "DROP TABLE generation_diagnostics; DROP TABLE diagnostic_generation_publications;",
    );
    admit_existing(
        &path,
        "lazily absent released diagnostics must be installed",
    )
    .await;
    verify_final_schema_connection(&TestConnection::open(&path))
        .await
...
#[tokio::test]
async fn released_project_schema_drift_is_refused_without_mutation() {
    for mutation in [
        "ALTER TABLE semantic_vector_stage_census_authority ADD COLUMN tamper TEXT;",
        "DROP TRIGGER semantic_vector_stage_census_after_stage_insert;",
...
async fn live_v35_project_store_converges_and_reopens_without_reset() {
    ...
    tamper(&path, "PRAGMA user_version = 35;");
    let seeded = seeded_project_rows(&path);
    let read_only = verify_final_schema_connection(&TestConnection::open(&path))
        .await
        .expect_err("a prior stamp needs a writer");
    assert!(read_only.reset_required_context().is_none(), "{read_only}");
    admit_existing(&path, "a live v35 store must migrate without reset").await;
    assert_eq!(seeded_project_rows(&path), seeded);
    assert_eq!(
        store_snapshot(&path).user_version,
        i64::from(SCHEMA_VERSION)
    );
