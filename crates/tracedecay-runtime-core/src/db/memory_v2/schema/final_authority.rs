...
pub(super) async fn install_final_memory_support(
    ...
    install_final_memory_entities(conn, operation).await?;
    install_final_memory_banks(conn, operation).await
}

async fn install_final_memory_entities(
    conn: &impl MemoryV2Executor,
    operation: &str,
) -> Result<()> {
    conn.execute_batch(
        "CREATE TABLE IF NOT EXISTS memory_v2_entities (
            ...
            entity_type TEXT NOT NULL CHECK(length(entity_type) BETWEEN 1 AND 256),
            aliases_json TEXT NOT NULL CHECK(
                json_valid(aliases_json) AND json_type(aliases_json) = 'array'
            ),
            created_at INTEGER NOT NULL,
            updated_at INTEGER NOT NULL CHECK(updated_at >= created_at),
            PRIMARY KEY(entity_id, owner_kind, project_id),
            UNIQUE(owner_kind, project_id, normalized_name),
            CHECK(
                (owner_kind = 'profile' AND project_id = '') OR
                (owner_kind = 'project' AND project_id <> '')
            )
        );
        CREATE TABLE IF NOT EXISTS memory_v2_fact_entities (
            fact_id TEXT NOT NULL,
            entity_id TEXT NOT NULL,
            owner_kind TEXT NOT NULL,
            project_id TEXT NOT NULL,
            linked_at INTEGER NOT NULL,
            PRIMARY KEY(fact_id, entity_id, owner_kind, project_id),
            FOREIGN KEY(fact_id, owner_kind, project_id)
                REFERENCES memory_v2_facts(fact_id, owner_kind, project_id),
            FOREIGN KEY(entity_id, owner_kind, project_id)
                REFERENCES memory_v2_entities(entity_id, owner_kind, project_id)
        );
        CREATE INDEX IF NOT EXISTS idx_memory_v2_entities_owner_name
            ON memory_v2_entities(owner_kind, project_id, normalized_name);
        CREATE INDEX IF NOT EXISTS idx_memory_v2_fact_entities_entity
            ON memory_v2_fact_entities(entity_id, owner_kind, project_id, fact_id);
        CREATE TRIGGER IF NOT EXISTS memory_v2_entity_identity_guard
        BEFORE UPDATE ON memory_v2_entities
        WHEN NEW.entity_id IS NOT OLD.entity_id
            ...
            OR NEW.updated_at < OLD.updated_at
        BEGIN
            SELECT RAISE(ABORT, 'memory_v2 entity identity is immutable');
        END;
        CREATE TRIGGER IF NOT EXISTS memory_v2_fact_entities_no_update
        BEFORE UPDATE ON memory_v2_fact_entities BEGIN
            SELECT RAISE(ABORT, 'memory_v2 fact entity links are immutable');
        END;",
    )
    .await
    .map_err(|error| db_error(operation, error))
