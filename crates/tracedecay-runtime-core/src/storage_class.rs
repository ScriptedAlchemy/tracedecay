//! Store durability classification for runtime recovery and retention.
//!
//! Large recoverable session stores and small curated stores need different
//! recovery and retention treatment. Bulk transcript and derived evidence can
//! be re-ingested, while curated state cannot.
//!
//! The root cause was treating every store as equally precious. This module
//! gives recovery and retention a typed vocabulary for that difference,
//! mirroring the product contract in
...
//!   over code). Rebuild from source rather than rewriting derived rows.
//! - [`StoreDurabilityClass::Durable`] -- irreplaceable curated knowledge
//!   (`user-memory.db`, per-project `memory_*` tables, the `global.db`
//!   registry, configuration tables). Never skip, drop, or silently lose it.
...
//!   `observation_repository_provenance`). Rebuild failures can be retried
...
/// How precious a store's data is to the owner.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum StoreDurabilityClass {
    /// Fully rebuildable from the git checkout.
    Derived,
    /// Irreplaceable curated knowledge that must never be silently lost.
    Durable,
    /// Re-ingestable from an upstream source of truth (provider JSONL, or
    /// re-derivable from other `Recoverable`/raw data).
    Recoverable,
}
...
///   `ensure_registered_schema` (`tracedecay-global-db/src/schema.rs`) applies to
...
/// verdict gates are non-destructive retries: failing one leaves every byte on
/// disk for the next open. `Project` is `Durable` because its dominant tables
...
/// Whether a whole store file of this kind may be dropped and rebuilt
/// outright -- the question a *destructive* whole-file operation must ask.
///
...
/// applied to `sessions.db` (see `tracedecay-global-db/src/schema.rs`'s
