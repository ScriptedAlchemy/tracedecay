...
3. Narrow temporal evidence with `lcm_grep` / `tracedecay_lcm_grep` (default
   ...
   Summary node IDs are opaque strings, not integers. The authenticated opaque
   continuation cursor applies only to summary source pages; numeric source
   offsets and limits are not accepted. Raw and external payload expansion use
   content pagination.
   Continue a summary page only by returning its opaque `next_cursor`
   unchanged with the same target and content slice; changing a
