...
pub async fn read_published_artifact_chain(
    ...
    for artifact in &artifacts {
        let kind = AutomationRunArtifactKind::parse(&artifact.kind).ok_or_else(|| {
            config_error(format!("invalid published artifact '{}'", artifact.kind))
        })?;
        if !seen.insert(kind.as_str()) || artifact.path != artifact_relative_path(run_id, kind) {
            return Err(config_error(format!(
                "artifact publication contains a duplicate or non-canonical '{}' entry",
                artifact.kind
            )));
        }
    ...
    {
        return Err(config_error(
            "artifact publication does not contain the complete chain",
        ));
    }
