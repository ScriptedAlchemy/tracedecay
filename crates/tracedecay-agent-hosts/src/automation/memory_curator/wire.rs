...
reviewed_fact is exactly {"fact_id":fact_id,"expected_last_event_id":event_id}. For every operation, confidence must be finite in [context.min_confidence,1]. Trust, when present, must be finite in [0,1]. evidence_facts must contain 1..=256 unique reviewed_fact pairs copied from the supplied facts. Every reviewed_fact pair must be copied from the same supplied fact. Merge losers must be nonempty, unique, and distinct from the winner. reason and source_label must be nonblank, at most 4096 bytes, and contain no control characters."#;
