...
pub async fn count_pending_fact_proposals<A: FactStore>(
    memory: &MemoryApplication<A>,
) -> PendingReviewCount {
    match memory.count_pending_fact_proposals().await {
...
pub async fn count_pending_automation_output<A: FactStore>(
    memory: &MemoryApplication<A>,
    profile_root: &Path,
...
pub async fn maybe_automation_staged_notice<A: FactStore>(
    memory: &MemoryApplication<A>,
    dashboard_root: &Path,
    profile_root: &Path,
) -> Option<String> {
