...
//!
//! The projection file is strictly post-commit display metadata; proposal
//! state, CAS, and applied facts always come from [`MemoryApplication`].
...
const MAX_PROPOSAL_PAGE_SIZE: usize = 1_000;
...
pub struct FactProposalRecord {
    ...
    /// empty rather than manufacturing a synthetic write outcome.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub async fn list_fact_proposals<A: FactCompatibilityStore>(
    ...
    let limit = limit.min(MAX_PROPOSAL_PAGE_SIZE);
    let page = memory
