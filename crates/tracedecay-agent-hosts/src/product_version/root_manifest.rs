...
/// Workspace table whose literal `version` is the TraceDecay product version.
pub const PRODUCT_VERSION_TABLE: &str = "[workspace.package]";
...
/// workspace product version rebuilds this crate instead of leaving a stale version baked
...
/// The product version, read from the workspace-root manifest below
/// `repo_root`.
///
/// `None` when the manifest is missing or unreadable, or when
...
pub fn resolve(repo_root: &std::path::Path) -> Option<String> {
    ...
    Some(workspace_package_field(&manifest, "version")?.to_string())
}

/// The only literal string assigned to `key` inside `[workspace.package]`.
///
...
pub fn workspace_package_field<'a>(manifest: &'a str, key: &str) -> Option<&'a str> {
    ...
    for line in manifest.lines() {
        ...
        if line.starts_with('[') {
            in_workspace_package = line == PRODUCT_VERSION_TABLE;
            continue;
        }
        ...
        if value.replace(literal).is_some() {
            return None;
        }
    }
    value
...
";
    ...
    #[test]
    fn the_workspace_package_table_wins_over_every_other_table() {
        assert_eq!(workspace_package_field(MANIFEST, "version"), Some("9.9.9"));
    }

    #[test]
    fn an_inherited_version_is_not_a_literal_this_can_stamp() {
        let manifest = "[workspace.package]\nversion.workspace = true\n";
        assert_eq!(workspace_package_field(manifest, "version"), None);
    }

    #[test]
    fn a_key_that_is_only_a_prefix_is_not_the_field() {
        let manifest = "[workspace.package]\nversions = \"nope\"\nversion = \"1.2.3\"\n";
        assert_eq!(workspace_package_field(manifest, "version"), Some("1.2.3"));
    }

    #[test]
    fn a_manifest_without_a_workspace_package_table_has_no_product_version() {
        assert_eq!(
            workspace_package_field("[workspace]\nversion = \"1\"\n", "version"),
            None
        );
    }
    ...
    /// A crate directory is not the workspace root; resolving against a
    /// package-only manifest must report nothing rather than that package's version.
    #[test]
    fn a_package_manifest_without_workspace_authority_is_refused() {
        let dir = tempfile::tempdir().expect("temp dir");
        std::fs::write(
    ...
    #[test]
    fn the_workspace_product_version_round_trips_through_a_file() {
        let dir = tempfile::tempdir().expect("temp dir");
