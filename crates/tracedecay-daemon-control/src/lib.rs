...
pub use service::{
    ...
    MaintenanceWindowOutcome, QuiescedDaemonLifecycle, daemon_reachable, daemon_socket_connectable,
    default_socket_path, install_service, install_service_under_lease,
    installed_service_process_proof, installed_service_socket_path, installed_service_state,
    prepare_scoop_package_service, quiesce_installed_service_before_lease,
...
pub fn client_identity(profile: &ProfileRoot) -> DaemonClientIdentity {
    DaemonClientIdentity::new(profile.data_dir().to_path_buf(), profile.global_db_path())
}

/// Handshake for `profile`'s client identity and caller-supplied binary
/// version.
pub fn handshake_for_current_client(
    ...
    Ok(DaemonHandshake {
        ...
        client_identity: client_identity(profile),
