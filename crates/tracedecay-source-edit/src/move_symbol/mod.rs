...
pub(crate) async fn move_symbol(
    ...
    let dest_rel = resolve_dest_rel(project_root, dest_file)?;
    ...
    validate_write_containment(project_root, &source_abs, "source")?;
    validate_write_containment(project_root, &dest_abs, "destination")?;
    ...
    // (a hard E0753).
    while start < end_inclusive && classify_leading_line(src_lines[start]) == LeadingKind::InnerDoc
    {
        start += 1;
    }
        ...
        .find(|candidate| candidate.name == target.name && is_importable_item(&candidate.kind))
    {
        let hint = MoveHint {
            ...
            suggestion: Some("rename one of them or choose a different destination".to_string()),
        };
        return Ok(fail(
    ...
    if let Some(hint) = module_missing_hint(project_root, &dest_rel).await {
        impact.push(hint);
    }
...
fn resolve_dest_rel(project_root: &Path, dest_file: &str) -> Result<String> {
...
async fn analyze_dependencies(
    ...
    let source_code_only = masked_rust_source_with(source_modified, MaskOptions::UNUSED_IMPORTS);
...
async fn module_missing_hint(project_root: &Path, dest_rel: &str) -> Option<MoveHint> {
    ...
    let parent = declaring_parent_file(project_root, dest_rel);
    Some(MoveHint {
...
fn declaring_parent_file(project_root: &Path, dest_rel: &str) -> String {
...
fn module_declared(project_root: &Path, dest_rel: &str, stem: &str) -> bool {
    for candidate in parent_module_candidates(dest_rel) {
        if let Ok(text) = std::fs::read_to_string(project_root.join(&candidate))
