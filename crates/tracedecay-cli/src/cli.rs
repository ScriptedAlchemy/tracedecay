...
generation, validates packaged native qualification evidence for the named \
profile, publishes the accepted evaluation, and compare-and-swaps the result \
into `active_profile` of the project's `semantic.runtime.v1` configuration. \
The daemon composes the model material and configuration revision itself; \
strict-semantic search reports `unavailable` until a profile is active.";
...
Failure states are typed: model acquisition names the blocking stage,
qualification distinguishes stale workload, failed qualification, and missing
evidence, and a lost configuration compare-and-swap reports a retryable
conflict. Inspect model, qualification, and activation separately with
`tracedecay tool runtime`.
