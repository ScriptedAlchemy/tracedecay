...
pub async fn similarity_computation(
    ...
    let rows = vector_rows(
        application
            .dashboard_vector_points(None, vector_cap, read_control)
            .await
            .map_err(|error| error.to_string())?,
    )?;
    ...
    let computed = tokio::task::spawn_blocking(
        move || -> Result<SimilarityComputation, MemoryAnalysisError> {
            ...
            build_similarity_computation(key, dim, facts, scored, &blocking_control)
        },
    )
    .await
...
pub async fn similarity_payload(
    ...
    let computation = match similarity_computation(state, read_control).await {
