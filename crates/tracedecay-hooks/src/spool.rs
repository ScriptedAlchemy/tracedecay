...
fn replayable_sessions(pending: &[HookSpoolRecordV1], now: UtcMicros) -> BTreeSet<[u8; 32]> {
    pending
        .iter()
        .filter(|record| !is_expired(record, now))
...
fn round_robin_after(sessions: &BTreeSet<[u8; 32]>, after: Option<[u8; 32]>) -> Vec<[u8; 32]> {
...
fn is_expired(record: &HookSpoolRecordV1, now: UtcMicros) -> bool {
