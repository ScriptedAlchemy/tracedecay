...
fn continuation_schema_and_runtime_reject_page_zero() {
    let schema =
        serde_json::to_value(schema_for!(tracedecay_contracts::MultiRootContinuationV1)).unwrap();
    ...
    assert!(serde_json::from_value::<tracedecay_contracts::MultiRootContinuationV1>(wire).is_err());
}
