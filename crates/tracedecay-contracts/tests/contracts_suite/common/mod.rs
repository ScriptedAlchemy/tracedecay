...
impl WorkGraphReadPortV1 for WorkProductAttemptStore {
    fn read_graph(
        ...
        .map_err(|_| WorkGraphReadPortErrorV1::Unavailable)?;
        match request.mode {
            tracedecay_contracts::WorkGraphReadModeV1::Current => Ok(WorkGraphReadV1::Current {
...
impl WorkProductAttemptAdmissionPortV1 for WorkProductAttemptStore {
    ...
    fn admit_retry(
        &self,
        _admission: &tracedecay_contracts::WorkProductRetryAdmissionV1,
    ) -> Result<
        (
    ...
    fn admit_synthesis(
        &self,
        admission: &tracedecay_contracts::WorkProductSynthesisAdmissionV1,
    ) -> Result<
        (
            WorkProductEventCommitV1,
            tracedecay_contracts::WorkSynthesisInsertOutcome,
        ),
        WorkProductAttemptAdmissionErrorV1,
    > {
        admission.validate()?;
