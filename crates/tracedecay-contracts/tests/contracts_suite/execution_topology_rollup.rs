...
impl ExecutionTopologyRollupQueryPort for StaticRollupPort {
    fn query_rollup_fragments<'a>(
        &'a self,
        _query: tracedecay_contracts::ExecutionTopologyRollupFragmentQueryV1,
...
) -> &'a tracedecay_contracts::ExecutionTopologyMeasurementV1 {
    model
        .measurements
        .iter()
        .find(|measurement| {
...
fn arbitrary_partial_boundaries_merge_with_full_day_interior_without_changing_projection() {
    ...
    let peak = find(
        ...
        &[
            tracedecay_contracts::ExecutionTopologyDimensionV1::FanoutPhase(
                tracedecay_contracts::ExecutionFanoutPhaseV1::PeakActive,
            ),
            ExecutionTopologyDimensionV1::WidthBucket(
                tracedecay_contracts::ExecutionWidthBucketV1::From3To4,
            ),
        ],
    );
    ...
    let duplicate_wall_micros = find(
        ...
        &[
            ExecutionTopologyDimensionV1::DuplicateKind(
                tracedecay_contracts::ExecutionDuplicateKindV1::ExactDuplicate,
            ),
            ExecutionTopologyDimensionV1::Unit(
                tracedecay_contracts::ExecutionQuantityUnitV1::WallMicros,
            ),
        ],
    );
...
fn canonical_serde_roundtrip_and_bad_missing_interiors_fail_closed() {
    ...
    let decoded: tracedecay_contracts::ExecutionTopologyRollupFragmentV1 =
    ...
    assert!(
        serde_json::from_str::<tracedecay_contracts::ExecutionTopologyRollupFragmentV1>(&malformed)
            .is_err()
    );
