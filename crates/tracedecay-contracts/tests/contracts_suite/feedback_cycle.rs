...
impl FeedbackDiagnosticsPort for DiagnosticsFixture {
    ...
    ) -> tracedecay_contracts::feedback::FeedbackPortFuture<
        'a,
    ...
    ) -> tracedecay_contracts::feedback::FeedbackPortFuture<'a, Vec<FeedbackDiagnosticBaselineV1>>
    {
...
impl FeedbackImpactPort for ImpactFixture {
    ...
    ) -> tracedecay_contracts::feedback::FeedbackPortFuture<'a, FeedbackImpactPortOutcome> {
        self.calls.set(self.calls.get() + 1);
...
impl FeedbackCycleDedupePort for DedupeFixture {
    ...
    ) -> tracedecay_contracts::feedback::FeedbackPortFuture<'a, FeedbackCycleDedupeState> {
        let state = self.0;
...
fn partial_provider_coverage_is_not_promoted_by_analyzer_admission() {
    let input = saved_input();
    let mut provider = provider_identity(&input);
    provider.coverage.completeness = tracedecay_contracts::CoverageCompleteness::Partial;
