...
use tracedecay_contracts::{
    CancellationContext, VerifiedWorkGraphVersionV1, WorkGraphVersionEntryV1,
    WorkflowFailurePolicy, WorkflowFanOutCensusEvidenceV1, WorkflowFanOutRequest,
    WorkflowNonDuplicateAttemptsEvidenceV1, WorkflowProviderAdmission,
    derive_workflow_fan_out_census, durable_workflow_fan_out_plan, prepare_workflow_fan_out,
};
...
use tracedecay_domain::{
    ...
    WorkProductSourceWatermarkV1, WorkProjectionSequenceV1, WorkProposalV1, WorkProviderBackendV1,
    WorkProviderProtocol, WorkProviderRouteId, WorkProviderRouteV1, WorkRouteDecisionV1,
    WorkRuntimeProjectionCoverageV1, WorkRuntimeProjectionV1, WorkSandboxPolicy, WorkScoreKindV1,
    WorkShapeAssessmentV1, WorkSizingV1, WorkTerminalEvidenceV1, WorkflowCensusCountV1,
    WorkflowCensusEvidenceReasonV1, WorkflowCensusGenerationV1, WorkflowDefinition, WorkflowFanOut,
...
struct Fixture {
    ...
    snapshot_accepted_only: WorkGraphVersionEntryV1,
...
fn fixture() -> Fixture {
    ...
    let snapshot = work_product_snapshot(&durable.children, true);
    let snapshot_accepted_only = work_product_snapshot(&durable.children, false);
...
fn work_product_snapshot(
    children: &[tracedecay_domain::WorkflowFanOutChildPlanV1],
    execution_admitted: bool,
) -> WorkGraphVersionEntryV1 {
    let mut graph = WorkProductGraphV1::new(
        WorkGraphVersionV1::initial(),
        children
            .iter()
            .map(|child| child.initiative.clone())
            .collect(),
        children.iter().map(|child| child.plan.clone()).collect(),
        children
            .iter()
            .map(|child| child.milestone.clone())
            .collect(),
        children.iter().map(|child| child.item.clone()).collect(),
    )
    .unwrap();
    ...
    let projected_at = UtcMicros(100);
    let runtime = WorkRuntimeProjectionV1::new(
        graph.version(),
        ProjectionGenerationId::new("generation.workflow.census.runtime").unwrap(),
...
fn census_evidence<'a>(
    fixture: &'a Fixture,
    snapshot: Option<&'a WorkGraphVersionEntryV1>,
    ...
    WorkflowFanOutCensusEvidenceV1 {
        ...
        non_duplicate_attempts: non_duplicates.map_or(
            WorkflowNonDuplicateAttemptsEvidenceV1::Unavailable(
                WorkflowCensusEvidenceReasonV1::DuplicateAdjudicationUnavailable,
            ),
            |attempts| WorkflowNonDuplicateAttemptsEvidenceV1::Complete(attempts.clone()),
        ),
...
#[test]
fn unavailable_work_topology_remains_typed_in_the_census() {
    let fixture = fixture();
    ...
    let previous = derive_workflow_fan_out_census(&fixture.projection, &first).unwrap();
    let evidence = WorkflowFanOutCensusEvidenceV1 {
        work_snapshot: Some(&fixture.snapshot),
        attempts: &fixture.attempts,
        attempt_reads_complete: true,
        shared_authority_waits: Some(&fixture.shared_waits),
        non_duplicate_attempts: WorkflowNonDuplicateAttemptsEvidenceV1::Unavailable(
            WorkflowCensusEvidenceReasonV1::WorkTopologyUnavailable,
        ),
        runnable_children: Some(&fixture.runnable),
        blocked_children: Some(&fixture.blocked),
    ...
    assert_eq!(
        ...
        WorkflowCensusCountV1::Unavailable {
            reason: WorkflowCensusEvidenceReasonV1::WorkTopologyUnavailable,
        }
    );
}
