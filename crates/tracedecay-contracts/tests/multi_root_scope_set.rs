...
fn federated_roots_refuse_a_foreign_profile_store_locator() {
    ...
    assert!(
        matches!(foreign, tracedecay_contracts::AuthorizedScopeSetError::Invalid(message)
...
fn scope_digests_separate_references_and_refuse_a_duplicate_exact_root() {
    ...
    assert!(
        matches!(
            duplicate,
            tracedecay_contracts::AuthorizedScopeSetError::DuplicateRoot
        ),
    ...
    assert!(matches!(
        empty,
        tracedecay_contracts::AuthorizedScopeSetError::Empty
    ));
}

#[test]
fn registered_roots_share_exact_brain_profile_and_store_identity() {
