...
use tracedecay_tool_catalog::{
    ...
    ExecutableSchemaAuthority, LifecycleClass, PaginationContract, PrivacyClass, ProfileId,
    ...
    macro_rules! add {
        ($operation:literal, $request:ty, crate::configuration::$result:ident) => {
            ...
            >(
                ...
                $operation,
                concat!(
                    "tracedecay_contracts::context_scout::",
                    stringify!($request)
                ),
                concat!("tracedecay_contracts::configuration::", stringify!($result)),
            )?)
        };
        ($operation:literal, $request:ty, $result:ty) => {
            schemas.push(context_scout_executable_schema::<$request, $result>(
                ...
                concat!("tracedecay_contracts::context_scout::", stringify!($result)),
            )?)
        };
    }
    add!(
...
#[cfg(test)]
mod tests {
    use tracedecay_tool_catalog::{IdempotencyContract, ReceiptContract, ReconciliationContract};
