//! Canonical public Git request wire contracts.
//!
...
use tracedecay_domain::{
    GitBlameV1, GitDiffScopeV1, GitDiffV1, GitHeadStateV1, GitHistoryV1, GitIndexCommitIntentV1,
    GitIndexPreviewId, GitIndexPreviewV1, GitIndexTransactionOperationV1, GitOidV1,
    GitOperationStateV1, HunkRefV1, ManifestDigest, RepositoryId, RepositoryStateSnapshotV1,
};
...
/// Normalized Git query selected by one public read surface.
///
/// Native query execution consumes this transport-neutral representation;
/// each public request above owns the wire shape that creates it.
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
...
impl GitReadRequestV1 {
    ...
    pub const fn use_case_id(&self) -> &'static str {
        match self {
...
pub struct GitQueryEnvelopeV1<T> {
    pub value: T,
    pub coverage: tracedecay_domain::GitCoverageV1,
...
/// Normalized bounds and query selected from a public Git read request.
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
