...
#[test]
fn semantic_lifecycle_controls_are_cataloged_on_the_configuration_surface() {
    let contribution = configuration_surface_catalog_contribution().expect("contribution");
    for operation in [
        ...
        let application_operation = configuration_surface_operation(operation)
            .expect("valid configuration operation name")
            .expect("semantic lifecycle operation must be cataloged");
        let manifest = contribution
            .capabilities()
            .iter()
            .find(|manifest| manifest.capability_id() == application_operation.capability_id())
            .expect("semantic lifecycle capability");
        assert_eq!(
            manifest.cancellation(),
            &CancellationContract::NotCancellable,
            "short serialized lifecycle controls must not overclaim in-flight cancellation"
        );
        assert!(
            !manifest
                .terminal_states()
    ...
    for operation in [
        ...
        assert!(
            manifest
                .cancellation()
                .observes(CancellationPoint::BeforeEffect)
        );
        assert!(
            manifest
                .cancellation()
