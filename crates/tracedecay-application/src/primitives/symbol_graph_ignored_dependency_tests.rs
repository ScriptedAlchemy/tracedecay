...
impl SymbolGraphCursorPort for FixtureCursor {
    ...
    fn finish_page<'a>(
        ...
        _has_more: bool,
        _observed_at: UtcMicros,
    ) -> SymbolGraphCursorFuture<'a, Option<tracedecay_contracts::OpaqueCursor>> {
...
async fn symbol_search_never_schedules_without_opt_in_or_after_a_positive_match() {
    ...
    let matched = adapter
        ...
        .await;
    assert_completed_names(matched, &["Widget", "WidgetFactory"]);
    assert!(scheduler.calls().is_empty());
}
...
fn assert_completed_names(
    outcome: SymbolGraphPortOutcome<tracedecay_contracts::retrieval::SymbolPrimitiveRecord>,
    expected: &[&str],
) {
