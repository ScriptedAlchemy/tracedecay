...
//! in the extracted crates, starting with
//! `tracedecay_sessions::host_ports`. Each slot has a conservative default so
//! an unwired process still runs — it just does less (no LCM redaction, no
//! session review, no standalone Codex admission).
//!
//! Only the composition root can fill them, and it must do so before any
//! transcript ingest runs. That is what
...
/// transcript ingest, which fails quietly when the root never registered.
