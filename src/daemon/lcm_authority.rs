...
use receipt::{
    terminal, terminal_failure, terminal_interruption, unavailable, unavailable_with_state,
};
...
impl DaemonLcmAuthority {
    ...
    async fn execute_compaction(
        ...
        let preflight = run_application_request_interruptible(
            context,
            cancellation,
            store.preflight(command.preflight),
            || {},
        )
        .await;
        match preflight {
            Ok(Ok(response)) => {
                let Ok(state) =
                    canonical_sha256(&(command.evidence.protocol().event_digest(), &response))
                else {
                    return terminal_failure(
                        context,
                        LcmAuthorityOperation::Compact,
                        started_at,
                        "LCM pressure receipt could not be encoded",
                    );
                };
