...
fn resolve_authority(
    ...
    let authorities = registered_context_scout_lifecycle_authorities()
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
    let Some(authority) = authorities.get(&(hook_project_id, hook_worktree_id)) else {
        tracing::debug!(
    ...
    let Some(sessions) = authority.sessions.upgrade() else {
        tracing::debug!(
            target: "tracedecay::context_scout_lifecycle",
            reason = "authority_session_store_dropped",
            "Context Scout lifecycle resolution failed closed"
        );
        return None;
    };
