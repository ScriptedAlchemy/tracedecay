...
impl DaemonAdvisoryRuntimeRegistrar {
    pub(crate) async fn register_deferred_hook_orchestrator(
        ...
        project_id: [u8; 16],
        worktree_id: [u8; 16],
    ) -> Result<Arc<DeferredAdvisoryHookOrchestratorV1>, DaemonAdvisoryRuntimeRegistrationError>
    {
        if project_id == [0; 16]
            || worktree_id == [0; 16]
            || !self
                .service
                .project_runtimes
                .holds::<Arc<Pr12FeedbackCycleRuntime>>(&project_root)
                .await
        {
            return Err(DaemonAdvisoryRuntimeRegistrationError::MissingFeedbackRuntime);
        }
        let candidate = DeferredAdvisoryHookOrchestratorV1::new(now_micros());
        let component = RegisteredAdvisoryHookOrchestrationRuntimeV1::new(
            project_id,
            worktree_id,
            Arc::clone(&candidate),
        );
        ...
        let runtime = if inserted {
            ...
            self.service
                .project_runtimes
                .read::<RegisteredAdvisoryHookOrchestrationRuntimeV1, _, _>(
                    &project_root,
                    |incumbent| {
                        incumbent
        ...
        let runtime_port: Arc<dyn AdvisoryHookOrchestrationPortV1> = runtime.clone();
        let runtime_weak: Weak<dyn AdvisoryHookOrchestrationPortV1> = Arc::downgrade(&runtime_port);
        let registered = match advisory_hook_orchestration_registry().lock() {
            Ok(mut registry) => {
        ...
        if registered {
            ...
            if inserted {
                self.service
                    .project_runtimes
                    .withdraw::<RegisteredAdvisoryHookOrchestrationRuntimeV1>(&project_root)
                    .await;
            }
