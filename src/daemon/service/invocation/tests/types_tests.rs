...
async fn advisory_hook_orchestration_runs_feedback_work_without_scout_lifecycle() {
    ...
    let runtime = BoundedAdvisoryHookOrchestratorV1::new(1, move |_, _| {
...
#[tokio::test]
async fn advisory_hook_orchestration_supersedes_same_address_without_replay_delay() {
    struct WorkGuard {
    ...
    let runtime = BoundedAdvisoryHookOrchestratorV1::new(1, move |request, cancellation| {
    ...
    let mut first = AdvisoryHookOrchestrationRequestV1::from_envelope(
        hook_envelope(HookEventV2::SavedEdit {
            file_id: [7; 16],
            changed_range_count: 1,
        }),
    ...
    let mut second = AdvisoryHookOrchestrationRequestV1::from_envelope(
        second_envelope,
        &hook_binding(),
        Some(hook_lifecycle()),
        1,
...
async fn superseded_blocking_work_is_joined_before_its_receipt_terminal() {
    ...
    let runtime = BoundedAdvisoryHookOrchestratorV1::new(1, move |request, _cancellation| {
    ...
    let successor = AdvisoryHookOrchestrationRequestV1::from_envelope(
        successor_envelope,
        &hook_binding(),
        Some(hook_lifecycle()),
        1,
...
async fn dropping_advisory_hook_orchestrator_cancels_daemon_owned_work_without_false_completion() {
    ...
    let runtime = BoundedAdvisoryHookOrchestratorV1::new(1, move |_, _| PendingWork {
...
async fn deferred_advisory_owner_reports_stable_warming_ready_and_unavailable_states() {
    ...
    let ready = BoundedAdvisoryHookOrchestratorV1::new(1, move |_, _| {
