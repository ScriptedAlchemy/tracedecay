...
//! in its public surface and reaches root-owned `daemon::{authority,
...
///
/// The schema lives in `tracedecay-global-db`, above this root composition
/// boundary, so the kernel reaches it through
/// `tracedecay_runtime_core::ports::registered_schema`. The
