...
//! in its public surface, while `tracedecay-global-db` depends on the kernel —
//! so the kernel taking that edge is a Cargo cycle. It also reaches `daemon::{authority,
...
/// kernel, so the kernel reaches it through the registered-schema port. The
