...
use tracedecay_rusqlite_runtime::remote_authority::{
    RegisteredRemoteEnrollmentAuthorityV1, RegisteredRemoteQueryPolicyAuthorityV1,
    RegisteredRemoteReplayPolicyAuthorityV1, RusqliteRemoteAuthorityStoreV1,
    RusqliteRemoteExactObservationQueryPortV1,
};
...
impl CanonicalDaemonRemoteProtocolOwnersV1 {
    ...
    pub(crate) fn new_with_registered_enrollment_replay_and_query(
        ...
        let replay_policy = Arc::new(RegisteredRemoteReplayPolicyAuthorityV1::from_registered(
            enrollment_store.clone(),
        )?);
        let query_policy = Arc::new(RegisteredRemoteQueryPolicyAuthorityV1::from_registered(
            enrollment_store,
        )?);
        let replay = Arc::new(RemoteReplayProtocolAdapterV1::new(
            RemoteReplayServiceV1::new(
...
pub enum DaemonRemoteRegisteredOwnerErrorV1 {
    ...
    #[error(transparent)]
    Query(#[from] tracedecay_application::remote::query::RemoteExactObservationQueryErrorV1),
}
