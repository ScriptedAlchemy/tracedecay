...
fn doctor_graph_schema_state(actual: i64) -> DoctorGraphSchemaState {
    match actual {
        ...
        actual if actual == i64::from(tracedecay_runtime_core::db::migrations::LATEST_VERSION) => {
...
async fn doctor_runtime_value_inner(
    ...
    let expected_schema_version = tracedecay_runtime_core::db::migrations::LATEST_VERSION;
    let mut value = json!({
        "tracedecay_version": crate::version::build_version(),
        "process": {
            "pid": std::process::id(),
