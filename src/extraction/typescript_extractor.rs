...
impl TypeScriptExtractor {
        ...
        match node.kind() {
            ...
            "expression_statement" => {
                ...
                if let Some(internal) = find_direct_child_by_kind(node, "internal_module") {
                    ...
                    if Self::is_test_framework_call(state, call) {
                        Self::visit_test_call(state, call);
                    }
                }
            }
            _ => {
    ...
    const TEST_CALLEES: &'static [&'static str] = &[
        "describe",
        "it",
        "test",
        "suite",
    ...
    fn is_test_framework_call(state: &ExtractionState, call: TsNode<'_>) -> bool {
        Self::test_call_root_callee(state, call)
            .is_some_and(|root| Self::TEST_CALLEES.contains(&root.as_str()))
    }
            ...
            match child.kind() {
                "string" => {
                    ...
                    let title = text
                        ...
                        .trim_matches('"')
                        .trim_matches('`');
                    return Some(Self::truncate_title(title));
                }
                "template_string" => {
                    let text = state.node_text(child);
    ...
    fn visit_test_call(state: &mut ExtractionState, call: TsNode<'_>) {
        // The arguments node holds the title and callback.
        let Some(args) = find_direct_child_by_kind(call, "arguments") else {
            return;
        };

        let root = Self::test_call_root_callee(state, call);
        let title = Self::test_call_title(state, args)
            .or(root)
        ...
        let callback = Self::test_call_callback(args);
        let is_async = callback.is_some_and(|cb| Self::has_child_kind(cb, "async"));
        let metrics = callback
            .map(|cb| count_complexity(cb, &TYPESCRIPT_COMPLEXITY, &state.source))
            .unwrap_or_default();

        let call_text = state.node_text(call);
        let signature = Self::truncate_title(call_text.lines().next().unwrap_or_default());
        ...
        if let Some(body) = find_direct_child_by_kind(callback, "statement_block") {
            Self::visit_test_body(state, body, &id);
        } else {
            // Expression-bodied callback: `it('x', () => foo())`.
            Self::extract_call_sites(state, callback, &id);
        }
        state.node_stack.pop();
    }
    ...
    fn visit_test_body(state: &mut ExtractionState, body: TsNode<'_>, test_id: &str) {
        ...
        loop {
            ...
            if !handled {
                ...
                if !Self::defines_own_callable(stmt) {
                    Self::extract_call_sites(state, stmt, test_id);
                }
            }
            if !cursor.goto_next_sibling() {
                break;
