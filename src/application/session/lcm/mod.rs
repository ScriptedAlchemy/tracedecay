...
//! `render` holds the truncation and typed-omission shaping applied after
//! canonical hydration. The session LCM engine re-exports these under
//! `sessions::lcm`, and the registered temporal adapters depend on them
//! directly, so neither side has to reach through the other.
