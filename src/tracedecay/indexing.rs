...
impl TraceDecay {
    ...
    pub async fn graph_rebuild_status(&self) -> Result<GraphRebuildStatusV1> {
        let Some(encoded) = self.db.get_metadata(GRAPH_REBUILD_STATE_KEY).await? else {
            let generation_is_current = self
                ...
                == Some(crate::db::migrations::GRAPH_GENERATION_SCHEMA_VERSION);
            if !generation_is_current {
                return Ok(GraphRebuildStatusV1::Indexing {
                    schema_version: crate::db::migrations::GRAPH_GENERATION_SCHEMA_VERSION,
                    completed_files: 0,
                    total_files: None,
    ...
    fn load_graph_rebuild_checkpoints(
        ...
        for path in paths {
            ...
            if batch.schema_version != crate::db::migrations::GRAPH_GENERATION_SCHEMA_VERSION {
                continue;
            }
        ...
        V: Fn(&str),
    {
        ...
        self.db
            .set_metadata_unguarded(
                ...
                &crate::db::migrations::GRAPH_GENERATION_SCHEMA_VERSION.to_string(),
            )
            .await?;
        if rebuild_availability.is_some() {
