//! Corruption detection and recovery around a registered project-store open.
...
/// constructing `Self`. `Recovered` carries a fail-closed corruption
/// result.
...
impl TraceDecay {
    ...
    /// abandoned dirty marker, repairs FTS-only damage, and validates the
    ...
    pub(super) async fn run_open_health_recovery(
                ...
                Ok(verification) => {
                    ...
                    match integrity {
                        ...
                        Err(error) => {
                            drop(recovery_lock);
                            return Ok(OpenHealthOutcome::Recovered(Box::new(
                                Self::fail_corrupt_store(db_path, error),
                            )));
                        }
                    }
                }
            ...
            .await
            {
                Ok(database) => match database.repair_fts_after_open().await {
                    ...
                    Err(repair_error) => {
                        ...
                        drop(recovery_lock);
                        return Ok(OpenHealthOutcome::Recovered(Box::new(
                            Self::fail_corrupt_store(db_path, repair_error),
                        )));
                    }
                },
                Err(repair_error) => {
        ...
        let db = match open_result {
            ...
            Err(e) if Database::is_corruption_error(&e) || crashed => {
                drop(recovery_lock);
                return Ok(OpenHealthOutcome::Recovered(Box::new(
                    Self::fail_corrupt_store(db_path, e),
                )));
            }
            Err(e) => return Err(e),
        };
        crate::db::schema::ensure_schema_current(&db).await?;
    ...
    fn fail_corrupt_store(db_path: &Path, detail: impl std::fmt::Display) -> Result<Self> {
