...
/// Retained startup task handle for the code-index catch-up phase.
#[derive(Default)]
...
/// Owns the startup code-index catch-up state.
///
/// Historical session convergence belongs to the daemon-retained session
/// scheduler and is deliberately absent from this per-MCP-server lifecycle.
...
impl StartupCatchUpMachineV1 {
    ...
    /// The code-index phase is done. Failures settle rather than stranding
    /// waiters.
    fn settle(&self) {
        let mut state = self.state();
        if matches!(*state, StartupCatchUpStateV1::Cancelled) {
    ...
    /// Shutdown abandoned the code-index phase.
...
impl McpServer {
    ...
    /// Shutdown-side teardown of the code-index startup catch-up.
    ...
    /// Returns `true` once the code-index startup catch-up has finished.
    pub fn startup_catch_up_done(&self) -> bool {
        self.startup_catch_up.sync_phase_settled()
    }

    /// Polls until the code-index startup catch-up completes or the timeout
    /// elapses. Historical session convergence is intentionally not part of
