...
pub(super) fn lcm_doctor_mode(args: &Value) -> Result<&str> {
    ...
    match mode {
        "diagnose" | "retention" => Ok(mode),
        _ => Err(argument_error("mode must be one of diagnose, retention")),
    }
}
...
pub(super) fn lcm_error(err: crate::sessions::lcm::LcmError) -> TraceDecayError {
    match err {
        crate::sessions::lcm::LcmError::Cancelled => TraceDecayError::project_route(
            "lcm_effect_cancelled",
            true,
            "LCM mutation was cancelled before commit",
        ),
        crate::sessions::lcm::LcmError::DeadlineExceeded => TraceDecayError::project_route(
            "lcm_effect_deadline_exceeded",
            true,
            "LCM mutation exceeded its daemon deadline before commit",
        ),
