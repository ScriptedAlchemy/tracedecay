...
pub(super) async fn dispatch_graph_tools(
    ...
    match tool_name {
        ...
        "tracedecay_ast_grep_search" => {
            ast_grep_search::handle_ast_grep_search(cg, args, selected_scope_prefix, control).await
        }
...
pub(super) async fn dispatch_analysis_tools(
    ...
    match tool_name {
        ...
        "tracedecay_constructors" => {
            let control = options.dispatch_control.as_ref().ok_or_else(|| {
                TraceDecayError::mcp_tool_dispatch(
                    ...
                    false,
                    "tracedecay_constructors has no admitted lifecycle control",
                )
            })?;
            analysis::handle_constructors(cg, args, scope_prefix, control).await
        }
...
pub(super) async fn dispatch_git_tools(
    ...
    let control = options.dispatch_control.as_ref().ok_or_else(|| {
        TraceDecayError::mcp_tool_dispatch(
            ...
            crate::mcp::server::McpToolDispatchStage::QueueAdmission.as_str(),
            false,
            format!("git tool '{tool_name}' has no admitted lifecycle control"),
        )
    })?;
    match tool_name {
...
pub(super) async fn dispatch_memory_operation(
    ...
    match operation {
        RetainedSurfaceOperation::FactStore => memory::handle_fact_store(cg, args, global_db).await,
        ...
        _ => Err(unknown_tool_error(operation.as_str())),
    }
}
