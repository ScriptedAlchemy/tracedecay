...
pub(super) fn def_lcm_doctor() -> ToolDefinition {
    def(
        ...
        "LCM Doctor",
        "Run read-only LCM diagnostics and retention inspection without repairing, cleaning, garbage-collecting, or opening writable state.",
        json!({
            "type": "object",
            "properties": {
                "provider": {
                    "type": "string",
                    "description": "Specific provider id to inspect. Required; 'all' is not accepted for this lifecycle tool."
                },
                "session_id": {
                    "type": "string",
                    "description": "Optional provider-local session id filter."
                ...
                "mode": {
                    "type": "string",
                    "enum": ["diagnose", "retention"],
                    "description": "diagnose reports read-only health; retention reports read-only retention candidates."
                }
            },
            "required": ["provider"]
        }),
        ...
        json!({
            ...
            "properties": {
                ...
                "cursor": {
                    "type": "string",
                    "minLength": 1,
                    "description": "Authenticated opaque continuation cursor returned by the same authorized root and target. Numeric source offsets are not accepted."
                }
            },
