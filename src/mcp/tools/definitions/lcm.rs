...
pub(super) fn def_lcm_doctor() -> ToolDefinition {
    def(
        ...
        "LCM Doctor",
        "Run bounded, read-only LCM health or retention diagnostics without payload body exposure.",
        json!({
            "type": "object",
            "properties": {
                "provider": {
                ...
                "mode": {
                    ...
                    "description": "diagnose reports health and retention reports retention candidates; both modes are read-only."
                }
            },
            "required": ["provider"]
        }),
...
pub(super) fn def_lcm_compress() -> ToolDefinition {
    def_rw(
        ...
        "LCM Compress",
        "Daemon-owned lifecycle operation that compresses already-ingested canonical session content. Host adapters provide identity and pressure hints, never raw transcript content.",
        json!({
            "type": "object",
            "properties": {
                "provider": {
