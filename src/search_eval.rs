...
use tracedecay_domain::{
    AuthorizationCanaryV1, ContextSpanV1, DecisionRecordDigest, DecisionRecordV1, EvalOutcomeV1,
    EvalPartitionV1, EvalQueryV1, EvaluationContractError, EvaluationFixtureBundleV1,
    EvaluationTaskV1, EvidenceClaimId, EvidenceIndexDigest, EvidenceIndexEntryV1, EvidenceIndexId,
    EvidenceIndexV1, ExactAdmissionOracleV1, FixtureAuthorityV1, FixtureContentDigest,
    FixtureFileDigestV1, FixtureManifestDigest, FixtureManifestV1, HoldoutRevealCapabilityV1,
    HoldoutSealDigest, HoldoutSealV1, LabelSetDigest, LabelSetId, LabelSetV1, QueryFamilyV1,
    QueryWorkloadV1, RelevanceJudgmentV1, RunManifestDigest, RunManifestV1, SavedCandidateSetV1,
    TemporalEventV1, WorkloadDigest,
};
...
pub struct CompareOptions {
    ...
    pub holdout_capability: Option<PathBuf>,
...
pub fn compare(options: &CompareOptions) -> Result<CompareResult, SearchEvalError> {
    ...
    // This check deliberately precedes reading either optional path. The
    // contract-only packet can prove what is missing without touching a
    // capability or any label-bearing artifact.
    let (outcome, rationale, blocked_on) = if run.authority != FixtureAuthorityV1::LockedQuality
        || fixtures.bundle.manifest.authority != FixtureAuthorityV1::LockedQuality
    {
        ...
        let capability_path = options.holdout_capability.as_ref().ok_or_else(|| {
            SearchEvalError::InvalidRun(
                "locked-quality run has no holdout reveal capability".to_string(),
            )
        })?;
        let capability: HoldoutRevealCapabilityV1 = read_json(capability_path)?;
        let _receipt = capability.issue_receipt(
            &run,
            &fixtures.bundle.manifest.holdout_seal,
            &fixtures.bundle.manifest.decision_owners,
        )?;
        return Err(SearchEvalError::InvalidRun(
            "a locked reveal capability was supplied, but no authoritative signed holdout label artifact is available in this fixture packet"
                .to_string(),
        ));
    };

    let saved_candidate_ablations = options
        .saved_candidates
        .as_deref()
    ...
    let evidence_index =
        build_evidence_index(&fixtures, &run, outcome, &blocked_on, report_artifact)?;
...
fn required_holdout_artifact(seal: &HoldoutSealV1) -> RequiredExternalArtifactV1 {
    RequiredExternalArtifactV1 {
        ...
        reason: "an authorized, signed, independently adjudicated locked-quality label artifact and reveal capability are required"
            .to_string(),
    }
}
...
) -> Result<Vec<SavedCandidateAblationSummaryV1>, SearchEvalError> {
...
fn build_evidence_index(
    ...
    let entry = EvidenceIndexEntryV1 {
        ...
        fixture_artifacts: std::iter::once("fixture-manifest-v1.json".to_string())
            ...
            .collect(),
        run_manifest_digest: run.digest.clone(),
        aggregate_artifacts: vec![report_artifact],
