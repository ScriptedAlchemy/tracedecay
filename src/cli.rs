...
pub enum GitAction {
    ...
    /// Mint bounded hunk references for a working-tree or staged diff
    Hunks {
        #[arg(long, value_enum, default_value_t = GitDiffScopeArg::WorkingTree)]
        scope: GitDiffScopeArg,
        /// Immutable daemon-minted preview identity that the hunk references bind to
        #[arg(long)]
        preview_id: String,
        /// Exact repository snapshot digest that the hunk references bind to
        #[arg(long)]
