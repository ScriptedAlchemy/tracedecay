...
pub(crate) async fn read_source(
    ...
    let body = match mode {
        ReadMode::Full => render_full(&crate::sync::read_source_file(&absolute_path).map_err(
            |error| TraceDecayError::Config {
                message: format!("cannot read '{file}': {error}"),
            },
        )?),
        ReadMode::Lines => render_lines(
            &crate::sync::read_source_file(&absolute_path).map_err(|error| {
