...
impl ParentMessageResolver {
    pub(in crate::global_db::session_temporal) fn register(
        &mut self,
        message_id: &str,
        occurrence_id: &str,
    ) {
    ...
    pub(in crate::global_db::session_temporal) fn reject_ambiguity(
        &self,
        operation: &'static str,
    ) -> SessionStoreResult<()> {
    ...
    pub(in crate::global_db::session_temporal) fn resolve(&self, message_id: &str) -> Option<&str> {
        self.occurrences
            .get(message_id)
