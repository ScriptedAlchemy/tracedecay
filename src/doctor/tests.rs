...
fn daemon_startup_pending_runtime_telemetry_is_retryable() {
    ...
    assert!(
        ...
        "an admitted project that has not published telemetry yet must be polled: {error}"
    );
    assert!(matches!(
...
fn daemon_startup_malformed_runtime_telemetry_stays_terminal() {
    ...
    assert!(
        !super::daemon_startup_error_is_retryable(&error),
        "present but malformed telemetry is a contract violation: {error}"
    );
}
...
async fn daemon_startup_health_converges_after_runtime_telemetry_appears() {
    ...
    .await
    .expect("startup health must converge once telemetry is published");

    assert!(attempts.load(std::sync::atomic::Ordering::Relaxed) >= 4);
}
