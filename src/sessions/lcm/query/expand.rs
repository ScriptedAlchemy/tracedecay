...
pub(crate) async fn expand_metadata(
    conn: &(impl QueryExecutor + ?Sized),
    request: LcmExpandRequest,
) -> Result<LcmExpandResponse, LcmError> {
    match request.target {
        LcmExpandTarget::RawMessage { store_id } => {
            let raw = raw::load_raw_message_metadata_by_store_id(conn, store_id).await?;
            if raw.provider != request.provider {
                return Err(LcmError::SummarySourceNotOwnedBySession);
            }
            let from_current_session = raw.session_id == request.session_id;
            ...
            Ok(LcmExpandResponse {
                kind: "raw_message".to_string(),
                content: String::new(),
                content_range: empty_content_range(request.content_slice),
                raw_message: Some(raw),
                summary_node: None,
                summary_sources: Vec::new(),
                payload_ref,
        ...
        LcmExpandTarget::SummaryNode { node_id } => {
            let expansion = dag::expand_summary_node_metadata(
                conn,
                &request.provider,
                &request.session_id,
                &node_id,
            ...
            .await?;
            let (summary_sources, source_pagination) = paginate_summary_sources(
                expansion.sources,
                request.source_offset,
                request.source_limit,
            );
        ...
        LcmExpandTarget::ExternalPayload { payload_ref } => {
            let payload = payload::load_expand_payload_metadata(
                conn,
                &request.provider,
                &request.session_id,
                &payload_ref,
            ...
            Ok(LcmExpandResponse {
                ...
                summary_node: None,
                summary_sources: Vec::new(),
                payload_ref: Some(payload.payload_ref),
                from_current_session: None,
                externalized_note: None,
                source_pagination: None,
            })
...
fn empty_content_range(slice: Option<LcmContentSlice>) -> LcmContentRange {
    let slice = slice.unwrap_or(LcmContentSlice {
        offset: 0,
        limit: usize::MAX,
    });
    LcmContentRange {
