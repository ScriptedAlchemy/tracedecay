...
This guide captures the C/C++ pilot migration strategy for consolidating duplicated tree-sitter traversal helpers under `crates/tracedecay-code-extraction/src/traversal.rs`.
...
The C/C++ pilot is the template for this shape: import helpers from `crate::traversal`, replace `Self::find_*_by_kind(...)` calls with the shared functions, remove the now-unused local helper definitions, and keep extractor behavior unchanged.
