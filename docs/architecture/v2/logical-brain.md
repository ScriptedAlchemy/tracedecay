...
The workflow split is explicit: `crates/tracedecay-workflow` owns pure IR, validation, replay, and taskgraph-candidate compilation, while root-private `src/v2/workflow_compiler_engine` alone owns source discovery/import, JavaScript normalization, TypeScript transpilation, source maps/callsites, and the selected bounded engine/placement adapter. The adapter implements the workflow-owned port and may depend only on domain, workflow, and application contracts. It cannot own storage, transport, scheduling, execution, effects, policy, provider integration, or network access; the pure kernel cannot import a JavaScript or TypeScript compiler/engine.
