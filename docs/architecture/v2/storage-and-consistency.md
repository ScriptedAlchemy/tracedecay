...
SQLite uses host-local files, WAL, synchronous FULL, one fenced writer authority per mutable shard, daemon-owned construction, snapshot fences, and deterministic shutdown/checkpoint receipts. The "deterministic shutdown" property is **not** satisfied until a never-resolving-task process-exit test passes: the current `runtime.shutdown_timeout(2s)` runtime-drop bound applies only after `run(cli)` returns and cannot bound a stuck outer `shutdown_background_tasks().await` (audit [29](../../plans/tracedecay-v2/29-baseline-delta-audit.md) §8.1; packet [30](../../plans/tracedecay-v2/30-baseline-refresh-candidate-packet.md) §7.4; FM-163). Remote sharing exchanges authenticated semantic snapshots/tails through application/API contracts; network-mounted SQLite, implicit multi-primary writes, and remote database URLs are forbidden.
