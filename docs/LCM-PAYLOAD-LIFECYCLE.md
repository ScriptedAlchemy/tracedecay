...
| Trigger | Behavior |
|---|---|
| **Memory fact hard-delete** | No LCM effect. Memory facts live in `memory_facts` and never cite LCM payloads (`crates/tracedecay-session-memory/src/fact_store/`, `crates/tracedecay-dashboard-api/src/memory_api.rs`). Independent subsystem. |
