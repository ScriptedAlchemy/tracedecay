...
> Pure source-only/internal routes change in place. Potentially retained
> branch-era routes remain compatible until an authorized
> installed-client/host/live-profile census proves absence.
...
Move one route family at a time. Released routes and branch-era routes
potentially retained by dogfood clients, saved navigation, bookmarks, or host
files delegate without owning duplicate logic. Replace only source-only routes;
an authorized installed-client/host/live-profile census gates retirement of
potentially retained routes. Revert the family and regenerated contract together.
