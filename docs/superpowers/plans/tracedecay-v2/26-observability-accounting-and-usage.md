...
- Source of truth is plan 06 §10's `HintOutcomeRecordV1` (defined there, stored per plan 02's hint outcome tables, projected terminal by plan 04's `policy_hint_v1`). This plan owns only the rollup: counts of eligible/emitted/delivered/observed/acted/ignored/corrected/missed/unresolvable per policy version, hint category, horizon bucket, and scope, each with explicit denominators and unresolved-horizon visibility.
