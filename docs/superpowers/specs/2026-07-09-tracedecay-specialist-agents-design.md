...
The canonical Claude-form source lives under `plugin/agents/`; Cursor receives
native adapters from the product plugin. Codex plugin install and update
materialize native user-profile `.codex/agents` TOML files through the existing
owned agent manifest, and uninstall removes only those owned files. No host
receives repo-local agent definitions. Names, descriptions, bodies, and
