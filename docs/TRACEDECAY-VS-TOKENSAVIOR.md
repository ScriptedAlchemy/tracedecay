...
## 1. Where tracedecay is stronger
...
`tracedecay_branch_search` query specific branches without disturbing the
...
| Tool | Purpose |
|---|---|
| `tracedecay_hotspots` | High-churn × high-complexity files |
| `tracedecay_complexity` | Cyclomatic + branch / loop / return / nesting counts |
| `tracedecay_doc_coverage` | % of public items with docstrings, per module |
| `tracedecay_god_class` | Outsized classes (members, fan-in, fan-out) |
| `tracedecay_coupling` | Inter-module afferent / efferent coupling |
...
When `tracedecay serve` is running, every MCP tool call first runs a
...
- The graph **persists** on disk and survives process restarts; token-savior
  ...
  shortcut).
- **Memory profile**: a tracedecay session is ~150 MiB RSS for a 2 700-file
...
tracedecay exposes a hierarchy of edit primitives at three levels of
abstraction:

| Level | Tool | When to use |
|---|---|---|
| Symbol | `tracedecay_replace_symbol` | "rewrite this function's body" |
| Symbol | `tracedecay_insert_at_symbol` | "add a method before/after this one" |
| Anchor | `tracedecay_insert_at` | "insert below this line" |
| Range  | `tracedecay_str_replace`, `tracedecay_multi_str_replace` | exact-string surgery |
| AST    | `tracedecay_ast_grep_rewrite` | structural rewrite via ast-grep pattern |
...
If `tracedecay serve` is launched from a subdirectory of an indexed project,
it (a) walks up the filesystem to find the `.tracedecay/` dir, (b) computes a
...
## 2. Where tracedecay doesn't go (and what we do instead)
...
(`SECRET-*`). tracedecay does not parse `.env*` files. **Workaround**: agents
fall back to `Read` + `Grep` on `.env*` plus reading the relevant
`os.environ.get` / `process.env.X` sites via `tracedecay_search`.
...
keep tracedecay focused on code-graph queries.
...
|---|---:|---:|---:|
| Cold index | 6.21 s | **2.15 s** | **tracedecay 2.9× faster** |
| Warm reindex | 1.58 s | **1.36 s** | tracedecay 1.2× |
| Cache / DB size | **5.2 MiB** | 28.6 MiB | token-savior 5.5× smaller |
| Files indexed | 2 715 | 2 677 | tied |
| Symbols / nodes | 2 740 | **29 019** | tracedecay 10× richer graph |
| `find_symbol` avg | **0.004 ms** | 0.136 ms | token-savior 34× (dict vs SQLite probe) |
| `get_function_source` avg | 0.133 ms | **0.117 ms** | tracedecay 1.1× |
| `get_change_impact` avg | 24.37 ms | **0.57 ms** | **tracedecay 43× faster** |
...
- Fork `bench.py` → `bench_tracedecay.py`. Swap the MCP config to launch
  `tracedecay serve -p <root> --timings`.
...
- **First-attempt, untuned.** token-savior reports 97.9% after multiple
  ...
  tracedecay's 95.8% is the *first run* of an unmodified-rubric
...
- **66% MCP tool usage.** Two-thirds of tool calls were `mcp__tracedecay__*`;
  ...
  where tracedecay has no direct tool. Zero `CANNOT_ANSWER` outcomes.
