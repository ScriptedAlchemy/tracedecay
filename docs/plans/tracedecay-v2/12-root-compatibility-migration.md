...
| PRs #437/#442/#444/#446, releases v0.0.53 through v0.0.56 | Latest accepted release merged as `e8883933` | Publication-only accepted inputs. Record package/tag/catalog/schema digests and checks; do not infer an installed runtime or architectural change from release state. |
...
| Local `fix/session-catchup-integrity` worktree, 2026-07-11 | **Unmerged evidence; not accepted base** | Preserve the measured FM-153/FM-154 regressions and V1 differential fixtures: provider-scoped scan-once catch-up, concurrent-request coalescing, ≤60 s 30-project cold-history target, exact concrete-DB recovery markers, and checkpoint-before-clean behavior. V2 does not port handler-static singleflight, multi-project transcript copies, branch databases, or `.dirty`/`.sync.lock` sidecars; it implements the semantics through capture, projectors, exact shard/generation identities, and the shared operation kernel. Reclassify this row from unmerged only after the execution-slice refresh proves a merge. |
