...
| PR 37C, branch graph/storage and superseded semantic runtime | `src/db/**`, DB-bound `src/graph/**`, `src/branch.rs`, `src/branch/**`, `src/branch_meta.rs`, V1 layout portions of `src/storage.rs`, and every superseded direct runtime/model integration, vector scan/query/storage/index, model cache, and duplicate semantic scheduler path | Code graph generations/query parity; #405 identity receipts; every memory/fact/entity/trust/feedback row migrated; plan-25 PR 33G gives every legacy vector artifact one rebuild/drop/quarantine receipt and proves zero old float import; PR 35C observation and rollback receipts prove the accepted root-private FastEmbed route or an explicitly disabled semantic lane; archive restore proven; source/dependency lint proves no parallel semantic implementation remains. |
...
3. Generate and freeze the package DAG from plan 19's `architecture-boundaries.toml`, `cargo metadata`, and the generated-public-contract edge. Enforce the <=12-package ceiling, whose sole admission above the prior ceiling is the pure `tracedecay-workflow` kernel. Publish `tracedecay-domain`; then the domain-only implementation wave (`tracedecay-store`, `tracedecay-capture`, `tracedecay-projectors`, `tracedecay-code-index`, `tracedecay-query`, `tracedecay-policy`, `tracedecay-tool-catalog`, and `tracedecay-workflow`); then `tracedecay-application`; then the official Rust `tracedecay-client` built from the same frozen public-contract digest and root with its private hook/presentation/API/workflow-compiler-and-engine/remote-Brain-transport modules. Peers may publish concurrently only when the generated DAG proves no dependency edge. The Rust client has no Cargo dependency on domain, application, API implementation, or root; no crates.io artifact exists for a root-only adapter.
...
- [ ] PR 37C deletes every superseded direct inference/model wire, vector scan/query/storage/index, model cache, and duplicate semantic scheduling path only after plan-25 PR 33G migration dispositions, PR 35C observation/rollback, and repository-wide dependency/import lint pass. Its generated deletion manifest names every removed path/symbol/dependency/artifact family, replacement or disabled disposition, callers, retained replay evidence, and proves no old vector bytes or dormant alternate runtime remain.
...
- Checked `architecture-boundaries.toml` plus its generated DAG/owner/release/deletion reports and plan-19 reuse/footprint reports: <=12 Rust packages including root/client and the sole added `tracedecay-workflow` kernel; hook/presentation/API/workflow-compiler-and-engine/remote-Brain-transport remain root-private modules; zero forbidden edge, unowned duplicate infrastructure engine, or package-admission drift.
