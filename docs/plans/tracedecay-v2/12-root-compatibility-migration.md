...
| PR #441, `fix(hermes): route memory and context safely` | Open at planning; remote head `152bfacc`, locally amended follow-ups for user memory and live branch-snapshot corruption | Preserve its session-workspace routing, project-selector propagation, bounded first-turn guidance, context-clone isolation, multi-profile deployment, and WAL/checkpoint snapshot-race evidence as FM-138–FM-146 fixtures. Do not copy adapter-local regex, process-CWD fallback, generalized response-handle dereference, a second user-memory database, live-main-file copy/reflink, or direct client SQLite access into V2. |
...
| Runtime/daemon logs, crash, and telemetry files | daemon/hook log files, `runtime_telemetry.rs` on-disk output, crash reports | version-stamped accounting/observability events plus log-safe archives | Sanitizer-scanned before archive; never imported as canonical activity; secret canaries block publication. Preserve a parseable historical producer version when the source record/header proves it. Pre-contract or ambiguous rows are explicitly `UnknownLegacy`, never assigned the importing/current binary version; migration receipts count known-by-version, unknown, redacted, quarantined, and skipped rows. |
