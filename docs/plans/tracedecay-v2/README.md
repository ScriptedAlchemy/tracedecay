...
or schema needs a V2 shape. TraceDecay persisted state has one exact final
shape. Non-final, partial, unknown, or old TraceDecay store bytes return
`ResetRequired` and are recreated; there is no backward reader, store
migration, store backfill, census, dual write, or compatibility state.
External historical host transcripts remain supported inputs: native adapters
capture or refetch provider records, sanitize them, and write only the final
store shape. Compatibility aliases or negotiated revisions survive only for
independently released public wire/API contracts or installed host clients.
Branch-local shapes, delivery order, historical type names, tests, and future
consumers are not publication evidence. Pure source-only/internal contracts
change to their final shape in place. A `V1` suffix may name an initial final
wire format without requiring a sibling version by itself.
...
- The committed capture workload and checked-in executed benchmark result
  record the production parse/sanitize/commit/project/replay baseline.
- Provider capture extends that path across the supported Claude, Codex, Cursor, Hermes,
...
- Executable native host fixtures and typed hook-telemetry readiness replace
  prose-only provider claims. Commit `05da230e` records the historical Linux
  benchmark measurement.

## Storage and authority

- Before remote delivery, one local daemon is the sole mutable SQLite
  authority; remote delivery preserves exactly one fenced daemon authority per mutable
...
4. Delete legacy or duplicate paths once the production route and recovery
   boundary permit it.
5. Accept the capability with a direct product journey, focused failure/recovery
...
authoritative product journey; they do not create independent queues, crates,
owner maps, or conformance projects.
...
Transport and SDK contracts prove structural, semantic, and lifecycle
...
proves absence. Direct fault, restart, concurrency, cross-platform, recovery,
and deletion tests remain part of the product journey they protect.
