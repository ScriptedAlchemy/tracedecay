...
- exact candidate commit and digest plus a fresh clean `git worktree list`/`git status` observation proving the canonically declared branch/worktree association;
...
- one independent review receipt over that exact candidate, including verdict, reviewer, receipt digest, and evidence anchors;
- the complete named required-test set and one command/exit/candidate/digest receipt per test;
...
- repo-local planning CLI: `python3 .codex/skills/executing-tracedecay-v2-plan/scripts/plan_execution.py --graph <execution-state.json> --root <authoritative-checkout> --canonical-ref <ref> --next-ready` emits bounded Markdown by default; `--format json` emits canonical typed JSON;
