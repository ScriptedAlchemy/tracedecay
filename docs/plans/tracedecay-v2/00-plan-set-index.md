...
The validator requires a live authoritative checkout (`--root`) and intended canonical ref. It resolves that ref, recomputes the current plan-inventory/source-block digest, and runs `git merge-base --is-ancestor` itself. The integration receipt embeds that sealed observation (repository, exact candidate/canonical commits and ref, method, exit/status, and observation digest). Candidate/review/test/integration/steering digests are recomputed over canonical payload bytes; a shape-valid SHA-256 string, asserted `candidate_is_ancestor=true`, or bare `independent=true` is not evidence. Git/source-command failure is `Unknown` and suppresses all packets.
...
Every selected node is rendered as one bounded worker packet carrying exact files (maximum 32), exact acceptance commands (maximum 32), branch/worktree binding, prerequisites, GPT-5.6-Sol reasoning/lifecycle ownership, acting runtime, the independent-review/remediation/successor-review/integration gates, and durable retrieval anchors (maximum 64). Optional Claude participation is at most one explicit read-only adversarial step with bounded acceptance criteria; its output is untrusted until GPT-5.6-Sol verifies the actual evidence. Missing or oversized packet fields reject the export rather than widening scope.
...
The checked fixtures `fixtures/positive-ready.json`, `fixtures/negative-candidate-only.json`, and `fixtures/negative-duplicate-owner.json` are synthetic deterministic contract inputs. Focused tests additionally prove live-checkout source freshness, sealed ancestry (including forged-true rejection), canonical receipt recomputation after tampering, exact test-command binding, distinct review authority, external-error `Unknown`, and the required-steering cases: unobserved required, stale-attempt acknowledgement, duplicate delivery, late pre-CAS abort, post-CAS remediation, and advisory-only non-fencing. They also retain cycle, unresolved prerequisite, retired FM-168, bounded packet, and duplicate JSON-key rejection. They are schema examples only and never completion receipts for live V2 work.
