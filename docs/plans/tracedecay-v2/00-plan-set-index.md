...
| [`13-research-provenance-and-context-anchors.md`](13-research-provenance-and-context-anchors.md) | Research manifest, durable retrieval anchors, subagent context, corpus hashes/cutoff, source recovery, future implementation handoff; [current anchor registry](../../../tests/fixtures/v2/research-anchor-manifest.json). |
...
Implementation does not depend on a generated plan graph, registry, bootstrap export, completion ledger, or next-ready controller. Plans are human context. Git history, the current product code, focused tests, and independent integrated-diff review are the delivery evidence.

### 2.2 Direct product delivery

Start from the PR or item named by the user. If none is named, select the first unfinished product outcome in written plan order by inspecting code and tests. Translate only that outcome into bounded implementation tasks with exact file ownership, dependencies, focused tests, and prohibited unrelated edits. Decompose every item across at least two agents or subagents; parallelize only disjoint files and serialize shared files or dependency edges.

Implement product code before coordination artifacts. Use explicit cancellation and no automatic wall-clock, agent, workflow, or no-progress timeout. Independently review the integrated diff, run focused tests, then the smallest relevant broader gate. A passing integrated checkout and its Git commit are completion evidence. Plan-format ambiguity alone does not block delivery; choose the smallest reasonable product slice and proceed.
...
- Every agent-facing synchronous path has one propagated monotonic deadline covering admission, queue wait, lock wait, IPC, application, query/store, rendering, and provider/tool work. No layer resets or omits the remaining budget, performs repair/reindex/recovery inline, or waits indefinitely. Work that cannot finish inside its registered latency class returns typed partial/unavailable coverage or a durable `OperationRef` with progress/cancel/status; bounded queues, memory/RSS ceilings, cancellation, health-aware admission, and circuit breakers prevent a slow provider or corrupt shard from consuming daemon or host capacity.
...
- [ ] End-to-end current/10x/load/fault benchmarks prove bounded p50/p95/p99 latency, queue/lock wait, memory/RSS, cancellation, and recovery for CLI/MCP/API/hooks, common reads, task execution, and workflows; no synchronous request, agent call, corrupt shard, or recovery loop can wait or grow resources without a declared limit and a typed terminal/progress outcome.
