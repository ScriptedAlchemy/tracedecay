...
Every selected node is rendered as one bounded worker packet carrying exact files (maximum 32), exact acceptance commands and required named tests (maximum 32 each), branch/worktree binding, prerequisites, GPT-5.6-Sol reasoning/lifecycle ownership, acting runtime, the independent-review/remediation/successor-review/integration gates, and durable retrieval anchors (maximum 64). Every scalar string reachable from a worker packet is limited to 2,048 characters, including owner, branch, worktree, lane, optional-review, gate, and source-anchor values. Optional Claude participation is at most one explicit read-only adversarial step with bounded acceptance criteria; its output is untrusted until GPT-5.6-Sol verifies the actual evidence. Missing or oversized packet fields reject the export rather than widening scope.
