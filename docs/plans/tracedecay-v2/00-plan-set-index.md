...
**Status:** navigation and ownership index for the total-rewrite plan. This pull request contains plans plus the sole repo-local read-only execution-helper implementation at [`.codex/skills/executing-tracedecay-v2-plan`](../../../.codex/skills/executing-tracedecay-v2-plan/SKILL.md); it contains no product implementation. Claude discovery uses only the thin `.claude/skills/executing-tracedecay-v2-plan/SKILL.md` wrapper, which must read the canonical skill completely and invoke canonical `.codex/.../scripts` paths. No `.claude` script, fixture, test, or fallback copy may exist.
...
| [`31-native-fastembed-semantic-code-search.md`](31-native-fastembed-semantic-code-search.md) | Optional disabled-by-default native FastEmbed semantic code search and similar-code/redundancy augmentation: deterministic code representations, exact model/runtime pins, incremental vector generations, Jina/GTE/BGE evaluation, bounded canonical pair discovery, structural-authoritative classification, signed offline artifact lifecycle, bounded optional model-assisted reranking, baseline-preserving failure, generated surfaces, and rebuild-only migration. |
...
`content_digest` is lowercase SHA-256 over RFC 8785 canonical JSON of the fully reconciled owner record after normalization, excluding `content_digest`, `idempotency_key`, lifecycle status, attempts, and receipts. Source-anchor hashes are included, so changed source cannot reuse an old generation. `source_set_digest` has one versioned projection everywhere: RFC 8785 canonical JSON of sorted `[repository-relative indexed-plan path, sha256:<raw Git-blob bytes>]` pairs read from the exact immutable `source_commit` tree. It is never computed from the caller's checkout, legacy inventory records, or reconciled-anchor subsets. `idempotency_key` is exactly `v2-slice-owner/v1:<percent-encoded-normalized-id>:<content_digest>` and is reused for every create/import/retry of that owner generation. A changed digest is a new generation; the previous generation is explicitly superseded, never mutated or duplicated.
...
The checked canonical `plan_authority_registry.json` explicitly owns 257 executable slices and eight non-executable series, including reviewed owner anchors, phases, commit subjects, and prerequisites. `compile_plan_authority.py` materializes that registry and every plan input from one immutable Git tree, validates pinned anchors/series/edges/cycles, and generates `execution-authority.json` plus an initial `activation_mode=verify_only` state. Verification-only contains the complete DAG/topological order but no completion entries, dispatch packets, tests, branches, or worktrees. `bootstrap_execution.py` is the only pre-V2 import/activation façade: it cross-checks manifest/state, stages both in one immutable generation, and atomically switches `.tracedecay/v2-execution-active.json`. Missing state blocks dispatch but not read-only plan audit. The pre-V2 bootstrap rejects dispatch mode; that later transition requires reviewed daemon authority, exact bounded packets, and a fenced prior-revision CAS. Neither Claude nor Codex owns another state path.
...
### 2.2 Completion ledger and deterministic next-ready gate

Completion authority is the machine-readable `tracedecay.v2.completion-ledger/v1`, never a card column, task status, checked box, branch name, worker summary, process exit, or candidate commit by itself. The read-only planning export is `tracedecay.v2.execution-state/v1` and contains exactly one verified `tracedecay.v2.canonical-dag/v1`, one ledger pinned to it, one bounded dispatch specification per DAG node, and the explicit retired-obligation tombstone set. The export remains operational state outside Git; only its schema, validator, synthetic fixtures, and tests are checked in.

The canonical DAG pins stable repository identity, exact canonical source/integration commit, source-set digest, positive graph revision, canonical graph digest, sorted node records, and one matching activation receipt. Every node has one normalized ID, one unique owner, one content digest, one digest sealing its complete dispatch/test/workspace contract, and explicit canonical prerequisite IDs. Dispatch packets, ledger required-test names, receipt commands, and workspace declarations must match that canonical per-node digest; coordinated rewrites of those export fields and their self-computed receipt digests are not new authority. Duplicate IDs or owners, unknown/self/retired endpoints, a cycle, stale digest, missing activation receipt, or any activation pin mismatch invalidates the entire view and emits no worker packet. `FM-168` remains a retired corrected tombstone and can never be materialized as a node, dependency, ledger entry, or dispatch obligation.

Each ledger entry is keyed by canonical slice ID and pins all of:

- exact candidate commit and digest plus a fresh clean `git worktree list`/`git status` observation while the candidate is not integrated; after verified integration, immutable candidate evidence and fresh ancestry replace any requirement to retain the historical worktree;
- implementation task and actor, parents, independent-review tasks, remediation tasks, successor-review tasks, and integration task lineage;
- one independent review receipt over that exact candidate, including verdict, reviewer, receipt digest, evidence anchors, and a matching trusted daemon/task-event observation outside the submitted export;
- the complete named required-test set and one command/exit/candidate/digest receipt per test, each matched to a trusted daemon/task-event observation outside the submitted export;
- canonical integration state naming the exact candidate and current canonical integration commit, canonical branch, candidate-ancestor proof, integration receipt, graph revision/digest, and source-set digest;
- the current canonical source commit, source-set digest, graph revision, and graph digest repeated at entry scope so stale evidence cannot be detached and replayed.
- the exact attempt ID, lease/fence epoch, last observed steering event sequence, current event sequence, and terminal compare-and-swap sequence, plus one canonical disposition receipt for every required directive through the observed cutoff. Each receipt binds directive ID, attempt/fence, event sequence, delivery boundary, delivered/acknowledged state, `applied|rejected|superseded` disposition, actor/authority, and payload digest.

The validator requires a live authoritative checkout (`--root`) and intended canonical ref. It resolves that input to one exact full live ref and commit, binds the integration receipt's canonical branch/ref to that same full ref, recomputes the source-set digest from exact plan blobs in that Git tree, inspects active candidate branch/commit/worktree/cleanliness association, and runs `git merge-base --is-ancestor` itself. Every Git subprocess has a finite timeout and bounded stdout/stderr; spawn, timeout, overflow, nonzero, malformed output, ref, or path failure is `Unknown`, invalidates the complete view, and suppresses all packets. The integration receipt embeds its sealed ancestry observation (repository, exact candidate/canonical commits and ref, method, exit/status, and observation digest). Candidate/workspace/review/test/integration/steering digests are recomputed over canonical payload bytes. Review/test digests must be present in the separately observed trusted event stream; arbitrary identities, anchors, exit codes, and recomputed hashes inside the export are self-certification, not evidence. A shape-valid SHA-256 string, asserted `candidate_is_ancestor=true`, or bare `independent=true` is not evidence.

Duplicate ledger entries are ambiguous and reject the export. Candidate-only, approved-but-unintegrated, missing-test, failed-test, self-review, negative-review, stale-candidate, non-ancestor, or unknown integration evidence never counts as completion. A rendered negative verdict remains terminal evidence and requires the explicit remediation/successor-review lineage, but only a later independent approval over the successor candidate plus complete test and canonical integration receipts can satisfy the slice. Missing ledger evidence means only “not proven complete”; it never inherits `done` from a board or task service.

Required steering through the observed watermark must have exactly one delivered, acknowledged, attempt/fence-current disposition receipt. Missing delivery, stale-attempt acknowledgement, duplicate delivery, or unresolved required disposition invalidates completion. A required directive observed after the attempt watermark but before terminal CAS aborts that CAS. A required directive after terminal CAS must bind explicit remediation and successor-review task IDs present in the entry lineage before it can become a valid blocker against immutable history; unrelated or merely non-empty lineage is insufficient. Advisory steering never fences completion. Card/comment presence alone is not delivery or acknowledgement evidence.

`next-ready` validates the whole export first, then derives completion solely from verified integrated ledger entries. It skips verified integrated nodes; blocks any node with candidate/incomplete evidence; blocks any node whose prerequisite lacks verified integrated completion; and selects only untouched nodes whose complete canonical prerequisite set is verified integrated. Ordering is canonical slice ID order. Any malformed, ambiguous, stale, cyclic, duplicate-owner, retired-obligation, or pin-mismatched authority suppresses the entire ready set rather than returning a partial answer.

Every selected node is rendered as one bounded worker packet carrying exact files (maximum 32), exact acceptance commands and required named tests (maximum 32 each), branch/worktree binding, prerequisites, GPT-5.6-Sol reasoning/lifecycle/decomposition/complex-review ownership, acting runtime, the independent-review/remediation/successor-review/integration gates, and durable retrieval anchors (maximum 64). Every scalar string reachable from a worker packet is limited to 2,048 characters, including owner, branch, worktree, lane, optional-worker, gate, and source-anchor values. Opus may appear only in a later explicitly reviewed packet for one GPT-decomposed single-branch micro-implementation; it never plans, designs, analyzes, performs adversarial or independent review, reasons across branches, integrates, or self-approves. Missing or oversized packet fields reject the export rather than widening scope.

One sealed `tracedecay.v2.next-ready-view/v1` backs every surface. It carries validity, canonical source/digest/revision pins, sorted diagnostics, complete bounded packets, and per-slice blocker reason codes:

- repo-local planning CLI: `python3 .codex/skills/executing-tracedecay-v2-plan/scripts/plan_execution.py --root <authoritative-checkout> --canonical-ref <ref> --next-ready` resolves `TRACEDECAY_V2_EXECUTION_STATE`, then `<repo-root>/.tracedecay/v2-execution-state.json`; explicit `--graph` overrides both. It emits bounded Markdown by default; `--format json` emits canonical typed JSON;
- generated product CLI/MCP: `task next-ready` and its catalog-generated MCP binding use the same application view; human/MCP default is Markdown and an explicit `format=json` returns the typed JSON document;
- generated HTTP/API/SDK: the catalog-owned next-ready capability returns the same `NextReadyViewV1` as JSON, or its shared presentation renderer as `text/markdown`; it never returns transport-authored eligibility, raw ledger internals, or a second pagination/order contract.

The checked fixtures `fixtures/positive-ready.json`, `fixtures/negative-candidate-only.json`, and `fixtures/negative-duplicate-owner.json` are synthetic deterministic schema examples only and never completion receipts for live V2 work. Positive completion and surface tests reseal those templates against an independently created temporary Git repository with real candidate/canonical commits, branches, clean worktrees, full canonical ref, ancestry, repository identity, and live observations. Focused adversarial tests additionally prove live-checkout source freshness; sealed ancestry and workspace evidence; canonical dispatch/test/workspace authority despite coordinated receipt rewrites; duplicate owner reuse across distinct nodes; canonical-ref equality; Git timeout/output bounds and malformed-ref failure; every packet scalar bound; compact Markdown/JSON semantic parity without embedding a JSON dump; canonical receipt recomputation after tampering; exact test-command binding; distinct review authority; external-error `Unknown`; and required-steering cases including explicitly bound post-CAS remediation/successor lineage. They also retain cycle, unresolved prerequisite, retired FM-168, and duplicate JSON-key rejection.

Execution follows checked PR/TDD slices, current repository instructions, and whatever orchestration tools are available at implementation time. No optional named agent skill is a dependency of this plan set. `.codex/skills/executing-tracedecay-v2-plan` is the sole implementation of the optional parser/checklist aid; any host wrapper only discovers and delegates to it, and conformance tests reject copied host scripts. Claude and Codex resolve the same `TRACEDECAY_V2_EXECUTION_STATE`, repo-root default, or explicit `--graph`, and resolve the bootstrap manifest through the same explicit/environment/repo-root precedence. They therefore read one execution-state document, completion ledger, tracking state, and receipt set. No host-scoped manifest/ledger/state/cache/output path exists. Its inventory output is never completion or dependency authority without Git/review/test/task evidence.

### 2.3 Versioned initial staged-dispatch transition

The first dispatch-enabling authority is a new schema, not a relaxation of `tracedecay.v2.execution-state/v1`. `compile_plan_authority.py` remains a V1 `activation_mode=verify_only` compiler and `bootstrap_execution.py` remains the V1-only pre-cutover installer; both continue to reject dispatch activation. `plan_execution.py` routes only the exact V1 and V2 schema identifiers and returns `unsupported execution-state schema/version` with an empty ready set for every other version.

`tracedecay.v2.execution-state/v2` is default-deny and covers all 257 canonical DAG nodes exactly once. Its `dispatch_specs` contains exactly one complete reviewed packet for `PR 1`; its `dispatch_blocks` contains exactly 256 canonically ordered records with reason code `not_in_dispatch_scope`. The two sets are disjoint and exhaustive. Each node's `dispatch_digest` domain-separates and binds its slice ID, repository/source/graph pins, reviewed policy and packet-source digests, and either the complete packet or complete blocker. A second digest seals the full sorted 257-entry contract set. Missing, duplicate, overlapping, unknown, unsealed, or extra packet/block records invalidate the entire export and suppress every packet. Topological order remains audit/display data and can never manufacture a packet or override explicit scope.

The sole checked staged source is `.codex/skills/executing-tracedecay-v2-plan/scripts/staged_dispatch_pr1.json`. It binds the reviewed current verify-only manifest/source-set authority and repeats PR 1's exact owner, content digest, commit subject, all eight acceptance criteria with immutable source anchors, both reviewed source blocks, 15-file effect ceiling (including the generated dependency policy and its generator), five commands, ten named architecture-boundary tests, deterministic external worktree/branch, GPT-5.6-Sol reasoning/lifecycle/decomposition/complex-review ownership, explicit prohibitions, and distinct implementation/review/remediation/successor-review/integration gates. Its retained Claude-review compatibility field is disabled with zero steps and no runtime or criteria. Opus can appear only in a later reviewed packet revision for one GPT-decomposed micro-implementation branch, never for planning, audit, reasoning, authority review, or integration. This stage authorizes no other slice. The PR 1/3 companion remains a retrieval anchor here; its implementation ownership is not silently inferred into this initial packet.

`transition_execution_authority.py` is the only V2 transition operation. The implementation, packet, and regenerated manifest are committed first; a fresh V1 predecessor is then compiled and bootstrapped from that exact immutable canonical commit/source set. Preparation reloads that named stored predecessor generation and the checked source from the same canonical Git tree. Activation requires a bound receipt beneath `.tracedecay/v2-authority-review-receipts/` whose digest appears in the fixed owner-only mode-`0600` `.tracedecay/v2-authority-review-observations.json`; no caller-selected observation path exists. The observation ledger is an explicit local operator trust assertion, not cryptographic proof of reviewer identity. Under the existing activation lock the transition revalidates exact compiler predecessor bytes, exact revision `prior+1`, live repository/source pins, packet/block partition, every digest, and active-pointer bytes before staging one immutable generation and atomically compare-and-swapping the pointer. Exact replay can reload the stored predecessor after the pointer advances and is idempotent; stale/missing generation, revision skip, source drift, unobserved/malformed receipt, equal-revision different bytes, concurrent activation, or any validation error writes no active pointer.

The reviewed transition flow is:
...
# Independent control-plane review records its exact receipt under
# .tracedecay/v2-authority-review-receipts/ and its digest in the fixed
# mode-0600 .tracedecay/v2-authority-review-observations.json ledger.
python3 .codex/skills/executing-tracedecay-v2-plan/scripts/transition_execution_authority.py \
  --root "$(git rev-parse --show-toplevel)" \
  --canonical-ref refs/heads/codex/tracedecay-total-redesign-plan \
  --expected-active-generation <exact-verify-only-predecessor-generation> \
  --activated-at <same-reviewed-RFC3339-time> \
  --candidate <review-candidate.json> \
  --trusted-review-receipt .tracedecay/v2-authority-review-receipts/<receipt>.json
```

TraceDecay validates but never creates, relocates, reuses ambiguously, pushes, merges, or grants Claude authority over the declared PR 1 worktree. Until the independently observed receipt and exact predecessor fence exist, preparation is review-only and activation remains fail-closed.

#### Event-observed initial completion transition

The initial staged authority cannot be completed by editing its operational JSON. `complete_execution_authority.py` is the sole bounded V2-to-V2 bootstrap recorder for PR 1. It consumes the exact activated packet, an empty-ledger predecessor generation, a PR 1 entry template with no caller-authored test receipts, fixed owner-only `.tracedecay/pr1-independent-review.json`, and fixed owner-only `.tracedecay/pr1-independent-review.event.json`. The first-party collaboration event must name reviewer task `/root/completion_receipt_schema` and bind its fixed receipt path/digest, verdict, observation time, and exact candidate under an outer event digest; the nested receipt binds the distinct reviewer principal/authority. No public review path exists. The recorder first fences the active pointer, proves the packet-declared candidate worktree is a real non-symlink directory on the exact candidate commit and branch with a clean status, enumerates the exact ten architecture test names, then independently reruns all five checked acceptance commands in that candidate worktree with bounded output. It proves every enumerated name executed successfully under `cargo test --test architecture_boundaries`; the four acceptance-only command receipts carry an empty test set. It rechecks candidate HEAD/branch/cleanliness before publishing observations. The union of grouped test names must equal the ten required tests exactly once, and each exact acceptance command must have exactly one successful receipt.

Only this recorder writes `.tracedecay/v2-completion-observations.json`. It atomically records the independently validated review digest and recorder-produced command/test digests in exact schema `tracedecay.v2.completion-observations/v1`, at fixed owner-only mode `0600`. Neither CLI arguments nor environment variables can select another observation path. Manually authored digest lists, pasted logs, thread claims, worker summaries, and receipt JSON whose command was not executed by this boundary remain untrusted. The ordinary read path only consumes this fixed observation ledger; it never promotes submitted receipt digests by seeing them in the candidate.

The completion transition reloads and validates the named stored V2 generation, fences exact predecessor or exact replay-target pointer bytes before commands or observation writes, and verifies current canonical ref/source set, live candidate ancestry, authority-review observation, independent-review principal/authority separation, complete packet, and all 256 blockers. Because no historical task-attempt stream predates this bootstrap, the recorder rejects any caller-claimed attempt, integration, or steering evidence and creates one clearly labeled `bootstrap-reconciliation` attempt bound deterministically to PR 1, candidate, and predecessor generation. It records sequence/watermark/terminal CAS `0`, an empty steering stream, a recorder-owned integration task, and never presents these as retroactive worker evidence. It changes only `completion_ledger.entries` from empty to the one verified PR 1 entry, keeps manifest/graph/policy/packet/block bytes unchanged, stages a new immutable generation, and atomically compare-and-swaps the pointer under the shared activation lock. Exact replay is idempotent. A stale pointer, nonempty predecessor ledger, source or candidate drift, failed/missing/unexpected test, post-command worktree mutation, missing observation, altered partition, supplied-candidate difference, or concurrent writer performs no pointer mutation.

The original staged-dispatch candidate digest seals the immutable graph/policy/packet/block authority and normalizes only append-only completion entries to empty; the authority-review receipt is not rewritten when completion is appended. Immediately after completion, `next-ready` is valid with PR 1 omitted, zero ready packets, and the same 256 explicit `not_in_dispatch_scope` blockers. Although PR 2, PR 2A, and PR 2B then have a verified prerequisite, none becomes dispatchable until a separate reviewed authority revision supplies exactly one next packet and a new exhaustive packet/block partition. Execution order never widens scope.
...
- The V2 implementation program does not use Hermes Kanban. The activated TraceDecay V2 manifest, completion ledger, and `next-ready` view are the sole task/dependency/completion authority. GPT-5.6-Sol owns orchestration, decomposition, reasoning, review, and fenced lifecycle. Opus may execute only one explicitly GPT-decomposed single-branch micro-implementation in a later reviewed packet; it has no planning, design, analysis, adversarial-review, independent-review, or integration authority. Record every actual participant and independently verify every candidate.
...
- PR 14A is the only semantic-search slice permitted in Phase 2: release-excluded evaluation over frozen synthetic code-document and redundancy pair/cluster fixtures after PR 2A. Search and redundancy contribution receive independent accepted/disabled verdicts. Production order is Phase 3: `PR 6C + PR 18D + either accepted PR 14A contribution -> PR 14E -> PR 14B`; PR 14C additionally requires the search contribution to be accepted. A fully disabled 14A terminalizes all three production-only descendants as skipped.
