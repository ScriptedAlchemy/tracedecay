...
| `gen_fts`, `gen_fingerprints`, `gen_complexity`, `gen_redundancy` | Rebuildable auxiliary logical families keyed by `(snapshot_id, occurrence_id \| file_id)` with the extractor/resolver version that produced them. Fingerprints/redundancy contain deterministic lexical, normalized-token/body, and structural evidence only; semantic vectors, nearest-neighbor candidates, and model scores never enter these rows. Their SQL columns, indexes, migrations, and triggers land only in plan 02 PR 6C; PR 18D/18F supply logical rows and conformance fixtures. FTS indexes only receipt-bound `SearchEligibleText`. |
...
| Complexity/redundancy/health scans | Auxiliary generation families (`gen_complexity`, `gen_redundancy`, `gen_fingerprints`) provide the exact/structural baseline; `SemanticCodeDocumentV1`/`SemanticCodeChunkV1` is the sole optional-vector handoff | Plan 05 aggregates and, only when plan 31's profile is effective, adds bounded query-time semantic-neighbor candidates without rewriting these rows; plan 19 §13 scorecard inputs |
