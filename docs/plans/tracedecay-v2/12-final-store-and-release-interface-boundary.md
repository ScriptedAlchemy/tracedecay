# Final store and released-interface boundary

## Status / role

Every TraceDecay database, store, spool, file, journal,
...
One local daemon owns a local live store. With remote shared Brain, exactly one
fenced daemon owns each mutable shard. Reset/recreation never creates another
writer.
