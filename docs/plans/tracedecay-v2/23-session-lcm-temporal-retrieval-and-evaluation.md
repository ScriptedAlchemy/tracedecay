...
`crates/tracedecay-application/src/session/types.rs::SessionTemporalMetadataView`. The view binds
...
- `src/query/temporal/mod.rs::execute_temporal_kernel` is the sole orchestration
  entry point.
- `crates/tracedecay-application/src/session/retrieval.rs::SessionRetrievalService::retrieve`
  owns authorization and canonical request construction.
- `crates/tracedecay-application/src/session/ports.rs::SessionTemporalExecutionPort` owns the
  application/kernel boundary.
- `crates/tracedecay-application/src/session/refresh.rs::SessionRefreshService` owns explicit
  refresh begin-or-join, status, and cancellation.
- `crates/tracedecay-application/src/session/types.rs` owns application request, result,
