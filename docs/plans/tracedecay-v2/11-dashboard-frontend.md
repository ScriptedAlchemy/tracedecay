...
- Task/attempt inspector consumes Plan 24 §4.5A's sealed `ReviewLineageViewV1` without joins or inference and shows active PlanVersion authority, canonical generation/predecessor/successor, decomposed reviewer components, effective heads and validity reasons, remediation/fallback/late-evidence state, typed anchor coverage/availability, readiness digest, cursor/watermark, and generated legal capabilities. Candidate versus active plan admission, plan-local labels and derived-work provenance, local plus enclosing dependency closure, assignment/route rationale, lifecycle owner and acting native-CLI/provider/adapter participants, requested versus actual host/provider/model/reasoning effort/tools/skills/grants, typed failure causality, fenced lease state, checkpoint state, packet/omissions, workspace authority, Turns/tools/artifacts/handoffs/outcomes, cancellation/reconciliation, costs, audit, and anchors remain in the same inspector.
- Active-attempt steering is first-class in that inspector, not a task-comment side effect. The **Comments** history shows immutable revisions and ordinary notifications. An authorized revision has an explicit **Promote to steering** action; a separate **Steer active attempt** composer targets the exact current `ExecutionAttemptId`, lease/fence epoch, accepted packet, graph revision, and steering-head version. It requires `Advisory` or `Required`, a closed directive kind, bounded payload/anchors, priority, expiry, and an idempotent compare-and-swap submit. The composer shows live measured bytes/tokens and the effective versus absolute payload, batch, per-Turn, rolling-rate, and cooldown ceilings from the generated view; crossing one disables submit or presents the server-authored advisory deflection without client-side truncation. The confirmation names the target agent/attempt and currently declared safe-delivery capability; it never promises “send now.”
- The steering rail is ordered by canonical `steering_sequence` and shows source comment when present, actor/authority, expected-versus-current packet/graph state, expiry, requirement, delivery boundary (`NativeInterrupt`, `AfterToolBeforeModel`, `StopContinuation`, or `NextTurnOnly`), claim/receipt, acknowledgement, and `Applied|Rejected|Superseded|Cancelled` resolution. `Deferred`, `DeliveryUnknown`, `BlockedByLimitChange`, stale lease, unsupported mid-Turn delivery, rate/cooldown deflection, and terminal-race rejection are visibly distinct. A limit-blocked row compares absolute/admitted/current-effective values without payload and offers only server-authorized supersede/cancel remediation. Duplicate or stale acknowledgement never appears as progress. Required unresolved/unknown/limit-blocked items display the server-authored completion/integration fence and its exact legal resolution actions; advisory items never render a blocking badge.
- Live SSE updates preserve the current draft and focus while installing only monotonically newer directive/receipt state. A reconnect resumes by event ID; a gap makes the rail stale, disables acknowledge/resolve/supersede, and reloads the authoritative unresolved set before actions return. Duplicate receipts never create duplicate cards or toasts. One compact Turn-delivery batch is shown as one boundary event with its member sequences; the UI does not infer model visibility from daemon acceptance or hook invocation.
- Attempt timeline and Causal Loom render `steering submitted → delivery boundary → acknowledged → disposition` as typed causal events beside tool/model/Turn boundaries. Playback can demonstrate supported current-Turn delivery, next-Turn fallback, duplicate/reconnect behavior, stale-fence refusal, and the late-terminal CAS race without delivering to a live attempt. Adapter conformance and Hint Lab remain read-only experiments; neither replay mutates a steering sequence or clears a completion fence.
- Generated legal actions expose `work_items.record_attestation`, `record_review`, `record_decision`, `record_exception`, `handoff`, `reopen`, and `reverse_transition` exactly. Review dialogs require cycle-authority and correction-head revisions plus typed role anchors; combined requests display the deterministic component decomposition and never submit an aggregate verdict. Stale/partial hydration and delayed deltas disable consequential actions and resync rather than regress the visible current cycle. Attestation/review/decision/exception/handoff cannot set derived readiness/acceptance directly; reopen creates a successor work-item version; reverse-transition shows the exact compensating edge and receipt and is never labeled generic “undo.”
...
- Test: `dashboard/tests/e2e/{work-initiative-plan,work-kanban-dag,work-attempt-packet,work-attempt-steering,work-executor-critical-path,work-worktree-lifecycle,work-markdown-edit,work-notifications}.spec.ts`
...
- [ ] Implement the section 13.0 steering rail/composer and comment-promotion flow from generated contracts. Pin the active attempt/lease/fence/packet/graph revisions, label actual safe-boundary capability, preserve ordered delivery/ack/disposition receipts, show required completion fences, and keep advisory state non-blocking. Never map comment creation, notification, daemon acceptance, or hook invocation to model-visible delivery.
...
- Task comment revisions, promoted/direct steering identity, attempt/lease/fence/sequence/packet/graph pins, actual safe delivery boundary, acknowledgement/disposition, and required completion-fence state equal the sealed API view; advisory state never blocks.
...
- During an active-attempt fixture, add one ordinary comment and prove it is not delivered; promote its exact revision to required steering, observe the declared safe-boundary or truthful next-Turn fallback, acknowledge and resolve it, then replay duplicate/reconnect/stale-lease/late-terminal cases while proving exactly one bounded continuation and advisory non-blocking in `<= 90 s`.
