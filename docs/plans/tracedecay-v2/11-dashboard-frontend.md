...
- The steering rail is ordered by canonical `steering_sequence` and shows source comment when present, actor/authority, expected-versus-current packet/graph state, expiry, requirement, delivery boundary (`NativeInterrupt`, `AfterToolBeforeModel`, `StopContinuation`, or `NextTurnOnly`), claim/receipt, acknowledgement, and `Applied|Rejected|Superseded|Cancelled` resolution. `Deferred`, `DeliveryUnknown`, stale lease, unsupported mid-Turn delivery, rate/cooldown deflection, and terminal-race rejection are visibly distinct. Duplicate or stale acknowledgement never appears as progress. Required unresolved/unknown items display the server-authored completion/integration fence and its exact legal resolution actions; advisory items never render a blocking badge.
