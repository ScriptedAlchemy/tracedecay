...
|---|---|---|
| Hint | historical event/session position or sanitized synthetic event; host/provider; project/ref/snapshot; deterministic/scout engine, policy/config/index/memory/tool/catalog/model capability; optional observed Codex definition/source/trust snapshot | normalization, trigger/context delta, hypotheses, approved bounded reads/anchors, model/deterministic candidates, suppression/dedupe/cooldown/escalation/budget, exact addressed envelope/payload, delivery timing, tokens/latency/cost, adoption/outcome; Codex replay additionally shows all ten event schemas, match result, definition/run grouping, concurrent completion order, timeout/exit/output decode, deny/continuation precedence, unsupported fields, trust/managed skips, and interception coverage gaps |
