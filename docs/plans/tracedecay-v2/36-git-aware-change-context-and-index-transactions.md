...
**Approval issuance is owner-decided and lands as a sixth operation**
(owner decision, 2026-08-07: an explicit `approve_native_integration`
operation bound to the exact preview identity *and* digest; this plan's
declared operation set is amended accordingly). Until that operation is
mounted end to end, `NativeIntegrationApprovalV1` still has no production
issuance surface, so every `apply_native_integration` request truthfully
fails closed: the daemon resolves approvals only from the durable store, a
missing or mismatched approval is denied without disclosing whether the
target was absent or denied, and no invalid-request façade replaces the
typed result. None of the safety constraints above are relaxed, and no
transport may fall back to local mutation in the meantime.
...
- Persist and query branch-stack relationships through the canonical embedded
  Grafeo authority in `tracedecay-graph-db`. Small standard-library collections
  may validate one bounded admitted snapshot, but no second graph model is
  retained. Plan 16/36 still own edge meaning, authorization, visibility,
