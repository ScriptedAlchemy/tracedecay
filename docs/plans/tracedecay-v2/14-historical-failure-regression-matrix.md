...
| PR15 | Explicit repository/worktree/ref and LSP workspace-folder targets never fall back to CWD, first workspace, or active checkout; cross-project results exact-load globally; dirty/stale graph and multi-root diagnostic coverage is explicit. Local worktree/branch-stack scope remains independent from review topology. Plan 36 applies only clean authorized policy-approved no-conflict fast-forward, two-parent merge, or exact ordered cherry-pick and returns one terminal receipt; semantic conflict escalates. [Plan 37](37-branch-aware-feedback-cycle-pr-review-and-agent-proximity.md) multi-root/cross-project scope-isolation binds each feedback-cycle trigger, GitHub remap, CI localization, and proximity warning to its exact owning root with per-root branch/worktree/head/generation identity; ambiguous, denied, stale, or unsupported roots return typed unavailable or partial coverage with no fallback to another root; cross-root proximity and privacy scoping never leak another session's content. Its optional GitHub Stacked PR adapter covers all four capability states, official linear/merge semantics, mandatory generic fallback, and zero TraceDecay rebase/force-push/provider write. |
...
| `topology.dimensions.no-git` | A valid task with no placement, branches, review, or integration remains visible and executable through its non-repository provider. |
| `topology.dimensions.worktree-unbranched` | A linked worktree with `Unbranched` does not invent a branch, PR, stack, or integration strategy. |
| `topology.dimensions.worktree-independent` | Independent branches in managed worktrees do not become a local or PR stack. |
| `topology.dimensions.local-stack-no-pr` | A local branch stack with no pull requests retains branch order and integration policy without review topology. |
| `topology.dimensions.pr-stack-no-worktree` | A GitHub PR stack with no managed worktree renders review order without inventing execution placement. |
| `topology.github-stack.unavailable` | `Unavailable` preserves the standard-Git/other-forge fallback and exposes no stack controls. |
| `topology.github-stack.private-preview-disabled` | `PrivatePreviewDisabled` is distinct from unavailable/degraded and never implies repository enrollment. |
| `topology.github-stack.enabled-linear` | `Enabled` requires one same-repository linear chain with exact stack ID, positions, base/head refs, and final target. |
| `topology.github-stack.degraded` | `Degraded` retains partial/stale reason and fallback availability without claiming current stack order. |
| `topology.github-stack.final-target-gates` | Every layer shows final-target branch protection and CI state independently from its immediate base. |
| `topology.github-stack.atomic-selected-layer` | Selecting a layer includes every unmerged lower layer in one provider-atomic merge and never skips the lowest unmerged layer. |
| `topology.github-stack.partial-merge` | Remaining layers show the observed provider rebase/retarget and a new snapshot; TraceDecay emits no rebase or force-push effect. |
| `topology.github-stack.merge-queue` | Direct and stack-aware merge-queue states remain distinct provider observations. |
| `topology.github-stack.cli-surface` | `gh stack init/add/rebase/push/submit/sync/modify/unstack/checkout` are recognized as optional external capability operations; mutating/rewrite commands are never auto-invoked. |
...
| `topology.integration.cross-merge-dry-run` | Native dry run remains effect-free with mechanical/semantic coverage; apply appears only for an eligible Plan 36 owner operation. |
| `topology.integration.cherry-pick-eligible` | Exact ordered single-parent commits, clean state, complete evidence, policy approval, and no conflict produce a Plan 36 apply action and immutable receipt. |
| `topology.integration.cherry-pick-blocked` | Merge commits, reordered commits, duplicate patch IDs, dirty state, incomplete evidence, conflict, or denied policy keep cherry-pick preview-only and escalate. |
...
| `operation.apply-native-integration-supported` | Eligible Plan 36 fast-forward, two-parent merge, and exact ordered cherry-pick expose owner-supplied apply/status/cancel with CAS, authorization, and receipts. |
| `operation.apply-history-rewrite-absent` | Rebase, force-push, arbitrary cherry-pick, and other history rewrites expose no DOM, keyboard, deep-link, or crafted-client apply path. |
