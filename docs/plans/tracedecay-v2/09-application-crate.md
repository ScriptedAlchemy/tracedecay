...
| Merged PRs #441/#443/#445, Hermes memory/context, rollout recovery, and projectless host routing | User/profile compatibility stores shipped; legacy session import distinguishes neutral/project/unresolved evidence; host-profile ownership comes from installed/configured plugin identity; provider workspace/home resets per session; user-scoped memory/LCM/message search bypasses project routing; registry/read-only/mutating routes are distinct. | Treat these as V1 parity and migration fixtures. Application owns canonical scope classification, mixed-selector rejection, route class, authority, and coverage; host adapters cannot derive data ownership from `HERMES_HOME`/CWD, keep previous-session routing, promote unresolved memory, or maintain tool-name route allowlists. |
...
Provider/session freshness is one registered operation kind above this same kernel. Its canonical key binds TraceDecay profile, provider, `SourceInstanceId` set, declared source scope, committed source frontier, requested target watermark, sanitizer/parser/config digests, and authority epoch. Equivalent concurrent requests join the same operation and observe the same progress, coverage, cancellation, failure, and terminal receipt; a waiter never treats leader disappearance or a process-local notification as success. The operation scans each source once, while project attribution and read projections fan out downstream. A changed target watermark or source/config generation is a new operation input, not an accidental join.
