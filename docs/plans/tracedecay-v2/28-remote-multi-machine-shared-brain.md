...
Earlier enrollment, topology, spool, replica, failover, fixture, and packet
artifact names are historical evidence, not standalone prerequisites or
mandatory recreation targets. Only actually independently released public
...
fenced replay, query, failover, platform, and regression behavior below.

The Remote Brain keeps no backup or restore surface. The authority's own
store is the single durable copy of its data; a node never snapshots it into
a sibling artifact, and nothing restores a store from one (AGENTS.md, *No
secondary copies*).

No remote enrollment/spool/replica format is established on `origin/master`
or in a published package/release. Pure source-only/internal enrollment
helpers, wire-visible V2 enrollment revisions, spool files, replica journals,
checkpoints, and receipts take their final shape in place. Only the exact final persisted shape is accepted; any other database,
...
Brain with honest local/remote coverage, and fail over to a standby without
admitting two writers or losing deletion state.
...
4. **Fence and fail over.** Promotion acquires a higher epoch with an
   ...
   verifies the standby has the required durable frontier. The old authority
   remains read-only on rejoin until explicitly reseeded.
...
spool state, replay receipts, query coverage, and failover/rejoin state. Human and structured output use the same
...
- A higher epoch must be durably installed at every selected-model sink before
  ...
  manifest, diagnostics publication, and other
...
- The remote offline-capture spool is distinct from the provider-capture daemon
  ...
  proximity never enter the spool, read cache, replica, trace, failover
  payload, or remote analyzer request.
...
- Durable saved-content feedback, GitHub-ingested read-only
  ...
  retention, and authority epoch across restart, promotion, and failover. No GitHub write path exists.
...
- Before replay admission, current deletion, tombstone, quarantine,
  retention, authorization, and exact project scope are evaluated. Older
  captured content cannot resurrect deleted data, bypass a newer policy, or
  republish stale GitHub/CI/feedback evidence.
...
- Node revocation immediately blocks commands, replay, cache refresh, and
  promotion credentials. Delayed packets from
...
- Build the first delivery on existing HTTP/SSE, rustls, and the daemon-owned
  ...
  admission, replay identity, coverage, and failover.
- At the concrete integration that needs them, consider `reqwest` plus
  ...
  `object_store` for an admitted object backend, and Hickory for discovery.
  Admit one only when it deletes named transport, stream, cancellation,
  backend, or discovery code and
...
4. **Complete failover.** Exercise higher-epoch promotion and read-only
   old-authority rejoin from the same operational surfaces.
5. **Keep protocol and storage separate across the journey.** An actual
   ...
   repository identity, retention/deletion, diagnostics, spools, caches, and
   replicas accept only their exact final persisted shape. Any other shape returns `ResetRequired` before interpretation and
...
- Remove standalone enrollment, topology, spool, replica, or failover contract
  phases that do not participate in this journey. Retain exact final schema
  validation and explicit reset/recreation in the first callable
  enrollment/capture, replay, query, or failover slice that uses persisted
  state.
- Remove duplicated provider-specific or Plan 37 acceptance matrices. Durable
  ...
  localization use the same fenced replay/query/failover path; overlays
...
remain node-local, promotes the standby under a higher installed fence, and proves the old authority cannot
...
cache manifests, newer tombstone/quarantine state, insufficient standby frontier, rollback, and
