...
- The catalog foundation requires immutable capability metadata composed with
  real application handlers and consumed by application and policy paths.
- Surface bindings require CLI, MCP, HTTP, LSP, and discovery bindings.
  Delivery requires
  every advertised operation to invoke the canonical handler and pass the
  direct parity journeys below.
  The dashboard delivery first ships dashboard binding, dashboard actions, and dashboard parity
  over the same CapabilityIds and application handlers. SDK bindings are added
  only when the official SDK methods ship.
- The work-capability delivery adds Plan 24 task/work graph and Plan 32 runtime capabilities through
...
- Work-capability auxiliary-provider descriptors for backend/executable/protocol identity
...
- **SDK — SDK bindings:** add Rust and TypeScript SDK BindingIds only
  ...
  or late delivery stage is not permission to omit a family. Surface discovery may describe
...
The core mostly LLM-free primitive families are deliberately narrow:
...
The work-capability delivery extends those families with TaskId/WorkItemId lookup and compact context,
...
handlers ship. Those operations are absent from core surface profiles and discovery until then.
...
1. **Callable foundation:** every contributed capability resolves one
...
3. **Binding journey:** switch CLI/MCP/HTTP/LSP discovery to the immutable
...
- Catalog unit tests cover stable ID serialization, immutable snapshots, duplicate/conflict rejection, profile ceilings, explicit absence, deprecation, availability, and deterministic lookup.
- Catalog integration tests prove every catalog entry resolves to one real
...
- Surface parity tests invoke representative read, write, administrative, streaming, and long-running use cases through CLI, MCP, HTTP, and LSP adapters and compare typed results before rendering.
- Feedback parity tests call the canonical diagnostics/impact readers
  ...
  data ownership.
- Advisory provider-contribution tests prove GitHub review, CI localization, and
  ...
  otherwise.
- Git parity tests compare CLI and MCP `git_preview`/`git_apply` semantic
...
- Dashboard contract tests add dashboard binding, dashboard actions, and dashboard parity on the same typed requests and CapabilityIds.
- Work-capability catalog tests prove every Plan 24/32 binding resolves to one application
...
- Provider-descriptor fixtures cover missing executables, version drift,
