...
**Plan 32 integration:** register its definition/version/source/run/node/history/taskgraph-candidate use cases, JSON Schema and engine/executor capabilities, effect/cost metadata, compact three-tool MCP profile plus paged history resource, and generated CLI/HTTP/SDK/plugin bindings in this one catalog. The catalog describes them but never compiles or executes workflow source, replays history, or reinterprets Plan-32 taskgraph eligibility.
