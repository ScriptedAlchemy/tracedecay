# Workspace refactoring and API cutover

## Status / role

The published read-only `tracedecay_rename_preview` and the apply-grade
rename/API-cutover journeys are one production capability. Every public
operation is mounted through the canonical application layer and SDKs; a tool
name, schema, or dormant adapter alone is not delivery.

Crate extraction uses ordinary source moves, narrow typed ports, and only
release-evidenced compatibility façades. It does not depend on a persisted
migration subsystem or create staged product modes.
...
The published `tracedecay_rename_preview` surface is release evidence and keeps
its compatibility obligation. Pure source-only/internal Plan 34 request
helpers and unreleased wire revisions change in place. Persisted state accepts
...
recreation. No old-state reader, backfill, dual write, or census path exists. An API alias,
