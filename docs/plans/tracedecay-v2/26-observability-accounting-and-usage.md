...
| `metric.usage.hook_calls` | Hook handler runs plus invocation-group/effect-arbitration conservation | Known from ledger | Bounded host/event/source-layer/managed/trust/result segmentation; exact definition/run/group/build IDs remain protected drill-down refs, never metric labels |
