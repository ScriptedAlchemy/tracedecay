...
- `task_offers(offer_id BLOB PRIMARY KEY, work_item_id BLOB NOT NULL, work_item_version_id BLOB NOT NULL, executor_registration_id BLOB NOT NULL, readiness_digest BLOB NOT NULL, state TEXT NOT NULL, issued_at INTEGER NOT NULL, expires_at INTEGER NOT NULL, claimed_attempt_id BLOB NULL, idempotency_key BLOB NOT NULL UNIQUE)`; partial unique `(work_item_id, executor_registration_id) WHERE state='open'`; indexes `(state, expires_at)`, `(executor_registration_id, state)`. Terminal offers retain 7 days plus their event/decision refs.
- `task_leases(lease_id BLOB PRIMARY KEY, work_item_id BLOB NOT NULL, attempt_id BLOB NOT NULL UNIQUE, executor_registration_id BLOB NOT NULL, fence_epoch INTEGER NOT NULL, state TEXT NOT NULL, heartbeat_at INTEGER NOT NULL, heartbeat_sequence INTEGER NOT NULL, expires_at INTEGER NOT NULL, expected_work_item_version BLOB NOT NULL, grant_digest BLOB NOT NULL, packet_digest BLOB NOT NULL, issued_at INTEGER NOT NULL, revoked_at INTEGER NULL, UNIQUE(work_item_id, fence_epoch))`; indexes `(state, expires_at)`, `(executor_registration_id, state)`. `task_lease_events(event_id BLOB PRIMARY KEY, lease_id BLOB NOT NULL, attempt_id BLOB NOT NULL, fence_epoch INTEGER NOT NULL, sequence INTEGER NOT NULL, kind TEXT NOT NULL, evidence_blob_id BLOB NULL, occurred_at INTEGER NOT NULL, UNIQUE(lease_id, sequence))`; index `(attempt_id, occurred_at)`.
- `execution_attempts(attempt_id BLOB PRIMARY KEY, work_item_id BLOB NOT NULL, work_item_version_id BLOB NOT NULL, plan_version_id BLOB NOT NULL, ordinal INTEGER NOT NULL, assignment_id BLOB NOT NULL, executor_registration_id BLOB NOT NULL, executor_instance_id BLOB NOT NULL, fence_epoch INTEGER NOT NULL, requested_route_blob_id BLOB NOT NULL, actual_route_blob_id BLOB NULL, workspace_binding_id BLOB NOT NULL, context_packet_id BLOB NOT NULL, capability_grant_set_id BLOB NOT NULL, budget_blob_id BLOB NOT NULL, state TEXT NOT NULL, authority_class TEXT NOT NULL DEFAULT 'canonical', started_at INTEGER NULL, ended_at INTEGER NULL, outcome_id BLOB NULL, state_version INTEGER NOT NULL, UNIQUE(work_item_id, ordinal), UNIQUE(work_item_id, fence_epoch))`; indexes `(work_item_id, state)`, `(executor_registration_id, state, started_at)`, `(state, started_at)`. Imported Hermes attempts use `authority_class='legacy_imported'`, `fence_epoch=0`, and can never be referenced by `active_lease_id`.
...
- `context_packet_manifests(packet_id BLOB PRIMARY KEY, work_item_id BLOB NOT NULL, work_item_version_id BLOB NOT NULL, attempt_id BLOB NULL, scope_resolution_id BLOB NOT NULL, query_digest BLOB NOT NULL, config_digest BLOB NOT NULL, catalog_generation INTEGER NOT NULL, vector_watermark_blob_id BLOB NOT NULL, visibility_digest BLOB NOT NULL, token_budget INTEGER NOT NULL, manifest_digest BLOB NOT NULL UNIQUE, state TEXT NOT NULL, created_at INTEGER NOT NULL, expires_at INTEGER NULL, superseded_by BLOB NULL)`; indexes `(work_item_id, created_at)`, `(attempt_id)`, `(state, expires_at)`. `context_packet_entries(packet_id BLOB, ordinal INTEGER, kind TEXT NOT NULL, anchor_id BLOB NOT NULL, payload_blob_id BLOB NULL, relevance_score INTEGER NOT NULL, inclusion_reason TEXT NOT NULL, omission_class TEXT NULL, PRIMARY KEY(packet_id, ordinal))`; indexes `(anchor_id)`, `(packet_id, kind)`.
...
- `task_idempotency_results(idempotency_key BLOB PRIMARY KEY, command_kind TEXT NOT NULL, work_item_id BLOB NULL, attempt_id BLOB NULL, expected_revision INTEGER NULL, result_blob_id BLOB NOT NULL, committed_at INTEGER NOT NULL)`; index `(work_item_id, committed_at)`, retention 7 days after the longest retry window. `saved_task_views(view_id BLOB PRIMARY KEY, owner_id BLOB NOT NULL, query_blob_id BLOB NOT NULL, query_digest BLOB NOT NULL, lens TEXT NOT NULL, sharing_policy_blob_id BLOB NOT NULL, version INTEGER NOT NULL, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, revoked_at INTEGER NULL)`; indexes `(owner_id, updated_at)`, `(query_digest)`. `task_view_shares(view_id BLOB, grantee_id BLOB, grant_blob_id BLOB NOT NULL, expires_at INTEGER NULL, revoked_at INTEGER NULL, PRIMARY KEY(view_id, grantee_id))`; index `(grantee_id, revoked_at)`.
...
- `config_layer_revisions(layer_id BLOB, revision INTEGER, target_kind TEXT NOT NULL, target_id BLOB NOT NULL, registry_digest BLOB NOT NULL, state TEXT NOT NULL, base_revision INTEGER NULL, idempotency_key BLOB NOT NULL, created_by BLOB NOT NULL, created_at INTEGER NOT NULL, activated_at INTEGER NULL, abandoned_at INTEGER NULL, entries_digest BLOB NOT NULL, PRIMARY KEY(layer_id, revision), UNIQUE(target_kind, target_id, idempotency_key))`; indexes `(target_kind, target_id, state)`, `(activated_at)`. `config_revision_entries(layer_id BLOB, revision INTEGER, key_id TEXT, value_blob_id BLOB NOT NULL, source_class TEXT NOT NULL, PRIMARY KEY(layer_id, revision, key_id))`; index `(key_id)`. Referenced activated revisions indefinite; abandoned unreferenced rows 30 days.
- `config_activation_manifests(manifest_id BLOB PRIMARY KEY, layer_id BLOB NOT NULL, revision INTEGER NOT NULL, effective_config_digest BLOB NOT NULL, registry_digest BLOB NOT NULL, affected_consumers_blob_id BLOB NOT NULL, impact_blob_id BLOB NOT NULL, state TEXT NOT NULL, activated_at INTEGER NOT NULL, completed_at INTEGER NULL, UNIQUE(layer_id, revision))`; indexes `(state, activated_at)`, `(effective_config_digest)`. `config_consumer_acknowledgements(manifest_id BLOB, consumer_id TEXT, generation INTEGER NOT NULL, state TEXT NOT NULL, observed_digest BLOB NULL, acknowledged_at INTEGER NULL, error_code TEXT NULL, PRIMARY KEY(manifest_id, consumer_id))`; index `(state, acknowledged_at)`. Indefinite activation/audit history; low thousands.

Scout/suggestion (activity owner; plan 22 semantics):

- `scout_runs(run_id BLOB PRIMARY KEY, session_id BLOB NOT NULL, thread_id BLOB NOT NULL, turn_id BLOB NOT NULL, agent_id BLOB NOT NULL, generation INTEGER NOT NULL, trigger_digest BLOB NOT NULL, input_manifest_id BLOB NOT NULL, policy_version TEXT NOT NULL, config_digest BLOB NOT NULL, catalog_generation INTEGER NOT NULL, state TEXT NOT NULL, started_at INTEGER NOT NULL, completed_at INTEGER NULL, disposition_reason TEXT NULL, UNIQUE(thread_id, turn_id, agent_id, generation, trigger_digest))`; indexes `(session_id, started_at)`, `(state, started_at)`. Runs retain 180 days plus eval/replay holds.
...
- `diagnostic_envelopes(envelope_id BLOB PRIMARY KEY, diagnostic_code TEXT NOT NULL, severity TEXT NOT NULL, subject_kind TEXT NOT NULL, subject_id BLOB NOT NULL, subject_version BLOB NULL, scope_resolution_id BLOB NOT NULL, summary_blob_id BLOB NOT NULL, state TEXT NOT NULL, actions_blob_id BLOB NOT NULL, evidence_blob_id BLOB NOT NULL, producer_version TEXT NOT NULL, config_digest BLOB NOT NULL, catalog_generation INTEGER NOT NULL, vector_watermark_blob_id BLOB NOT NULL, observed_at INTEGER NOT NULL, expires_at INTEGER NULL)`; indexes `(diagnostic_code, observed_at)`, `(subject_kind, subject_id, state)`, `(expires_at)`. Retention follows subject/evidence horizon; plan 24/01 envelope exactness.
