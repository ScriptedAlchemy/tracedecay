...
| A20 | RC-BLOCKING | Exact PR-head/manual branch activation | PR-head poll activation is now wired: bootstrap injects the daemon-owned scheduler into `pr_autotrack` and `track_pr` prepares a linked worktree then mounts it (`src/daemon/bootstrap.rs`; `src/daemon/pr_autotrack.rs` `track_pr` / `activate_pr_worktree`). Public `reconcile_project` stays fail-closed because that API has no scheduler to inject (`reconciliation_without_scheduler_fails_*`). Manual `branch_add_response` still returns `code_index_scheduler_unavailable` unconditionally (`src/daemon/branch_add.rs:43-70`). |
