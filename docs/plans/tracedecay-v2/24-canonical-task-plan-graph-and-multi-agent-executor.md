...
50. Pre-V2 implementation dispatch uses the plan-00 `tracedecay.v2.completion-ledger/v1` and fail-closed `next-ready` gate. Completion requires exact candidate and current canonical integration SHAs, task/review/remediation/successor/integration lineage, independent exact-candidate approval, every named test receipt, live source/digest and sealed Git ancestry observations, branch/worktree association, and disposition receipts for every required steering directive through the attempt's terminal-CAS fence. Card/task status or comment presence is never an input. Missing, ambiguous, stale, candidate-only, unintegrated, cyclic, duplicate-owner, unresolved, unacknowledged required steering, or retired-obligation evidence emits no eligible packet. Late required steering before terminal CAS aborts completion; after terminal CAS it opens immutable-history remediation, while advisory steering never fences. This repo-local planning contract is replaced—not copied into a parallel runtime authority—when the canonical V2 task graph and sealed application views land.
