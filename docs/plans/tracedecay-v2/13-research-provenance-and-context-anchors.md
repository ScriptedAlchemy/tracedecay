...
The corresponding local code evidence is worktree `/fast/projects/tracedecay/.worktrees/codex-session-catchup-integrity`, branch `fix/session-catchup-integrity`, observed with eight modified files and no commit at this audit. Its bounded semantic diff demonstrates provider-filtered user catch-up, one Hermes scan routed across destinations, handler-process singleflight, a 30-project sub-60-second test, dirty state bound to the concrete graph database, and checkpoint-before-clear. It is **unmerged evidence**, not accepted architecture. Plans 02/03/09/12/14/23/25/26 translate the requirements into canonical profile activity plus attribution projections, durable per-source composite frontiers, exact shard/generation operation identity, and `OperationKernelV1`; they deliberately do not port handler-static state, transcript-body fan-out, branch databases, or V1 sidecars.
...
|---|---|---|
| Publication-base master | `origin/master` commit `e888393368ce8704e6fa123f0daecebd1af9ef8d` | Crate version 0.0.56; release PR #446 merged at this commit after #441/#443/#445. At the 2026-07-11 refresh, draft plan PR #421 was the only open PR. |
| Local catch-up/integrity evidence | Worktree `/fast/projects/tracedecay/.worktrees/codex-session-catchup-integrity`; branch `fix/session-catchup-integrity`; eight modified files, no commit when observed | Unmerged FM-153/FM-154 regression source only. Recheck branch/PR/merge state before implementation and never treat the V1 mechanism as V2 authority. |
...
| v0.0.56 publication | PR #446; merge `e888393368ce8704e6fa123f0daecebd1af9ef8d` | Current source/package baseline containing #441/#443/#445; publication state does not prove the installed/running runtime upgraded. |
...
| v0.0.53 publication | PR #437; head `0960d0d94157ddd3232f7d2114a25e85d7e2a454`; merge `273f50c0372f063b97f4755563a3ded65ef324d5` | Historical publication checkpoint containing #439/#440; superseded as current source baseline by v0.0.56 while remaining the exact version for observations captured there. |
