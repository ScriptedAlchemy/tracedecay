...
The Hermes follow-up is recoverable without copying private transcript content. Public artifact: PR [`#441`](https://github.com/ScriptedAlchemy/tracedecay/pull/441), branch `fix/hermes-plugin-nudge-guard`, remote head `152bfacce3d2bd92e0b34715dda2aea4e1ff9139`, and open review comment `discussion_r3563267059`. Local evidence snapshot: worktree `/fast/projects/tracedecay`, amended head `2662669844f76b709ae8e4114feb7e53a9116c0e`, with a user-owned uncommitted continuation that must not be treated as merged code. TraceDecay `sessions_for(worktree=...)` plus branch-filtered `message_search` identified Codex session `019f4f87-0652-7460-bfee-f492c6858a6c`; scoped read-only LCM stores `2416437`, `2417051`, `2417056`, and `2417199` preserve the request, project-routing explanation, confirmation request, and projectless-memory correction. Durable facts `fact:496` and `fact:551` were recalled and rated helpful. These anchors drive FM-138–FM-143 and the memory/host-profile contracts in plans 01/02/05/06/09/12/16/21/27; no raw Hermes/Telegram log or message payload enters Git.
...
### 4.2C MCP and host-bundle primary-source registry
