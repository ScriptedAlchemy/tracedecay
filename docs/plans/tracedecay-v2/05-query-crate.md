...
- Merged #441/#445 make profile/projectless routing a required query baseline: single Profile-root fact, LCM, memory-status, and message-search requests resolve the profile-activity owner before any project discovery, reject legacy scalar user aliases mixed with compatibility project fields, preserve canonical Profile+Project multi-root reads, and cannot inherit CWD, host profile/home, or previous-session state.
...
Merged #445 adds a V1 regression fixture, not another planner path: a single `ScopeRootV2::Profile` fact, LCM, memory-status, or message-search query—optionally filtered by `DeclaredScope::Profile` or `DeclaredScope::ZeroProject` in the canonical query AST—prunes project discovery before it begins, opens only the profile-activity owner at a captured watermark, and binds cursor/coverage to that exact root. Legacy `memory_scope=user`/`storage_scope=user` is lowered before planning and is invalid with any compatibility project selector. Client CWD, session workspace, host profile/home, or unavailable project shard cannot change the plan or turn missing profile activity into empty success/project fallback.
