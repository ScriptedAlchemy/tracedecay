...
### 5.3A End-to-end latency and anti-stall architecture

- V2 treats indefinite synchronous request waiting and unbounded resource growth as correctness failures, not merely performance regressions. One monotonic request deadline and cancellation token propagate through admission, queue wait, lock wait, daemon IPC, application, query/store, rendering, and each bounded provider/tool call. Queue and lock wait consume the same request budget. Work that outlives its synchronous class returns an `OperationRef` and continues under explicit cancellation and durable progress; the request deadline is never repurposed as an agent, attempt, or workflow completion timeout.
- Every catalog capability declares a latency class, synchronous ceiling, queue and memory ceiling, cancellation boundary, progress contract, idempotency/effect semantics, and typed timeout/unavailable/partial result. Initial reference-machine targets are: local health/status p95 at or below 100 ms; common warm local reads p95 at or below 500 ms; bounded complex reads p95 at or below two seconds; and acknowledgement with `OperationRef` or explicit progress within 250 ms for work that exceeds its synchronous class. No ordinary synchronous agent-facing request may exceed 30 seconds. Provider/model, indexing, migration, recovery, backup, and broad evaluation work is operation-backed and cannot hold an MCP/CLI/API request open awaiting completion. The daemon reserves an independent health/control lane with bounded queues, weighted fairness, memory/RSS ceilings, admission shedding, bulkheads, and circuit breakers. Store recovery is generation-scoped and off the request path; a slow provider call, busy lock, corrupt shard, recovery attempt, or saturated enrichment lane cannot starve control operations or healthy reads.
- Task and workflow execution uses bounded recipient-specific context packets plus retrieval anchors and paged pull, never entire repositories, diffs, boards, transcripts, or sibling prompts. Packets declare token, tool-result, output, retry, queue, and memory ceilings; provider/tool calls declare their own bounded request deadlines and effect-reconciliation rules. Agent attempts and workflows have advisory target durations, durable checkpoints, heartbeat visibility, explicit cancellation, and resumable handoffs, but no automatic wall-clock, per-agent, workflow, or no-progress timeout. Lack of progress raises an observable incident and may prompt an explicit operator/scheduler decision; elapsed time alone never converts incomplete work into success, failure, cancellation, or rerouting.
- Release benchmarks cover cold/warm, current/10x corpus, 1/8/16-agent concurrency, slow/absent provider, lock contention, full queue, disk pressure, corrupt shard, daemon/executor restart, explicit cancellation, and lost acknowledgement. Receipts report per-stage p50/p95/p99, max, queue/lock wait, request timeout/cancel latency, throughput, CPU, RSS/heap slope, bytes/tokens/cost, retries, breaker state, and coverage. Averages alone never pass a gate; unexplained synchronous tail stalls, resource growth, or missing request outcomes block cutover, while long-running operation/agent progress remains visible without automatic termination.
...
- During this V2 implementation program, the checked TraceDecay V2 manifest, completion ledger, and `next-ready` view are the sole task/dependency authority; Hermes Kanban is not used for dispatch, lifecycle, status, or completion. GPT-5.6-Sol owns orchestration, decomposition, cross-branch reasoning, planning, analysis, review, integration, and fenced lifecycle. Opus may execute only one explicitly GPT-decomposed single-branch micro-implementation through the native Claude Code CLI in a later reviewed packet; it receives no planning, design, analysis, adversarial-review, independent-review, integration, or self-approval authority. The handoff records both participants, exact bounded branch, and candidate; native-CLI output alone never completes a task.
...
- PR 14A benchmarks the optional native FastEmbed code path defined by plan 31 through the sole release-excluded root-private benchmark/test adapter; it has no production route or publication authority. `JinaEmbeddingsV2BaseCode` is the primary code candidate and `GTELargeENV15Q` the required comparator, both pinned to an exact FastEmbed/runtime/model/tokenizer/chunker manifest and disabled until the frozen gates pass. The same fixture-only adapter independently compares the fingerprint/normalized-body/structural redundancy baseline against bounded semantic-neighbor augmentation on frozen pair/cluster qrels and hard negatives. Search and redundancy contribution receive separate verdicts. Other retrieval research may record disabled evidence, but it creates no second production embedding runtime.
- PR 14B adds exact-tier-preserving RRF, bounded typed graph expansion, hard-negative mining, cross-project/provider/time holdouts, and per-component ablations over compatible immutable generations. If the redundancy contribution passed independently, it also adds bounded neighbor retrieval, canonical chunk-to-entity pair collapse, structural-plus-semantic explanations, explicit semantic-analogue labeling, and byte-stable structural fallback; semantic proximity alone is never duplication or consolidation authority.
...
Plan [`tracedecay-v2/31-native-fastembed-semantic-code-search.md`](tracedecay-v2/31-native-fastembed-semantic-code-search.md) is the cross-cutting semantic-code-search and optional redundancy-augmentation authority. It adds no crate, vector family, pair store, or transport alias: plan 25 owns deterministic code documents plus structural/fingerprint baselines, plan 02 owns vector/artifact persistence, plan 05 owns ports/fusion/pair classification/fallback, plan 09 owns lifecycle, the root-private adapter alone depends on FastEmbed, plan 20/25E owns generated controls, and plan 15/31J owns the frozen search and pair/cluster evaluation. Historical direct-ORT/Nomic/brute-force CodeGraph designs remain provenance and are superseded for V2 implementation.

**Phase/order split:** PR 14A remains Phase 2 because it is release-excluded evaluation over frozen synthetic code-document and pair/cluster fixtures after PR 2A. Production semantic work is Phase 3 and follows `PR 6C + PR 18D + either accepted PR 14A contribution -> PR 14E -> PR 14B`; PR 14C additionally requires the search contribution to be accepted. A fully disabled PR 14A disposition terminalizes 14E/14B/14C as skipped; it never blocks lexical/structural retrieval or later nonsemantic Phase-2 query work.
...
- **Ordering:** the Plan-32 reconciliation gate precedes PR 38A; PR 38B and PR 38C require 38A; PR 38D requires both 38B and 38C; PR 38E/38F/38H require 38D; PR 38G requires 38E. PR 38I may develop fixture-only adapters after 38D but cannot merge until PR 24F, PR 24P, PR 24S, PR 22F-LE, and PR 30J provide canonical hook-safe-boundary, remote-routing, and accounting contracts. PR 38J requires merged PR 38E–38I.
