...
| Total tracedecay tool calls | ~200 |
...
**Gap:** `tracedecay_callers` covers call sites of functions/methods. Nothing covers field
reads vs. writes.
...
**Gap:** `tracedecay_type_hierarchy` handles class inheritance. Nothing returns "all types that
...
`tracedecay_affected`) is not used to scope checks.

**Proposed API:**
```json
{ "scope": "workspace" }
{ "scope": "affected", "since": "HEAD~1" }
{ "scope": "package", "name": "tracedecay" }
...
structured response that's already mapped to graph nodes. The biggest single Bash : tracedecay
...
**Gap:** No AST-aware finder for these markers. `tracedecay_todos` covers comment-style
...
Pairs with `tracedecay_test_risk` for "what risky paths aren't tested?"
...
that takes `&mut self`". Currently answered by `tracedecay_search` (name-only) plus
...
**Gap:** No tool parses lockfiles. `tracedecay_branch_diff` covers symbol-level diffs
...
**Gap:** TraceDecay's graph is project-local. External symbols are unreachable; users have
...
`tracedecay_callers`-style queries work against external code.
...
Smaller blast radius than diagnostics but disproportionately common in the tracedecay
...
Observed across all projects (tracedecay + claurst + bruto-pascal + amexx + others). The
...
| `tracedecay_field_sites` | proposed | tracedecay e790d3c4, 67f09223 | Medium (field write edges) | High |
| `tracedecay_constructors` | proposed | tracedecay e790d3c4 | Low (struct-literal node kind) | Medium |
| `tracedecay_feature_map` | proposed | tracedecay 67f09223, others | High (full-text index) | Medium |
| `tracedecay_config` | proposed | 62 config greps | Medium (TOML/YAML parser) | Medium |
| `tracedecay_lockfile_diff` | proposed | 6 lockfile greps | Low (lockfile parser) | Low-Medium |
| `tracedecay_external_node` | proposed | recurring registry spelunking | High (on-demand indexing) | Medium |
| `tracedecay_macro_expand` | proposed | niche, high cost when used | High (cargo expand integration) | Low-Medium |
...
`tracedecay_field_sites`, `tracedecay_feature_map`, and `tracedecay_external_node` may need a
