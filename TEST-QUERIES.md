...
> **Note:** Most tools are read-only and safe to call in parallel. The exceptions mutate state and should not be parallelised: the edit tools (`tracedecay_str_replace`, `tracedecay_multi_str_replace`, `tracedecay_insert_at`, `tracedecay_insert_at_symbol`, `tracedecay_replace_symbol`, `tracedecay_ast_grep_rewrite`) modify source files; `tracedecay_fact_store_add`, `tracedecay_fact_store_update`, `tracedecay_fact_store_remove`, and `tracedecay_fact_feedback` write to `.tracedecay/`; and `tracedecay_run_affected_tests` runs a `cargo` test subprocess. The other exact `tracedecay_fact_store_*` routes and `tracedecay_memory_status` only read current state.
